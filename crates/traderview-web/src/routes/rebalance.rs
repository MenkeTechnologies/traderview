...
use traderview_db::rebalance::{
    PlanResponse, RebalanceTarget, RebalanceTargetInput, RunBody,
};
use uuid::Uuid;
...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route("/rebalance/targets/:id",    axum::routing::delete(delete_target))
...
async fn list_targets(
    ...
    Ok(Json(traderview_db::rebalance::list(&s.pool, u.id)
...
async fn save_target(
    ...
    Ok(Json(traderview_db::rebalance::create(&s.pool, u.id, &body)
        .await.map_err(ApiError::Internal)?))
}
...
async fn run(
    ...
    Ok(Json(traderview_db::rebalance::run(&s.pool, &body)
        .await.map_err(ApiError::Internal)?))
}
...
async fn run_csv(
    ...
    for t in &r.plan.trades {
        csv.push_str(&format!(
            "{},{},{},{:.4},{:.2},{},{},{:.4},{:.4},{:.4}\n",
            t.symbol, t.side, t.trade_qty, t.price, t.trade_value,
            t.current_qty as i64, t.target_qty,
            t.current_pct, t.target_pct, t.drift_pct,
        ));
    }
    Ok((
        StatusCode::OK,
        [
            ...
            (CONTENT_DISPOSITION,
             HeaderValue::from_str(&format!("attachment; filename=\"rebalance-{}.csv\"", body.account_id))
