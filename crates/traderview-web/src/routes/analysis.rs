...
struct MQ {
    symbols: String,        // CSV
    #[serde(default = "default_days")] days: i64,
}
...
async fn matrix(State(s): State<AppState>, _u: AuthUser, Query(q): Query<MQ>)
    -> Result<Json<MatrixResp>, ApiError>
{
    let syms: Vec<String> = q.symbols.split(',').map(|x| x.trim().to_uppercase()).filter(|s| !s.is_empty()).collect();
    if syms.len() < 2 { return Err(ApiError::BadRequest("need ≥ 2 symbols".into())); }
    let to = Utc::now();
    ...
    if min_n < 5 { return Err(ApiError::BadRequest("not enough bars".into())); }
    let returns: Vec<Vec<f64>> = all_closes.iter()
    ...
    for i in 0..n {
        for j in 0..n {
            mat[i][j] = if i == j { 1.0 } else {
                pearson(&returns[i], &returns[j]).unwrap_or(0.0)
            };
        }
    }
    Ok(Json(MatrixResp { symbols: syms, matrix: mat, days: q.days, samples: min_n }))
}

fn dec(d: Decimal) -> f64 { d.to_string().parse().unwrap_or(0.0) }

#[derive(Deserialize)]
struct PQ { a: String, b: String, #[serde(default = "default_days")] days: i64 }

async fn pair(State(s): State<AppState>, _u: AuthUser, Query(q): Query<PQ>)
    -> Result<Json<PairAnalysis>, ApiError>
{
    let to = Utc::now();
    let from = to - Duration::days(q.days);
    let ba = traderview_db::prices::get_bars(&s.pool, &q.a.to_uppercase(), BarInterval::D1, from, to)
        .await.map_err(ApiError::Internal)?;
    let bb = traderview_db::prices::get_bars(&s.pool, &q.b.to_uppercase(), BarInterval::D1, from, to)
        .await.map_err(ApiError::Internal)?;
    let pa: Vec<f64> = ba.iter().map(|b| dec(b.close)).collect();
    let pb: Vec<f64> = bb.iter().map(|b| dec(b.close)).collect();
    let n = pa.len().min(pb.len());
    if n < 30 { return Err(ApiError::BadRequest("need ≥ 30 overlapping bars".into())); }
    let pa = &pa[pa.len() - n..];
    let pb = &pb[pb.len() - n..];
    pair_analysis(pa, pb).map(Json)
