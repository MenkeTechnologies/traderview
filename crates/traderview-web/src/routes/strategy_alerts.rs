...
async fn list(
    ...
    Ok(Json(
        traderview_db::strategy_alerts::list(&s.pool, u.id)
            .await
...
async fn create(
    ...
    Ok(Json(
        traderview_db::strategy_alerts::create(&s.pool, u.id, &body)
            .await
            .map_err(ApiError::Internal)?,
    ))
}
...
async fn recent_fires(
    ...
    Ok(Json(
        traderview_db::strategy_alerts::recent_fires(&s.pool, u.id, 100)
            .await
            .map_err(ApiError::Internal)?,
    ))
}
