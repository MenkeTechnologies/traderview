...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route("/symbols/:symbol/financials-reported", get(financials_reported))
        ...
        .route("/symbols/:symbol/net-income-estimate", get(net_income_estimate))
        .route("/symbols/:symbol/pretax-income-estimate", get(pretax_income_estimate))
        .route("/symbols/:symbol/gross-income-estimate", get(gross_income_estimate))
        ...
        .route("/symbols/:symbol/dividends-basic", get(stock_basic_dividends))
        ...
        .route("/symbols/:symbol/finnhub-insiders", get(insider_transactions))
        ...
        .route("/symbols/:symbol/historical-market-cap", get(historical_market_cap))
        .route("/symbols/:symbol/historical-employee-count", get(historical_employee_count))
        .route("/symbols/:symbol/earnings-quality-score", get(earnings_quality_score))
        .route("/symbols/:symbol/revenue-breakdown", get(revenue_breakdown))
        .route("/symbols/:symbol/revenue-breakdown2", get(revenue_breakdown2))
        .route("/symbols/:symbol/presentation", get(presentation))
        .route("/symbols/:symbol/newsroom", get(newsroom))
        .route("/symbols/:symbol/congressional-trading", get(congressional_trading))
        ...
        .route("/finnhub/calendar/earnings-call-live", get(earnings_call_live))
        // ── broad news ──────────────────────────────────────────────
        ...
        .route("/finnhub/index/:symbol/constituents", get(index_constituents))
        .route("/finnhub/index/:symbol/historical-constituents", get(index_historical))
        ...
        .route("/finnhub/institutional/:cik/profile", get(institutional_profile))
        .route("/finnhub/institutional/:cik/portfolio", get(institutional_portfolio))
        .route("/finnhub/institutional/:symbol/ownership", get(institutional_ownership))
        // ── meta / discovery ────────────────────────────────────────
...
fn from_to_default(q: FromTo, window_days: i64) -> (String, String) {
    ...
    let from = q.from.unwrap_or_else(|| (today - chrono::Duration::days(window_days)).to_string());
...
#[derive(Deserialize)]
struct FreqQuery { freq: Option<String> }
#[derive(Deserialize)]
struct ResolutionQuery { resolution: Option<String> }
#[derive(Deserialize)]
struct DateQuery { date: Option<String> }
#[derive(Deserialize)]
struct CalEarningsQuery { from: Option<String>, to: Option<String>, symbol: Option<String> }
#[derive(Deserialize)]
struct CategoryQuery { category: Option<String> }
#[derive(Deserialize)]
struct BaseQuery { base: Option<String> }
#[derive(Deserialize)]
...
#[derive(Deserialize)]
struct QQuery { q: String }
#[derive(Deserialize)]
struct ThemeQuery { theme: String }
#[derive(Deserialize)]
struct FormQuery { from: Option<String>, to: Option<String>, form: Option<String> }
#[derive(Deserialize)]
struct ExchangeOnly { #[serde(default)] exchange: String }
#[derive(Deserialize)]
struct AirlineQuery {
    airline: String,
    from: Option<String>,
...
#[derive(Deserialize)]
struct HoldingsSkip { skip: Option<i64> }

fn today_str() -> String { chrono::Utc::now().date_naive().to_string() }
