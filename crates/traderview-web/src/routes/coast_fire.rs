...
use traderview_db::coast_fire;

pub fn router() -> Router<AppState> {
    Router::new().route("/coast-fire/compute", post(compute))
}

async fn compute(
    State(_s): State<AppState>,
    _user: AuthUser,
    Json(input): Json<coast_fire::CoastFireInput>,
) -> Result<Json<coast_fire::CoastFireReport>, ApiError> {
    ...
    Ok(Json(coast_fire::compute(&input)))
}
