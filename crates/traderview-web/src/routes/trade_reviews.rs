...
async fn list(
    ...
    Ok(Json(traderview_db::trade_reviews::list(&s.pool, u.id, limit)
...
async fn needed(
    ...
    let limit = p.limit.unwrap_or(50).clamp(1, 500);
    Ok(Json(traderview_db::trade_reviews::needs_review(&s.pool, u.id, account_id, limit)
        .await.map_err(ApiError::Internal)?))
}
...
async fn stats(
    ...
    ensure_account_owner(&s, u.id, account_id).await?;
    Ok(Json(traderview_db::trade_reviews::stats(&s.pool, u.id, account_id)
        .await.map_err(ApiError::Internal)?))
}
...
async fn for_trade(
    ...
    Ok(Json(traderview_db::trade_reviews::for_trade(&s.pool, u.id, trade_id)
        .await.map_err(ApiError::Internal)?))
}
...
async fn upsert(
    ...
    Ok(Json(traderview_db::trade_reviews::upsert(&s.pool, u.id, &body)
        .await.map_err(ApiError::Internal)?))
}
