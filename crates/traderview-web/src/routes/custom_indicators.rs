...
use traderview_db::custom_indicators::{
    CustomIndicator, EvalResult, IndicatorInput,
};
use uuid::Uuid;
...
async fn list(
    ...
    Ok(Json(traderview_db::custom_indicators::list(&s.pool, u.id)
...
async fn create(
    ...
    Ok(Json(traderview_db::custom_indicators::create(&s.pool, u.id, &body)
        .await.map_err(ApiError::Internal)?))
}
...
async fn eval(
    ...
    Ok(Json(traderview_db::custom_indicators::evaluate(
        &s.pool, u.id, &sym, interval, from, to, &b.indicator_ids,
    ).await.map_err(ApiError::Internal)?))
}
