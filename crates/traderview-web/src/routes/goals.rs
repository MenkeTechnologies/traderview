...
async fn list(
    ...
    Ok(Json(traderview_db::goals::list(&s.pool, u.id)
...
async fn create(
    ...
    if body.end_date < body.start_date {
        return Err(ApiError::BadRequest("end_date must be >= start_date".into()));
    }
    Ok(Json(traderview_db::goals::create(&s.pool, u.id, &body)
        .await.map_err(ApiError::Internal)?))
}
...
async fn progress(
    ...
    Ok(Json(traderview_db::goals::progress(&s.pool, u.id, id)
        .await.map_err(ApiError::Internal)?))
}
