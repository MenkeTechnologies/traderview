...
async fn by_symbol(
    ...
    Ok(Json(
        traderview_db::news::recent_for_symbol(&s.pool, &sym, limit)
            .await
...
async fn recent_global(
    ...
    Ok(Json(
        traderview_db::news::recent_global(&s.pool, limit)
            .await
            .map_err(ApiError::Internal)?,
    ))
}
...
async fn search(
    ...
    Ok(Json(
        traderview_db::news::search(&s.pool, &p.q, limit)
            .await
            .map_err(ApiError::Internal)?,
    ))
}

async fn poll_now(State(s): State<AppState>, _u: AuthUser) -> Result<Json<PollStats>, ApiError> {
    Ok(Json(
        traderview_db::news::poll_watchlists(&s.pool)
            .await
            .map_err(ApiError::Internal)?,
    ))
}
