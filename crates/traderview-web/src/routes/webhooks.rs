...
async fn list(State(s): State<AppState>, user: AuthUser) -> Result<Json<Vec<Webhook>>, ApiError> {
    Ok(Json(
        traderview_db::webhooks::list(&s.pool, user.id)
            .await
...
async fn create(
    ...
    if !["discord", "slack", "generic"].contains(&b.kind.as_str()) {
        return Err(ApiError::BadRequest(
            "kind must be discord|slack|generic".into(),
        ));
    }
    Ok(Json(
        traderview_db::webhooks::create(
            &s.pool,
            user.id,
            &b.name,
            &b.kind,
...
async fn delete_one(
    ...
    Ok(Json(
        traderview_db::webhooks::delete(&s.pool, user.id, id)
            .await
            .map_err(ApiError::Internal)?,
    ))
}
...
async fn toggle(
    ...
    Ok(Json(
        traderview_db::webhooks::toggle(&s.pool, user.id, id, b.enabled)
            .await
            .map_err(ApiError::Internal)?,
    ))
}
