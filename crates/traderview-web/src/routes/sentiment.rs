...
async fn feed(
    ...
    Ok(Json(
        traderview_db::sentiment::feed(&s.pool, q.limit)
            .await
...
async fn ranked(
    ...
    Ok(Json(
        traderview_db::sentiment::ranked(&s.pool, q.hours, q.limit)
            .await
            .map_err(ApiError::Internal)?,
    ))
}
...
async fn poll_now(
    ...
    Ok(Json(
        serde_json::json!({"wsb_inserted": wsb, "stocktwits_inserted": st}),
    ))
}

#[derive(Deserialize)]
struct SymQ {
    #[serde(default = "default_sym_hours")]
    hours: i64,
    #[serde(default = "default_sym_limit")]
    limit: i64,
}
fn default_sym_hours() -> i64 {
    24
}
fn default_sym_limit() -> i64 {
    100
}
...
async fn per_symbol(
    ...
    Ok(Json(
        traderview_db::sentiment::for_symbol(&s.pool, &sym.to_uppercase(), q.hours, q.limit)
            .await
            .map_err(ApiError::Internal)?,
    ))
}
...
async fn series(
    ...
    Ok(Json(
        traderview_db::sentiment::timeseries(&s.pool, &sym.to_uppercase(), q.hours)
            .await
            .map_err(ApiError::Internal)?,
    ))
}
