...
async fn calendar(
    ...
    Ok(Json(
        traderview_db::earnings_cal::calendar_upcoming(&s.pool, days)
            .await
...
async fn surprises(
    ...
    Ok(Json(
        traderview_db::earnings_cal::surprises_recent(&s.pool, days)
            .await
            .map_err(ApiError::Internal)?,
    ))
}
...
async fn poll_now(State(s): State<AppState>, _u: AuthUser) -> Result<Json<PollStats>, ApiError> {
    Ok(Json(
        traderview_db::earnings_cal::poll_watchlists(&s.pool)
            .await
            .map_err(ApiError::Internal)?,
    ))
}
