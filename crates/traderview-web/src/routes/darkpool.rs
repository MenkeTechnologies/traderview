...
#[derive(Deserialize)]
struct Q { #[serde(default = "default_days")] days: i64 }
fn default_days() -> i64 { 30 }

async fn symbol(State(s): State<AppState>, _u: AuthUser, Path(sym): Path<String>, Query(q): Query<Q>)
    -> Result<Json<DarkSeries>, ApiError>
{
    Ok(Json(traderview_db::darkpool::series(&s.pool, &sym, q.days)
...
async fn ranked(State(s): State<AppState>, user: AuthUser, Query(q): Query<RQ>)
    -> Result<Json<Vec<DarkRanked>>, ApiError>
{
    ...
    Ok(Json(traderview_db::darkpool::ranked(&s.pool, &universe, q.days).await))
}
