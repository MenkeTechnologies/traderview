...
use traderview_db::lean_fire;

pub fn router() -> Router<AppState> {
    Router::new().route("/lean-fire/compute", post(compute))
}

async fn compute(
    State(_s): State<AppState>,
    _user: AuthUser,
    Json(input): Json<lean_fire::LeanFireInput>,
) -> Result<Json<lean_fire::LeanFireReport>, ApiError> {
    ...
    Ok(Json(lean_fire::compute(&input)))
}
