...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route("/backtest-presets/:id",       axum::routing::delete(delete).put(update))
...
async fn list_mine(
    ...
    Ok(Json(traderview_db::backtest_presets::list_mine(&s.pool, u.id)
...
async fn list_public(
    ...
    Ok(Json(traderview_db::backtest_presets::list_public(&s.pool, limit)
        .await.map_err(ApiError::Internal)?))
}
...
async fn create(
    ...
    Ok(Json(traderview_db::backtest_presets::create(&s.pool, u.id, &body)
        .await.map_err(ApiError::Internal)?))
}
...
async fn fork(
    ...
    Ok(Json(traderview_db::backtest_presets::fork(&s.pool, u.id, &slug)
        .await.map_err(ApiError::Internal)?))
}
