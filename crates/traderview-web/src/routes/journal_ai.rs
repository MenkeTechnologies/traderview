...
async fn get_settings(
    ...
    Ok(Json(traderview_db::journal_ai::get_llm_settings(&s.pool, u.id)
...
async fn analyze(
    ...
    Ok(Json(traderview_db::journal_ai::analyze(&s.pool, u.id, trade_id)
        .await.map_err(ApiError::Internal)?))
}
...
async fn cached(
    ...
    let (_, hash) = traderview_db::journal_ai::build_context(&s.pool, u.id, trade_id)
        .await.map_err(ApiError::Internal)?;
    Ok(Json(traderview_db::journal_ai::get_cached(&s.pool, u.id, trade_id, &hash)
        .await.map_err(ApiError::Internal)?))
}
