...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route(
            ...
            post(order_book_imbalance_route),
        )
        .route(
            "/microstructure/order-flow-classify",
            post(order_flow_classify_route),
        )
        .route(
            "/microstructure/order-flow-aggregate",
            post(order_flow_aggregate_route),
        )
        .route("/microstructure/liquidity", post(liquidity_route))
        .route("/microstructure/market-impact", post(market_impact_route))
        .route(
            "/microstructure/per-symbol-slippage",
            post(per_symbol_slippage_route),
        )
        .route("/microstructure/vwap-slippage", post(vwap_slippage_route))
        .route(
            "/microstructure/order-staleness",
            post(order_staleness_route),
        )
        // ── Heatmaps ───────────────────────────────────────────────────
        ...
        .route(
            "/discipline/trade-plan-checklist",
            post(trade_plan_checklist_route),
        )
        .route(
            "/discipline/stop-loss-backtest",
            post(stop_loss_backtest_route),
        )
        .route(
            "/discipline/stop-loss-best-of",
            post(stop_loss_best_of_route),
        )
        ...
        .route(
            "/microstructure/implementation-shortfall",
            post(implementation_shortfall_route),
        )
}
...
async fn order_book_imbalance_route(
    ...
    Json(order_book_imbalance::compute(
        &b.bid_sizes,
        &b.ask_sizes,
        b.levels,
    ))
...
#[derive(Deserialize)]
struct OrderFlowBody {
    ticks: Vec<order_flow::Tick>,
}

async fn order_flow_classify_route(
    _u: AuthUser,
    Json(b): Json<OrderFlowBody>,
...
#[derive(Deserialize)]
struct PerSymbolSlippageBody {
    records: Vec<SlippageRecord>,
}

async fn per_symbol_slippage_route(
    _u: AuthUser,
    Json(b): Json<PerSymbolSlippageBody>,
) -> Json<Vec<per_symbol_slippage::SymbolSlippage>> {
    let tuples: Vec<(String, f64)> = b
        .records
        .into_iter()
...
async fn vwap_slippage_route(
    _u: AuthUser,
    Json(b): Json<VwapSlippageBody>,
) -> Json<VwapSlippageResp> {
    let input = vwap_slippage::VwapInput {
        side: b.side,
    ...
    let resp = match vwap_slippage::compute(&input) {
        ...
        None => VwapSlippageResp::Empty {
            reason: "no bars or zero total volume",
        },
    };
    Json(resp)
}
...
#[derive(Deserialize)]
struct IntradayHeatmapBody {
    trades: Vec<intraday_heatmap::IntradayTrade>,
}

async fn intraday_heatmap_route(
    _u: AuthUser,
    Json(b): Json<IntradayHeatmapBody>,
) -> Json<intraday_heatmap::IntradayHeatmapReport> {
    Json(intraday_heatmap::build(&b.trades))
}

#[derive(Deserialize)]
struct DowHourHeatmapBody {
    trades: Vec<Trade>,
}

async fn dow_hour_heatmap_route(
    _u: AuthUser,
    Json(b): Json<DowHourHeatmapBody>,
...
async fn open_type_route(
    _u: AuthUser,
    Json(input): Json<open_type::OpenInput>,
...
async fn stop_loss_backtest_route(
    _u: AuthUser,
    Json(b): Json<StopBacktestBody>,
) -> Json<stop_loss_backtest::MethodResult> {
    Json(stop_loss_backtest::simulate(
        &b.trades,
        &b.params,
        b.side_long,
    ))
...
async fn stop_loss_best_of_route(
    _u: AuthUser,
    Json(b): Json<StopBestOfBody>,
) -> Json<Vec<stop_loss_backtest::MethodResult>> {
    Json(stop_loss_backtest::best_of(
        &b.trades,
        &b.candidates,
        b.side_long,
    ))
...
async fn pyramid_plan_route(
    _u: AuthUser,
    Json(input): Json<pyramid::PlanInput>,
...
async fn iv_rank_route(_u: AuthUser, Json(b): Json<IvRankBody>) -> Json<iv_rank::IvRankReport> {
...
async fn cluster_analysis_route(
    _u: AuthUser,
    Json(b): Json<ClusterAnalysisBody>,
...
async fn correlation_clusters_route(
    _u: AuthUser,
    Json(b): Json<CorrelationClustersBody>,
    ...
    Json(correlation_clusters::cluster(
        &b.positions,
        &corr,
        b.threshold,
    ))
...
async fn footprint_route(
    ...
    if !(b.tick_size.is_finite() && b.tick_size > 0.0) {
        return Err(ApiError::BadRequest(
            "tick_size must be a positive finite number".into(),
        ));
    }
...
async fn stress_test_route(
    _u: AuthUser,
    Json(input): Json<stress_test::StressInput>,
...
#[derive(Deserialize)]
struct CohortTiltBody {
    positions: Vec<tilt_indicator::TraderPosition>,
}

async fn cohort_tilt_route(
    _u: AuthUser,
    Json(b): Json<CohortTiltBody>,
...
#[derive(Deserialize)]
struct SpreadTrackerBody {
    samples: Vec<spread_tracker::QuoteSnapshot>,
}

async fn spread_tracker_route(
    _u: AuthUser,
    Json(b): Json<SpreadTrackerBody>,
...
async fn implementation_shortfall_route(
    _u: AuthUser,
    Json(input): Json<implementation_shortfall::ShortfallInput>,
