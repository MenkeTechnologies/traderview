...
use traderview_db::debt_snowball;

pub fn router() -> Router<AppState> {
    Router::new().route("/debt-snowball/compute", post(compute))
}

async fn compute(
    State(_s): State<AppState>,
    _user: AuthUser,
    Json(input): Json<debt_snowball::DebtSnowballInput>,
) -> Result<Json<debt_snowball::DebtSnowballReport>, ApiError> {
    ...
    Ok(Json(debt_snowball::compute(&input)))
}
