    ...
    for (id, symbol, side, asset_class, qty, entry, mult, opened_at) in rows {
        let q = match quote_cache.get(&symbol) {
            ...
            None => match crate::market_data::quote(pool, &symbol).await {
                Ok(q) => { quote_cache.insert(symbol.clone(), q.clone()); q }
                Err(_) => continue,
            },
        };
        let qty_f      = dec(qty);
        ...
        let day_pnl = q.prev_close.map(|pc| (last - pc) * qty_f * mult_f * side_sign);
        positions.push(LivePosition {
            trade_id: id, symbol: symbol.clone(), side, asset_class,
            qty: qty_f, entry_avg: entry_f, multiplier: mult_f,
            last_price: last, prev_close: q.prev_close, change_pct: q.change_pct,
            notional, unrealized_pnl: unrealized, unrealized_pct,
            day_pnl, opened_at, market_state: q.market_state,
        });
    }
