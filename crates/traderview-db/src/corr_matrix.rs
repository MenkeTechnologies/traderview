...
pub async fn for_watchlist(pool: &PgPool, user_id: Uuid, watchlist_id: Uuid, days: i64)
    -> anyhow::Result<CorrMatrix>
{
    let symbols: Vec<String> = sqlx::query_scalar(
        "SELECT s.symbol FROM watchlist_symbols s
          ...
          ORDER BY s.symbol",
    ).bind(watchlist_id).bind(user_id).fetch_all(pool).await?;
    matrix_from_symbols(pool, &symbols, days).await
}

pub async fn for_symbols(pool: &PgPool, symbols: &[String], days: i64)
    -> anyhow::Result<CorrMatrix>
{
    matrix_from_symbols(pool, symbols, days).await
}

async fn matrix_from_symbols(pool: &PgPool, symbols: &[String], days: i64)
    -> anyhow::Result<CorrMatrix>
{
    let to = Utc::now();
    ...
    for i in 0..n {
        ...
        for j in (i + 1)..n {
            ...
            values[i][j] = v;
            values[j][i] = v;
            pairs.push(CorrCell { a: a.clone(), b: b.clone(), value: v, samples });
        }
    }
    ...
    let top_diversifying: Vec<CorrCell> = if any_neg {
        ...
        by_abs.sort_by(|x, y| x.value.unwrap_or(0.0).abs()
            .partial_cmp(&y.value.unwrap_or(0.0).abs()).unwrap());
    ...
    Ok(CorrMatrix {
        ...
        days, values, pairs,
        top_correlated, top_diversifying,
        computed_at: Utc::now(),
    })
}

fn align_series(a: &BTreeMap<chrono::NaiveDate, f64>, b: &BTreeMap<chrono::NaiveDate, f64>)
