...
pub async fn snapshot(pool: &PgPool) -> anyhow::Result<BreadthSnapshot> {
    let tick = fetch(pool, "^TICK", "NYSE TICK", |v| {
        if v >= 800.0 { "extreme buying" }
        else if v >= 400.0 { "buying" }
        else if v >= -400.0 { "balanced" }
        else if v >= -800.0 { "selling" }
        else { "extreme selling" }.into()
    }).await;
    let trin = fetch(pool, "^TRIN", "NYSE TRIN (Arms)", |v| {
        // <1 = bullish (volume into up issues), >1 = bearish
        if v <= 0.5 { "very bullish (extreme buying)" }
        else if v <= 0.9 { "bullish" }
        else if v <= 1.1 { "neutral" }
        else if v <= 2.0 { "bearish" }
        else { "very bearish (extreme selling)" }.into()
    }).await;
    let addn = fetch(pool, "^ADD", "NYSE Adv−Dec issues", |v| {
        if v >= 1500.0 { "broad rally" }
        else if v >= 500.0 { "advances leading" }
        else if v >= -500.0 { "mixed" }
        else if v >= -1500.0 { "declines leading" }
        else { "broad selloff" }.into()
    }).await;
    let vold = fetch(pool, "^VOLD", "NYSE Up−Down vol", |v| {
        if v >= 0.0 { "more up-volume" } else { "more down-volume" }.into()
    }).await;
    // CBOE put-call total: ^CPC (sometimes ^PCC). Try a couple.
    let pcr = fetch(pool, "^CPC", "Put-Call ratio (CBOE)", |v| {
        if v <= 0.6 { "extreme greed (low puts → contrarian sell)" }
        else if v <= 0.8 { "bullish" }
        else if v <= 1.0 { "balanced" }
        else if v <= 1.2 { "bearish" }
        else { "extreme fear (high puts → contrarian buy)" }.into()
    }).await;
    ...
    let regime = if score >=  30 { "bullish" }
                 else if score <= -30 { "bearish" }
                 else                 { "neutral" };
    Ok(BreadthSnapshot {
        tick, trin, addn, vold, pcr,
...
async fn fetch<F>(pool: &PgPool, sym: &'static str, label: &'static str, interp: F) -> Option<Indicator>
where F: FnOnce(f64) -> String,
{
