...
pub async fn report(
    ...
    for (pid, _ltid, psym, p_qty, p_stop, tid, opened, exit_avg, side, t_qty, t_stop) in plan_rows {
        ...
        let p_side: Option<String> =
            sqlx::query_scalar("SELECT side::text FROM trade_plans WHERE id = $1")
                .bind(pid)
                .fetch_optional(pool)
                .await
                .ok()
    ...
    let window = |label: &'static str, days: i64| -> WindowScore {
        ...
        let in_w: Vec<&RuleEval> = rule_evals.iter().filter(|r| r.date >= cutoff).collect();
        let linked = in_w.len();
        let pass = in_w.iter().filter(|r| r.overall_pass).count();
        let pct = if linked > 0 {
            pass as f64 / linked as f64 * 100.0
        } else {
            0.0
        };
        WindowScore {
    ...
    let all_time = WindowScore {
        ...
        discipline_pct: if rule_evals.is_empty() {
            0.0
        } else {
            rule_evals.iter().filter(|r| r.overall_pass).count() as f64 / rule_evals.len() as f64
                * 100.0
        },
    };
