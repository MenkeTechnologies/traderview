...
pub async fn delete(pool: &PgPool, user_id: Uuid, id: Uuid) -> anyhow::Result<bool> {
    Ok(
        sqlx::query("DELETE FROM backtest_presets WHERE id = $1 AND user_id = $2")
            .bind(id)
            .bind(user_id)
            .execute(pool)
            .await?
...
pub async fn fork(
    ...
    let source = by_slug(pool, source_slug)
        .await?
    ...
    let existing: Vec<(String,)> =
        sqlx::query_as("SELECT name FROM backtest_presets WHERE user_id = $1 AND name LIKE $2")
            .bind(user_id)
            .bind(format!("{} (fork%", source.name))
            .fetch_all(pool)
            .await
            .unwrap_or_default();
    ...
    let _ = sqlx::query("UPDATE backtest_presets SET fork_count = fork_count + 1 WHERE id = $1")
        .bind(source.id)
        .execute(pool)
        .await;
    Ok(row)
...
async fn generate_unique_slug(pool: &PgPool, name: &str) -> anyhow::Result<String> {
    ...
    for attempt in 0..6 {
        ...
        let exists: Option<(Uuid,)> =
            sqlx::query_as("SELECT id FROM backtest_presets WHERE slug = $1")
                .bind(&candidate)
                .fetch_optional(pool)
                .await?;
        if exists.is_none() {
