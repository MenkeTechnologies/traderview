...
pub async fn report(pool: &PgPool, user_id: Uuid, account_id: Uuid) -> anyhow::Result<MoodReport> {
    ...
    let per_day: Vec<MoodPerDayRow> = sqlx::query_as(
    ...
    for p in &pairs {
        by_mood.entry(p.mood).or_default().push(p);
    }
