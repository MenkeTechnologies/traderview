...
pub async fn ensure_local(pool: &PgPool) -> anyhow::Result<Uuid> {
    if let Some((id,)) = sqlx::query_as::<_, (Uuid,)>(
        "SELECT id FROM users WHERE is_local = true LIMIT 1",
    )
    .fetch_optional(pool)
    .await?
    {
...
pub async fn find_by_email(pool: &PgPool, email: &str) -> anyhow::Result<Option<UserAuth>> {
    let row: Option<UserAuthRow> =
        ...
        .fetch_optional(pool)
        .await?;
    Ok(row.map(|(id, email, password_hash, display_name, is_local, created_at)| UserAuth {
        user: User { id, email, display_name, is_local, created_at },
        password_hash,
    }))
}
...
pub async fn find_by_id(pool: &PgPool, id: Uuid) -> anyhow::Result<Option<User>> {
    let row: Option<UserRow> =
        ...
        .fetch_optional(pool)
        .await?;
    Ok(row.map(|(id, email, display_name, is_local, created_at)| User {
        id, email, display_name, is_local, created_at,
    }))
}
