...
pub enum AbcdDirection { #[default] Bullish, Bearish }

#[derive(Debug, Clone, Copy, Serialize, Deserialize)]
pub struct AbcdMatch {
    pub direction: AbcdDirection,
    pub a: Pivot, pub b: Pivot, pub c: Pivot, pub d: Pivot,
    pub bc_to_ab: f64,
    pub cd_to_ab: f64,
    pub cd_to_bc: f64,
}
...
pub fn detect(pivots: &[Pivot], tolerance: f64) -> Vec<AbcdMatch> {
    ...
    for w in pivots.windows(4) {
        ...
        if ab <= 0.0 || bc <= 0.0 { continue; }
        let bc_to_ab = bc / ab;
        let cd_to_ab = cd / ab;
        let cd_to_bc = cd / bc;
        if !(0.618 - tolerance..=0.786 + tolerance).contains(&bc_to_ab) { continue; }
        if !(1.000 - tolerance..=1.272 + tolerance).contains(&cd_to_ab) { continue; }
        if !(1.272 - tolerance..=1.618 + tolerance).contains(&cd_to_bc) { continue; }
        let direction = if a.is_high { AbcdDirection::Bullish } else { AbcdDirection::Bearish };
        out.push(AbcdMatch {
            direction, a, b, c, d, bc_to_ab, cd_to_ab, cd_to_bc,
        });
    }
    out
}
