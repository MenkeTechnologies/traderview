...
pub fn compute(bars: &[Bar], period: usize) -> DonchianReport {
    ...
    if period < 2 || n < period {
        return DonchianReport { upper, middle, lower };
    }
    for i in (period - 1)..n {
        ...
        if !hi.is_finite() || !lo.is_finite() { continue; }
        upper[i] = Some(hi);
        lower[i] = Some(lo);
        middle[i] = Some((hi + lo) / 2.0);
    }
    DonchianReport { upper, middle, lower }
}
...
mod tests {
    ...
    fn middle_is_midpoint_of_upper_and_lower() {
        let bars: Vec<_> = (0..30).map(|i| b(100.0 + i as f64, 90.0 - i as f64)).collect();
        let r = compute(&bars, 10);
        for i in 9..30 {
