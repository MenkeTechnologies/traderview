...
pub fn svensson_yield(tenor: f64, p: &NssParams) -> f64 {
    if tenor <= 0.0 || p.tau1 <= 0.0 || p.tau2 <= 0.0 || !p.tau1.is_finite() || !p.tau2.is_finite()
    {
        return f64::NAN;
    }
    let x1 = tenor / p.tau1;
...
pub fn fit_nelson_siegel(tenors: &[f64], yields: &[f64], tau: f64) -> Option<NsParams> {
    ...
    if b.len() != 3 || b.iter().any(|x| !x.is_finite()) {
        return None;
    }
    Some(NsParams {
        beta0: b[0],
...
pub fn fit_svensson(tenors: &[f64], yields: &[f64], tau1: f64, tau2: f64) -> Option<NssParams> {
    ...
    if b.len() != 4 || b.iter().any(|x| !x.is_finite()) {
        return None;
    }
    Some(NssParams {
        beta0: b[0],
...
mod tests {
    ...
    fn fit_recovers_known_ns_params() {
        ...
        let yields: Vec<f64> = tenors
            .iter()
            .map(|t| nelson_siegel_yield(*t, &true_p))
            .collect();
