...
pub fn price(
    spot: f64, strike: f64,
    time_to_expiry: f64,
    rate_domestic: f64, rate_foreign: f64,
    dividend_yield: f64,
    sigma_asset: f64, sigma_fx: f64,
    ...
    if !spot.is_finite() || spot <= 0.0
        ...
        || !rate_domestic.is_finite() || !rate_foreign.is_finite()
        || !dividend_yield.is_finite()
        || !sigma_asset.is_finite() || sigma_asset <= 0.0
        || !sigma_fx.is_finite() || sigma_fx < 0.0
        || !correlation_asset_fx.is_finite() || !(-1.0..=1.0).contains(&correlation_asset_fx)
    {
        return None;
    }
    ...
    Some(QuantoReport { price: p.max(0.0), effective_drift: mu, d1, d2 })
}
...
mod tests {
    ...
    fn invalid_inputs_return_none() {
        for bad in [0.0, -1.0, f64::NAN] {
            assert!(price(bad, 100.0, 0.5, 0.03, 0.01, 0.0, 0.2, 0.1, 0.3, OptionKind::Call).is_none());
            assert!(price(100.0, bad, 0.5, 0.03, 0.01, 0.0, 0.2, 0.1, 0.3, OptionKind::Call).is_none());
            assert!(price(100.0, 100.0, bad, 0.03, 0.01, 0.0, 0.2, 0.1, 0.3, OptionKind::Call).is_none());
            assert!(price(100.0, 100.0, 0.5, 0.03, 0.01, 0.0, bad, 0.1, 0.3, OptionKind::Call).is_none());
        }
        // Correlation out of [-1, 1].
        assert!(price(100.0, 100.0, 0.5, 0.03, 0.01, 0.0, 0.2, 0.1, 1.5, OptionKind::Call).is_none());
        assert!(price(100.0, 100.0, 0.5, 0.03, 0.01, 0.0, 0.2, 0.1, -1.5, OptionKind::Call).is_none());
    }
    ...
    fn zero_correlation_collapses_to_drift_minus_rf() {
        ...
        let r = price(100.0, 100.0, 1.0, 0.03, 0.01, 0.0, 0.20, 0.10, 0.0,
    ...
    fn negative_correlation_raises_call_price() {
        ...
        let r_pos = price(common.0, common.1, common.2, common.3, common.4, common.5,
            common.6, common.7, 0.5, OptionKind::Call).unwrap();
        let r_neg = price(common.0, common.1, common.2, common.3, common.4, common.5,
            common.6, common.7, -0.5, OptionKind::Call).unwrap();
        assert!(r_neg.price > r_pos.price,
            "neg corr → bigger μ → bigger call: pos={} neg={}", r_pos.price, r_neg.price);
    }

    #[test]
    fn at_the_money_call_price_positive() {
        let r = price(100.0, 100.0, 0.5, 0.03, 0.01, 0.0, 0.20, 0.10, 0.3,
            OptionKind::Call).unwrap();
        assert!(r.price > 0.0);
    }
    ...
    fn deep_itm_call_approaches_intrinsic_discounted() {
        let r_d = 0.03;
        let t = 0.25;
        let r = price(200.0, 100.0, t, r_d, 0.01, 0.0, 0.20, 0.10, 0.0,
            OptionKind::Call).unwrap();
    ...
    fn quanto_put_call_parity_holds() {
        ...
        let s = 100.0; let k = 105.0; let t = 0.5;
        let r_d = 0.03; let r_f = 0.01; let q = 0.0;
        let s_a = 0.20; let s_fx = 0.10; let rho = 0.3;
    ...
    fn higher_vol_asset_inflates_atm_call() {
        let r_low  = price(100.0, 100.0, 0.5, 0.03, 0.01, 0.0, 0.10, 0.10, 0.3,
            OptionKind::Call).unwrap();
        let r_high = price(100.0, 100.0, 0.5, 0.03, 0.01, 0.0, 0.40, 0.10, 0.3,
            OptionKind::Call).unwrap();
