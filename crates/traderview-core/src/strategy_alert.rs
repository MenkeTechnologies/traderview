...
#[serde(tag = "kind", rename_all = "snake_case")]
pub enum Node {
    Leaf { symbol: String, metric: Metric, op: Op, value: f64 },
...
where F: FnMut(&str, &Metric) -> MetricInput,
{
    match node {
        Node::Leaf { symbol, metric, op, value } => {
        ...
        Node::And { left, right } => evaluate(left, resolver, trace) && evaluate(right, resolver, trace),
        Node::Or  { left, right } => evaluate(left, resolver, trace) || evaluate(right, resolver, trace),
...
mod tests {
    ...
    fn resolver_for(
        sym: &str, price: f64, cls: Vec<f64>,
    ) -> impl FnMut(&str, &Metric) -> MetricInput {
        let s = sym.to_string();
        move |q: &str, _m: &Metric| {
            if q == s { MetricInput { latest_price: Some(price), closes: cls.clone() } }
            else      { MetricInput { latest_price: None, closes: vec![] } }
        }
    }
    ...
    fn leaf_price_gt_threshold() {
        let node = Node::Leaf {
            symbol: "AAPL".into(),
            metric: Metric::Price, op: Op::Gt, value: 100.0,
        };
    ...
    fn and_short_circuits_on_left_false() {
        ...
        // 1 entry only.
        let left = Node::Leaf { symbol: "AAPL".into(), metric: Metric::Price, op: Op::Gt, value: 100.0 };
        let right = Node::Leaf { symbol: "AAPL".into(), metric: Metric::Price, op: Op::Lt, value: 200.0 };
        let node = Node::And { left: Box::new(left), right: Box::new(right) };
    ...
    fn collect_symbols_aggregates_max_history() {
        let n = Node::And {
            left:  Box::new(Node::Leaf { symbol: "AAPL".into(),
                metric: Metric::Sma { period: 20 }, op: Op::Gt, value: 0.0 }),
            right: Box::new(Node::Leaf { symbol: "AAPL".into(),
                metric: Metric::Sma { period: 200 }, op: Op::Gt, value: 0.0 }),
        };
