...
pub fn compute(bars: &[OhlcBar], max_n: usize) -> RwiReport {
    ...
    RwiReport { rwi_high: rwi_h, rwi_low: rwi_l, latest_high, latest_low, bias }
}
...
mod tests {
    ...
    fn strong_uptrend_gives_up_bias() {
        // Big steady uptrend.
        let bars: Vec<OhlcBar> = (0..20).map(|i| {
            let p = 100.0 + i as f64 * 2.0;
            b(p + 0.1, p - 0.1, p + 0.05)
        }).collect();
        let r = compute(&bars, 8);
        assert!(matches!(r.bias, RwiBias::Up),
            "got {:?} high={:?} low={:?}", r.bias, r.latest_high, r.latest_low);
    ...
    fn flat_oscillation_is_neutral() {
        ...
        assert!(matches!(r.bias, RwiBias::Neutral),
            "expected Neutral, got {:?}", r.bias);
    }
