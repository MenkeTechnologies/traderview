...
pub fn compute(bars: &[Bar]) -> Option<CorwinSchultzReport> {
    ...
    if bars.iter().any(|b| !b.high.is_finite() || !b.low.is_finite()
        || b.high <= 0.0 || b.low <= 0.0 || b.low > b.high) {
        return None;
    }
    ...
    for i in 0..(n - 1) {
        ...
        let h12 = h1.max(h2);
        let l12 = l1.min(l2);
        if l12 <= 0.0 { per_pair.push(0.0); continue; }
        ...
        if alpha < 0.0 { neg_alpha_count += 1; }
        let s = 2.0 * (alpha.exp() - 1.0) / (1.0 + alpha.exp());
        per_pair.push(s.max(0.0));
    }
    if per_pair.is_empty() { return None; }
...
mod tests {
    ...
    fn median_is_in_per_pair_range() {
        ...
        let r = compute(&bars).unwrap();
        let min = r.per_pair_spread.iter().cloned().fold(f64::INFINITY, f64::min);
        let max = r.per_pair_spread.iter().cloned().fold(f64::NEG_INFINITY, f64::max);
