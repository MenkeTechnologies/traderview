...
pub fn compute(returns: &[f64], alpha: f64) -> Option<CornishFisherReport> {
    ...
    if stdev <= flat_threshold { return None; }
...
fn monotone_check(skew: f64, kurt_excess: f64) -> bool {
    ...
    let derivative = |z: f64| 1.0 + z * skew / 3.0
        + (3.0 * z * z - 3.0) * kurt_excess / 24.0
        - (6.0 * z * z - 5.0) * skew * skew / 36.0;
    [-3.0_f64, -2.0, 2.0, 3.0].iter().all(|z| derivative(*z) > 0.0)
}
...
mod tests {
    ...
    fn gaussian_returns_yield_cf_approximately_equal_to_gaussian() {
        ...
        assert!(rel_err < 0.15, "Gaussian draws should give CF ≈ Gaussian VaR: cf={} gauss={}",
            rep.var_cornish_fisher, rep.var_gaussian);
    }
    ...
    fn negative_skew_shifts_z_cf_more_negative() {
        ...
        for _ in 0..400 {
            ...
            r.push(if u < 0.10 { -0.03 + 0.005 * z } else { 0.01 * z });
        }
        let rep = compute(&r, 0.05).unwrap();
        assert!(rep.skewness < -0.1, "expected negative skew, got {}", rep.skewness);
        assert!(rep.z_alpha_cf < rep.z_alpha,
