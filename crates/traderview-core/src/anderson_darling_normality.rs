...
pub fn test(sample: &[f64]) -> Option<AndersonDarlingReport> {
    ...
    let phi: Vec<f64> = sorted.iter().map(|x| {
        standard_normal_cdf((x - mean) / sd).clamp(1e-12, 1.0 - 1e-12)
    }).collect();
    let mut acc = 0.0_f64;
    for i in 0..n {
        let i_f = (i + 1) as f64;
...
mod tests {
    ...
    fn normal_sample_does_not_reject() {
        ...
        assert!(!r.reject_at_1pct,
            "Gaussian shouldn't reject at 1%, A²* = {}", r.a_squared_adjusted);
    }
    ...
    fn skewed_sample_rejects() {
        ...
        assert!(r.reject_at_5pct,
            "skewed sample should reject, A²* = {}", r.a_squared_adjusted);
    }
    ...
    fn heavy_mixture_rejects() {
        ...
        let r = test(&s).unwrap();
        assert!(r.reject_at_1pct,
            "heavy-tail mixture should reject at 1%, A²* = {}", r.a_squared_adjusted);
    }
