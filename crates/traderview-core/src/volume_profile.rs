...
pub fn build(prints: &[PrintAt], tick_size: f64) -> VolumeProfile {
    ...
    if prints.is_empty() || tick_size <= 0.0 { return report; }
    ...
    while covered < target {
        let next_up = if hi + 1 < levels.len() { Some(levels[hi + 1].volume) } else { None };
        let next_down = if lo > 0 { Some(levels[lo - 1].volume) } else { None };
        match (next_up, next_down) {
            (Some(u), Some(d)) => {
                if u >= d { hi += 1; covered += u; }
                else      { lo -= 1; covered += d; }
            }
            (Some(u), None) => { hi += 1; covered += u; }
            (None, Some(d)) => { lo -= 1; covered += d; }
            (None, None) => break,
        }
    }
    report.poc = poc;
...
mod tests {
    ...
    fn p(price: f64, vol: f64) -> PrintAt { PrintAt { price, volume: vol } }
    ...
    fn value_area_covers_at_least_70_pct() {
        ...
        let total = 1000.0;
        // Find VA volume.
        let in_va: f64 = r.levels.iter()
            .filter(|l| l.price >= r.val && l.price <= r.vah)
            .map(|l| l.volume)
            .sum();
        assert!(in_va >= total * 0.70, "VA should cover ≥70% — got {}/{}", in_va, total);
    }
