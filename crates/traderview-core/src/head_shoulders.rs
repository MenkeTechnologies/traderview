...
pub fn detect(bars: &[Bar], cfg: &Config) -> Vec<HsCandidate> {
    ...
    for trio in highs.windows(3) {
        ...
        if rs - ls > cfg.max_pattern_bars {
            continue;
        }
        let ls_h = bars[ls].high;
        let hd_h = bars[hd].high;
        let rs_h = bars[rs].high;
        if !ls_h.is_finite() || !hd_h.is_finite() || !rs_h.is_finite() {
            continue;
        }
        if hd_h <= ls_h || hd_h <= rs_h {
            continue;
        }
        let sym = (ls_h - rs_h).abs() / ls_h.max(rs_h);
        if sym > cfg.shoulder_symmetry_pct {
            continue;
        }
        ...
        let (Some(nl), Some(nr)) = (n_left, n_right) else {
            continue;
        };
    ...
    for trio in lows.windows(3) {
        ...
        if !ls_l.is_finite() || !hd_l.is_finite() || !rs_l.is_finite() {
            continue;
        }
        if hd_l >= ls_l || hd_l >= rs_l {
            continue;
        }
...
mod tests {
    ...
    fn classic_top_pattern_detected() {
        ...
        assert!(
            cands.iter().any(|c| c.kind == PatternKind::HeadShoulders),
    ...
    fn inverse_bottom_pattern_detected() {
        ...
        assert!(
            cands
                .iter()
                .any(|c| c.kind == PatternKind::InverseHeadShoulders),
    ...
    fn shoulder_asymmetry_rejects() {
        ...
        let cands: Vec<_> = detect(&bars, &cfg)
            .into_iter()
            .filter(|c| c.kind == PatternKind::HeadShoulders)
            .collect();
    ...
    fn projection_target_below_neckline_for_top() {
        ...
        let cand = detect(&bars, &cfg)
            .into_iter()
            .find(|c| c.kind == PatternKind::HeadShoulders)
            .expect("HS");
