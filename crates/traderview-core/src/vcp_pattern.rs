...
pub fn detect(bars: &[Bar], cfg: &Config) -> Vec<VcpCandidate> {
    let n = bars.len();
    let mut out = Vec::new();
    if cfg.pivot_lookback == 0 || cfg.min_contractions < 2
        || cfg.max_pattern_bars == 0 || cfg.max_final_pullback_pct <= 0.0
    ...
    if contractions.len() < cfg.min_contractions { return out; }
    ...
    for start in 0..=(contractions.len() - cfg.min_contractions) {
        ...
        if span > cfg.max_pattern_bars { continue; }
        let monotone = slice.windows(2).all(|w| w[1].pullback_pct < w[0].pullback_pct);
        if !monotone { continue; }
        let final_pct = slice.last().unwrap().pullback_pct;
        if final_pct > cfg.max_final_pullback_pct { continue; }
...
mod tests {
    ...
    fn classic_vcp_detected() {
        ...
        let cfg = Config { pivot_lookback: 2, min_contractions: 3, max_pattern_bars: 100,
            max_final_pullback_pct: 0.10 };
    ...
    fn expanding_pullbacks_not_classified() {
        ...
        let cands = detect(&bars, &cfg);
        assert!(cands.is_empty(), "expanding pullbacks should NOT classify as VCP");
    }
