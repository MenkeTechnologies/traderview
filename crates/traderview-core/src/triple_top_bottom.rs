...
impl Default for Config {
    fn default() -> Self {
        Self { pivot_lookback: 5, peak_tolerance_pct: 0.03, max_pattern_bars: 250 }
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum PatternKind { TripleTop, TripleBottom }
...
pub fn detect(bars: &[Bar], cfg: &Config) -> Vec<TripleCandidate> {
    ...
    for trio in window_3(&highs) {
        let (h1, h2, h3) = trio;
        if h3 - h1 > cfg.max_pattern_bars { continue; }
        let p1 = bars[h1].high;
        let p2 = bars[h2].high;
        let p3 = bars[h3].high;
        if !p1.is_finite() || !p2.is_finite() || !p3.is_finite() { continue; }
        let avg = (p1 + p2 + p3) / 3.0;
        let max_dev = ((p1 - avg).abs().max((p2 - avg).abs())).max((p3 - avg).abs());
        if max_dev / avg > cfg.peak_tolerance_pct { continue; }
    ...
    for trio in window_3(&lows) {
        let (l1, l2, l3) = trio;
        if l3 - l1 > cfg.max_pattern_bars { continue; }
        let p1 = bars[l1].low;
        let p2 = bars[l2].low;
        let p3 = bars[l3].low;
        if !p1.is_finite() || !p2.is_finite() || !p3.is_finite() { continue; }
        let avg = (p1 + p2 + p3) / 3.0;
        let max_dev = ((p1 - avg).abs().max((p2 - avg).abs())).max((p3 - avg).abs());
        if max_dev / avg.abs() > cfg.peak_tolerance_pct { continue; }
...
mod tests {
    ...
    fn classic_triple_top_detected() {
        ...
        let cfg = Config { pivot_lookback: 2, peak_tolerance_pct: 0.05, ..Default::default() };
        let cands = detect(&bars, &cfg);
        assert!(cands.iter().any(|c| c.kind == PatternKind::TripleTop),
    ...
    fn classic_triple_bottom_detected() {
        ...
        let cfg = Config { pivot_lookback: 2, peak_tolerance_pct: 0.05, ..Default::default() };
        let cands = detect(&bars, &cfg);
        assert!(cands.iter().any(|c| c.kind == PatternKind::TripleBottom),
    ...
    fn peaks_too_unequal_rejected() {
        ...
        let cfg = Config { pivot_lookback: 2, peak_tolerance_pct: 0.03, ..Default::default() };
        let cands: Vec<_> = detect(&bars, &cfg).into_iter()
            .filter(|c| c.kind == PatternKind::TripleTop).collect();
    ...
    fn projection_target_below_neckline_for_top() {
        ...
        let cand = detect(&bars, &cfg).into_iter()
            .find(|c| c.kind == PatternKind::TripleTop).expect("triple top");
