...
pub fn test(x: &[f64], y: &[f64], lags: usize) -> Option<GrangerReport> {
    ...
    let restricted_cols: Vec<Vec<f64>> = std::iter::once(intercept.clone())
        .chain(x_lags.clone())
        .collect();
    ...
    if !f_stat.is_finite() || f_stat.is_nan() {
        return None;
    }
    Some(GrangerReport {
        f_statistic: f_stat,
...
mod tests {
    ...
    fn y_causes_x_yields_large_f_stat() {
        ...
        assert!(
            r.f_statistic > 3.0,
            "expected large F (Y causes X), got {}",
            r.f_statistic
        );
    }
    ...
    fn independent_series_yield_modest_f_stat() {
        ...
        assert!(
            r.f_statistic < 10.0,
            "independent series shouldn't show large F, got {}",
            r.f_statistic
        );
    }
