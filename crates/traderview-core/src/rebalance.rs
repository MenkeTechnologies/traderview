...
pub fn compute(
    ...
    if sum_w > 1.0 + 1e-9 {
        warnings.push(format!("target weights sum to {:.4} (> 1.0) — over-allocated", sum_w));
    }
    ...
    for h in current { held.insert(h.symbol.as_str(), h); }

    // Index targets by symbol.
    let mut target_by_sym: std::collections::BTreeMap<&str, &TargetInput> = Default::default();
    for t in targets { target_by_sym.insert(t.symbol.as_str(), t); }

    // Union of all symbols (held ∪ targeted).
    let mut symbols: Vec<String> = Vec::new();
    for h in current { symbols.push(h.symbol.clone()); }
    for t in targets { if !symbols.iter().any(|s| s == &t.symbol) { symbols.push(t.symbol.clone()); } }
    symbols.sort();
    ...
    for sym in &symbols {
        ...
        let price = tgt.and_then(|t| t.price).or_else(|| cur.map(|c| c.price)).unwrap_or(0.0);
        let cur_qty = cur.map(|c| c.qty).unwrap_or(0.0);
        let cur_value = cur_qty * price;
        let cur_pct   = if total_value > 0.0 { cur_value / total_value } else { 0.0 };
        let tgt_pct   = tgt.map(|t| t.weight).unwrap_or(0.0);
        let tgt_value = total_value * tgt_pct;
        let tgt_qty   = if price > 0.0 { (tgt_value / price).floor() as i64 } else { 0 };
        let trade_qty = tgt_qty - cur_qty as i64;
        let side = if trade_qty > 0 { "buy" } else if trade_qty < 0 { "sell" } else { "hold" };
    ...
    let mut trade_pool: Vec<PlanRow> = rows.iter()
        .filter(|r| r.trade_qty != 0)
        .cloned()
        .collect();
    trade_pool.sort_by(|a, b| b.trade_value.abs()
        .partial_cmp(&a.trade_value.abs()).unwrap_or(std::cmp::Ordering::Equal));
...
mod tests {
    ...
    fn no_drift_yields_no_trades() {
        ...
        let cur = vec![
            HoldingInput { symbol: "SPY".into(), qty: 10.0, price: 100.0 },
            HoldingInput { symbol: "QQQ".into(), qty: 10.0, price: 100.0 },
        ];
        let tgt = vec![
            TargetInput { symbol: "SPY".into(), weight: 0.5, price: None },
            TargetInput { symbol: "QQQ".into(), weight: 0.5, price: None },
        ];
    ...
    fn buy_and_sell_to_meet_targets() {
        ...
        let cur = vec![HoldingInput { symbol: "SPY".into(), qty: 100.0, price: 100.0 }];
        let tgt = vec![
            TargetInput { symbol: "SPY".into(), weight: 0.5, price: None },
            TargetInput { symbol: "QQQ".into(), weight: 0.5, price: Some(200.0) },
        ];
        let p = compute(&cur, &tgt, 0.0, 10);
    ...
    fn max_trades_caps_to_biggest_dollar_moves() {
        ...
        let cur = vec![
            HoldingInput { symbol: "A".into(), qty: 100.0, price: 10.0 },  // $1000
            HoldingInput { symbol: "B".into(), qty: 100.0, price: 50.0 },  // $5000
            HoldingInput { symbol: "C".into(), qty: 100.0, price: 100.0 }, // $10000 ← biggest sell
            HoldingInput { symbol: "D".into(), qty: 100.0, price: 20.0 },  // $2000
            HoldingInput { symbol: "E".into(), qty: 100.0, price: 80.0 },  // $8000 ← 2nd biggest sell
        ];
    ...
    fn over_allocated_warning() {
        let cur = vec![];
        let tgt = vec![
            TargetInput { symbol: "A".into(), weight: 0.6, price: Some(10.0) },
            TargetInput { symbol: "B".into(), weight: 0.6, price: Some(10.0) },
        ];
