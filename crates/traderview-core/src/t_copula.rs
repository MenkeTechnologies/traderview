...
pub fn fit(observations: &[Vec<f64>], degrees_of_freedom: f64) -> Option<TCopulaReport> {
    ...
    let t_means: Vec<f64> = (0..p)
        .map(|j| t_scores.iter().map(|row| row[j]).sum::<f64>() / n_f)
        .collect();
    let t_stds: Vec<f64> = (0..p)
        .map(|j| {
            let var: f64 = t_scores
                .iter()
                .map(|row| (row[j] - t_means[j]).powi(2))
                .sum::<f64>()
                / (n_f - 1.0);
            var.max(1e-18).sqrt()
        })
...
fn student_t_pdf(x: f64, dof: f64) -> f64 {
    let coef =
        (lgamma((dof + 1.0) / 2.0) - lgamma(dof / 2.0) - 0.5 * (dof * std::f64::consts::PI).ln())
            .exp();
