...
pub fn analyze(
    ...
    if !bond_clean_price.is_finite()
        || bond_clean_price <= 0.0
        || !par_value.is_finite()
        || par_value <= 0.0
        || !fixed_coupon_rate.is_finite()
        || fixed_coupon_rate < 0.0
        || !par_swap_rate.is_finite()
        || par_swap_rate < 0.0
        || cash_flows.is_empty()
    {
        return None;
    }
    if cash_flows.iter().any(|cf| {
        !cf.time_years.is_finite()
            || cf.time_years <= 0.0
            || !cf.accrual.is_finite()
            || cf.accrual <= 0.0
            || !cf.discount_factor.is_finite()
            || cf.discount_factor <= 0.0
            || cf.discount_factor > 1.0 + 1e-9
    }) {
        return None;
    }
    let annuity: f64 = cash_flows
        .iter()
        .map(|cf| cf.accrual * cf.discount_factor)
        .sum();
    if annuity <= 0.0 {
        return None;
    }
    ...
    let asw = par_swap_rate - fixed_coupon_rate
        + (par_value - bond_clean_price) / (annuity_per_unit * par_value);
    let asw_bps = asw * 10_000.0;
    if !asw_bps.is_finite() {
        return None;
    }
