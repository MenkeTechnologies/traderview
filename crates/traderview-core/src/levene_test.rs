...
pub fn test(groups: &[Vec<f64>]) -> Option<LeveneReport> {
    let k = groups.len();
    if k < 2 { return None; }
    if groups.iter().any(|g| g.len() < 3 || g.iter().any(|x| !x.is_finite())) {
        return None;
    }
    ...
    let z_groups: Vec<Vec<f64>> = groups.iter().zip(medians.iter())
        .map(|(g, m)| g.iter().map(|x| (x - m).abs()).collect()).collect();
    let z_group_means: Vec<f64> = z_groups.iter().map(|z| {
        z.iter().sum::<f64>() / z.len() as f64
    }).collect();
    let z_overall_mean: f64 = z_groups.iter().flatten().sum::<f64>() / n_total as f64;
    let numerator: f64 = z_groups.iter().zip(z_group_means.iter()).map(|(z, m)| {
        z.len() as f64 * (m - z_overall_mean).powi(2)
    }).sum();
    let denominator: f64 = z_groups.iter().zip(z_group_means.iter()).map(|(z, m)| {
        z.iter().map(|zi| (zi - m).powi(2)).sum::<f64>()
    }).sum();
...
fn f_5pct_critical(d1: f64, d2: f64) -> f64 {
    ...
    if d2 > 60.0 {
        ...
        match d1 as usize {
            1 => 3.84, 2 => 3.00, 3 => 2.60, 4 => 2.37, 5 => 2.21,
            _ => 2.10,
        }
    } else if d1 == 1.0 {
        4.20
...
mod tests {
    ...
    fn equal_variance_does_not_reject() {
        ...
        assert!(!r.reject_at_5pct,
            "equal-variance groups shouldn't reject, W = {}", r.w_statistic);
    }
    ...
    fn different_variance_rejects() {
        ...
        let r = test(&[g1, g2]).unwrap();
        assert!(r.reject_at_5pct,
            "5x variance difference should reject, W = {}", r.w_statistic);
    }
