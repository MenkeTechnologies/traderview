...
pub fn matches(hit: &ScanHit, preset: Preset) -> bool {
    match preset {
        ...
        Preset::GapUpWideRangeDeepBelowYearHighHotVol => {
            hit.gap_pct > 2.0
                && hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 8.0
                && hit.rel_volume >= 1.5
                && hit.year_high_pct >= 20.0
        }
        Preset::GapDownWideRangeDeepAboveYearLowHotVol => {
            hit.gap_pct < -2.0
                && hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 8.0
                && hit.rel_volume >= 1.5
                && hit.year_low_pct >= 20.0
        }
        Preset::GapUpWideRangeMidYearHighHotVol => {
            hit.gap_pct > 2.0
                && hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 8.0
                && hit.rel_volume >= 1.5
                && hit.year_high_pct >= 5.0
        ...
        Preset::GapUpWideRangeJustOffYearHighHotVol => {
            hit.gap_pct > 2.0
                && hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 8.0
                && hit.rel_volume >= 1.5
                && hit.year_high_pct >= 2.0
        ...
        Preset::GapDownWideRangeJustOffYearLowHotVol => {
            hit.gap_pct < -2.0
                && hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 8.0
                && hit.rel_volume >= 1.5
                && hit.year_low_pct >= 2.0
        ...
        Preset::GapUpTightRangeConfirmedAboveYearHighHotVol => {
            hit.gap_pct > 2.0
                && hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() < 1.0
                && hit.rel_volume >= 1.5
                && hit.year_high_pct >= -3.0
        ...
        Preset::GapDownTightRangeConfirmedBelowYearLowHotVol => {
            hit.gap_pct < -2.0
                && hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() < 1.0
                && hit.rel_volume >= 1.5
                && hit.year_low_pct >= -3.0
        ...
        Preset::GapUpTightRangeMidYearHighHotVol => {
            hit.gap_pct > 2.0
                && hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() < 1.0
                && hit.rel_volume >= 1.5
                && hit.year_high_pct >= 5.0
        ...
        Preset::GapDownTightRangeMidYearLowHotVol => {
            hit.gap_pct < -2.0
                && hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() < 1.0
                && hit.rel_volume >= 1.5
                && hit.year_low_pct >= 5.0
        ...
        Preset::GapUpTightRangeJustOffYearHighHotVol => {
            hit.gap_pct > 2.0
                && hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() < 1.0
                && hit.rel_volume >= 1.5
                && hit.year_high_pct >= 2.0
        ...
        Preset::DecupledVolUpJustOffYearHighHotVol => {
            hit.rel_volume >= 10.0
                && hit.change_pct > 3.0
                && hit.year_high_pct >= 2.0
                && hit.year_high_pct < 5.0
        }
        Preset::DecupledVolDownJustOffYearLowHotVol => {
            hit.rel_volume >= 10.0
                && hit.change_pct < -3.0
                && hit.year_low_pct >= 2.0
                && hit.year_low_pct < 5.0
        }
        Preset::DecupledVolCloseAtHodHotVol => {
            hit.rel_volume >= 10.0 && hit.hod_dist_pct.abs() < 0.5 && hit.change_pct > 1.0
        }
        Preset::DecupledVolCloseAtLodHotVol => {
            hit.rel_volume >= 10.0 && hit.lod_dist_pct.abs() < 0.5 && hit.change_pct < -1.0
        }
        Preset::DecupledVolGapUpCloseAtHodHotVol => {
            hit.rel_volume >= 10.0
                && hit.gap_pct > 2.0
                && hit.hod_dist_pct.abs() < 0.5
                && hit.change_pct > 1.0
        ...
        Preset::DecupledVolGapUpMidpointHotVol => {
            hit.rel_volume >= 10.0
                && hit.gap_pct > 2.0
                && hit.hod_dist_pct.abs() > 0.5
                && hit.lod_dist_pct.abs() > 0.5
        ...
        Preset::QuadConfluenceHighHotVol => {
            hit.hod_dist_pct.abs() < 0.5
                && hit.year_high_pct < 0.5
                && hit.change_pct > 1.0
                && hit.rel_volume >= 2.0
        ...
        Preset::QuadConfluenceLowHotVol => {
            hit.lod_dist_pct.abs() < 0.5
                && hit.year_low_pct < 0.5
                && hit.change_pct < -1.0
                && hit.rel_volume >= 2.0
        ...
        Preset::HexaConfluenceHighHotVol => {
            hit.hod_dist_pct.abs() < 0.5
                && hit.year_high_pct < 0.5
                && hit.change_pct > 1.0
                && hit.rel_volume >= 5.0
        ...
        Preset::HexaConfluenceLowHotVol => {
            hit.lod_dist_pct.abs() < 0.5
                && hit.year_low_pct < 0.5
                && hit.change_pct < -1.0
                && hit.rel_volume >= 5.0
        ...
        Preset::BreakoutPocketHighHotVol => {
            hit.hod_dist_pct.abs() < 0.5
                && hit.year_high_pct < 0.5
                && hit.change_pct > 1.0
                && hit.rel_volume >= 2.0
        ...
        Preset::BreakdownPocketLowHotVol => {
            hit.lod_dist_pct.abs() < 0.5
                && hit.year_low_pct < 0.5
                && hit.change_pct < -1.0
                && hit.rel_volume >= 2.0
        ...
        Preset::CloseInUpperThirdNearYearHighHotVol => {
            hit.hod_dist_pct.abs() * 2.0 < hit.lod_dist_pct.abs()
                && hit.hod_dist_pct.abs() > 0.3
                && hit.year_high_pct < 2.0
                && hit.rel_volume >= 1.5
        ...
        Preset::CloseInUpperThirdDeepBelowYearHighHotVol => {
            hit.hod_dist_pct.abs() * 2.0 < hit.lod_dist_pct.abs()
                && hit.hod_dist_pct.abs() > 0.3
                && hit.year_high_pct >= 20.0
                && hit.rel_volume >= 1.5
        }
        Preset::CloseInLowerThirdDeepAboveYearLowHotVol => {
            hit.lod_dist_pct.abs() * 2.0 < hit.hod_dist_pct.abs()
                && hit.lod_dist_pct.abs() > 0.3
                && hit.year_low_pct >= 20.0
                && hit.rel_volume >= 1.5
        ...
        Preset::CloseInLowerThirdMidYearLowHotVol => {
            hit.lod_dist_pct.abs() * 2.0 < hit.hod_dist_pct.abs()
                && hit.lod_dist_pct.abs() > 0.3
                && hit.year_low_pct >= 5.0
                && hit.year_low_pct < 20.0
        ...
        Preset::BullRecoveryWickHotVol => {
            hit.lod_dist_pct.abs() > 3.0
                && hit.change_pct > 0.0
                && hit.hod_dist_pct.abs() > 1.0
                && hit.rel_volume >= 1.5
        }
        Preset::BearRejectionWickHotVol => {
            hit.hod_dist_pct.abs() > 3.0
                && hit.change_pct < 0.0
                && hit.lod_dist_pct.abs() > 1.0
                && hit.rel_volume >= 1.5
        ...
        Preset::BullRecoveryWickDeepBelowYearHighHotVol => {
            hit.lod_dist_pct.abs() > 3.0
                && hit.change_pct > 0.0
                && hit.hod_dist_pct.abs() > 1.0
                && hit.year_high_pct >= 20.0
        ...
        Preset::BearRejectionWickDeepAboveYearLowHotVol => {
            hit.hod_dist_pct.abs() > 3.0
                && hit.change_pct < 0.0
                && hit.lod_dist_pct.abs() > 1.0
                && hit.year_low_pct >= 20.0
        ...
        Preset::BullRecoveryWickMidYearHighHotVol => {
            hit.lod_dist_pct.abs() > 3.0
                && hit.change_pct > 0.0
                && hit.hod_dist_pct.abs() > 1.0
                && hit.year_high_pct >= 5.0
        ...
        Preset::BearRejectionWickMidYearLowHotVol => {
            hit.hod_dist_pct.abs() > 3.0
                && hit.change_pct < 0.0
                && hit.lod_dist_pct.abs() > 1.0
                && hit.year_low_pct >= 5.0
        ...
        Preset::BullRecoveryWickJustOffYearLowHotVol => {
            hit.lod_dist_pct.abs() > 3.0
                && hit.change_pct > 0.0
                && hit.hod_dist_pct.abs() > 1.0
                && hit.year_low_pct >= 2.0
        ...
        Preset::BearRejectionWickJustOffYearHighHotVol => {
            hit.hod_dist_pct.abs() > 3.0
                && hit.change_pct < 0.0
                && hit.lod_dist_pct.abs() > 1.0
                && hit.year_high_pct >= 2.0
        ...
        Preset::MegaRangeUpDeepBelowYearHighHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct > 0.0
                && hit.rel_volume >= 2.0
                && hit.year_high_pct >= 20.0
        }
        Preset::MegaRangeDownDeepAboveYearLowHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct < 0.0
                && hit.rel_volume >= 2.0
                && hit.year_low_pct >= 20.0
        ...
        Preset::MegaRangeUpMidYearHighHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct > 0.0
                && hit.rel_volume >= 2.0
                && hit.year_high_pct >= 5.0
        ...
        Preset::MegaRangeDownMidYearLowHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct < 0.0
                && hit.rel_volume >= 2.0
                && hit.year_low_pct >= 5.0
        ...
        Preset::MegaRangeFlatDayGapUpHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.day_pct.abs() < 0.5
                && hit.gap_pct > 0.0
                && hit.rel_volume >= 2.0
        }
        Preset::MegaRangeFlatDayGapDownHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.day_pct.abs() < 0.5
                && hit.gap_pct < 0.0
                && hit.rel_volume >= 2.0
        ...
        Preset::MegaRangeMidpointCloseNearYearLowHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.hod_dist_pct.abs() > 0.5
                && hit.lod_dist_pct.abs() > 0.5
                && (hit.hod_dist_pct.abs() - hit.lod_dist_pct.abs()).abs() < 0.5
        ...
        Preset::MegaRangeMidpointCloseJustOffYearHighHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.hod_dist_pct.abs() > 0.5
                && hit.lod_dist_pct.abs() > 0.5
                && (hit.hod_dist_pct.abs() - hit.lod_dist_pct.abs()).abs() < 0.5
        ...
        Preset::MegaRangeMidpointCloseJustOffYearLowHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.hod_dist_pct.abs() > 0.5
                && hit.lod_dist_pct.abs() > 0.5
                && (hit.hod_dist_pct.abs() - hit.lod_dist_pct.abs()).abs() < 0.5
        ...
        Preset::MegaRangeMidpointCloseAtYearHighDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.hod_dist_pct.abs() > 0.5
                && hit.lod_dist_pct.abs() > 0.5
                && (hit.hod_dist_pct.abs() - hit.lod_dist_pct.abs()).abs() < 0.5
        ...
        Preset::MegaRangeMidpointCloseConfirmedAboveYearHighDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.hod_dist_pct.abs() > 0.5
                && hit.lod_dist_pct.abs() > 0.5
                && (hit.hod_dist_pct.abs() - hit.lod_dist_pct.abs()).abs() < 0.5
        ...
        Preset::MegaRangeMidpointCloseConfirmedBelowYearLowDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.hod_dist_pct.abs() > 0.5
                && hit.lod_dist_pct.abs() > 0.5
                && (hit.hod_dist_pct.abs() - hit.lod_dist_pct.abs()).abs() < 0.5
        ...
        Preset::MegaRangeMidpointCloseMidYearHighDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.hod_dist_pct.abs() > 0.5
                && hit.lod_dist_pct.abs() > 0.5
                && (hit.hod_dist_pct.abs() - hit.lod_dist_pct.abs()).abs() < 0.5
        ...
        Preset::MegaRangeMidpointCloseMidYearLowDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.hod_dist_pct.abs() > 0.5
                && hit.lod_dist_pct.abs() > 0.5
                && (hit.hod_dist_pct.abs() - hit.lod_dist_pct.abs()).abs() < 0.5
        ...
        Preset::MegaRangeMidpointCloseJustOffYearHighDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.hod_dist_pct.abs() > 0.5
                && hit.lod_dist_pct.abs() > 0.5
                && (hit.hod_dist_pct.abs() - hit.lod_dist_pct.abs()).abs() < 0.5
        ...
        Preset::MegaRangeMidpointCloseJustOffYearLowDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.hod_dist_pct.abs() > 0.5
                && hit.lod_dist_pct.abs() > 0.5
                && (hit.hod_dist_pct.abs() - hit.lod_dist_pct.abs()).abs() < 0.5
        ...
        Preset::MegaRangeUpNearYearHighDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct > 0.0
                && hit.rel_volume >= 10.0
                && hit.year_high_pct < 2.0
        }
        Preset::MegaRangeDownNearYearLowDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct < 0.0
                && hit.rel_volume >= 10.0
                && hit.year_low_pct < 2.0
        ...
        Preset::MegaRangeUpDeepBelowYearHighDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct > 0.0
                && hit.rel_volume >= 10.0
                && hit.year_high_pct >= 20.0
        }
        Preset::MegaRangeDownDeepAboveYearLowDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct < 0.0
                && hit.rel_volume >= 10.0
                && hit.year_low_pct >= 20.0
        ...
        Preset::MegaRangeUpJustOffYearHighDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct > 0.0
                && hit.rel_volume >= 10.0
                && hit.year_high_pct >= 2.0
        ...
        Preset::MegaRangeDownJustOffYearLowDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct < 0.0
                && hit.rel_volume >= 10.0
                && hit.year_low_pct >= 2.0
        ...
        Preset::MegaRangeUpperThirdNearYearHighDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.hod_dist_pct.abs() * 2.0 < hit.lod_dist_pct.abs()
                && hit.hod_dist_pct.abs() > 0.3
                && hit.rel_volume >= 10.0
        ...
        Preset::MegaRangeUpperThirdNearYearLowDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.hod_dist_pct.abs() * 2.0 < hit.lod_dist_pct.abs()
                && hit.hod_dist_pct.abs() > 0.3
                && hit.rel_volume >= 10.0
        ...
        Preset::MegaRangeUpperThirdConfirmedAboveYearHighDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.hod_dist_pct.abs() * 2.0 < hit.lod_dist_pct.abs()
                && hit.hod_dist_pct.abs() > 0.3
                && hit.rel_volume >= 10.0
        ...
        Preset::MegaRangeUpperThirdConfirmedBelowYearLowDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.hod_dist_pct.abs() * 2.0 < hit.lod_dist_pct.abs()
                && hit.hod_dist_pct.abs() > 0.3
                && hit.rel_volume >= 10.0
        ...
        Preset::MegaRangeUpperThirdDeepBelowYearHighDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.hod_dist_pct.abs() * 2.0 < hit.lod_dist_pct.abs()
                && hit.hod_dist_pct.abs() > 0.3
                && hit.rel_volume >= 10.0
        ...
        Preset::MegaRangeUpperThirdDeepAboveYearLowDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.hod_dist_pct.abs() * 2.0 < hit.lod_dist_pct.abs()
                && hit.hod_dist_pct.abs() > 0.3
                && hit.rel_volume >= 10.0
        ...
        Preset::MegaRangeUpperThirdJustOffYearHighDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.hod_dist_pct.abs() * 2.0 < hit.lod_dist_pct.abs()
                && hit.hod_dist_pct.abs() > 0.3
                && hit.rel_volume >= 10.0
        ...
        Preset::MegaRangeUpperThirdJustOffYearLowDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.hod_dist_pct.abs() * 2.0 < hit.lod_dist_pct.abs()
                && hit.hod_dist_pct.abs() > 0.3
                && hit.rel_volume >= 10.0
        ...
        Preset::MegaRangeLowerThirdNearYearHighDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.lod_dist_pct.abs() * 2.0 < hit.hod_dist_pct.abs()
                && hit.lod_dist_pct.abs() > 0.3
                && hit.rel_volume >= 10.0
        ...
        Preset::MegaRangeLowerThirdNearYearLowDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.lod_dist_pct.abs() * 2.0 < hit.hod_dist_pct.abs()
                && hit.lod_dist_pct.abs() > 0.3
                && hit.rel_volume >= 10.0
        ...
        Preset::MegaRangeLowerThirdConfirmedAboveYearHighDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.lod_dist_pct.abs() * 2.0 < hit.hod_dist_pct.abs()
                && hit.lod_dist_pct.abs() > 0.3
                && hit.rel_volume >= 10.0
        ...
        Preset::MegaRangeLowerThirdConfirmedBelowYearLowDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.lod_dist_pct.abs() * 2.0 < hit.hod_dist_pct.abs()
                && hit.lod_dist_pct.abs() > 0.3
                && hit.rel_volume >= 10.0
        ...
        Preset::MegaRangeLowerThirdDeepBelowYearHighDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.lod_dist_pct.abs() * 2.0 < hit.hod_dist_pct.abs()
                && hit.lod_dist_pct.abs() > 0.3
                && hit.rel_volume >= 10.0
        ...
        Preset::MegaRangeLowerThirdDeepAboveYearLowDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.lod_dist_pct.abs() * 2.0 < hit.hod_dist_pct.abs()
                && hit.lod_dist_pct.abs() > 0.3
                && hit.rel_volume >= 10.0
        ...
        Preset::MegaRangeLowerThirdMidYearHighDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.lod_dist_pct.abs() * 2.0 < hit.hod_dist_pct.abs()
                && hit.lod_dist_pct.abs() > 0.3
                && hit.rel_volume >= 10.0
        ...
        Preset::MegaRangeLowerThirdMidYearLowDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.lod_dist_pct.abs() * 2.0 < hit.hod_dist_pct.abs()
                && hit.lod_dist_pct.abs() > 0.3
                && hit.rel_volume >= 10.0
        ...
        Preset::MegaRangeLowerThirdJustOffYearHighDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.lod_dist_pct.abs() * 2.0 < hit.hod_dist_pct.abs()
                && hit.lod_dist_pct.abs() > 0.3
                && hit.rel_volume >= 10.0
        ...
        Preset::MegaRangeLowerThirdJustOffYearLowDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.lod_dist_pct.abs() * 2.0 < hit.hod_dist_pct.abs()
                && hit.lod_dist_pct.abs() > 0.3
                && hit.rel_volume >= 10.0
        ...
        Preset::MegaRangeTrendDayUpNearYearHighDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct > 0.0
                && hit.hod_dist_pct.abs() < 0.5
                && hit.rel_volume >= 10.0
        ...
        Preset::MegaRangeTrendDayUpNearYearLowDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct > 0.0
                && hit.hod_dist_pct.abs() < 0.5
                && hit.rel_volume >= 10.0
        ...
        Preset::MegaRangeTrendDayUpConfirmedAboveYearHighDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct > 0.0
                && hit.hod_dist_pct.abs() < 0.5
                && hit.rel_volume >= 10.0
        ...
        Preset::MegaRangeTrendDayUpConfirmedBelowYearLowDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct > 0.0
                && hit.hod_dist_pct.abs() < 0.5
                && hit.rel_volume >= 10.0
        ...
        Preset::MegaRangeTrendDayUpDeepBelowYearHighDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct > 0.0
                && hit.hod_dist_pct.abs() < 0.5
                && hit.rel_volume >= 10.0
        ...
        Preset::MegaRangeTrendDayUpMidYearLowDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct > 0.0
                && hit.hod_dist_pct.abs() < 0.5
                && hit.rel_volume >= 10.0
        ...
        Preset::MegaRangeTrendDayDownNearYearHighDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct < 0.0
                && hit.lod_dist_pct.abs() < 0.5
                && hit.rel_volume >= 10.0
        ...
        Preset::MegaRangeTrendDayDownNearYearLowDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct < 0.0
                && hit.lod_dist_pct.abs() < 0.5
                && hit.rel_volume >= 10.0
        ...
        Preset::MegaRangeTrendDayDownConfirmedAboveYearHighDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct < 0.0
                && hit.lod_dist_pct.abs() < 0.5
                && hit.rel_volume >= 10.0
        ...
        Preset::MegaRangeTrendDayDownConfirmedBelowYearLowDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct < 0.0
                && hit.lod_dist_pct.abs() < 0.5
                && hit.rel_volume >= 10.0
        ...
        Preset::MegaRangeTrendDayDownDeepBelowYearHighDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct < 0.0
                && hit.lod_dist_pct.abs() < 0.5
                && hit.rel_volume >= 10.0
        ...
        Preset::MegaRangeTrendDayDownDeepAboveYearLowDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct < 0.0
                && hit.lod_dist_pct.abs() < 0.5
                && hit.rel_volume >= 10.0
        ...
        Preset::MegaRangeTrendDayDownMidYearHighDecupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct < 0.0
                && hit.lod_dist_pct.abs() < 0.5
                && hit.rel_volume >= 10.0
        ...
        Preset::MegaRangeTrendDayUpConfirmedAboveYearHighHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct > 0.0
                && hit.hod_dist_pct.abs() < 0.5
                && hit.rel_volume >= 2.0
        ...
        Preset::MegaRangeTrendDayUpConfirmedBelowYearLowHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct > 0.0
                && hit.hod_dist_pct.abs() < 0.5
                && hit.rel_volume >= 2.0
        ...
        Preset::MegaRangeTrendDayUpDeepBelowYearHighHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct > 0.0
                && hit.hod_dist_pct.abs() < 0.5
                && hit.rel_volume >= 2.0
        ...
        Preset::MegaRangeTrendDayUpDeepAboveYearLowHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct > 0.0
                && hit.hod_dist_pct.abs() < 0.5
                && hit.rel_volume >= 2.0
        ...
        Preset::MegaRangeTrendDayUpMidYearHighHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct > 0.0
                && hit.hod_dist_pct.abs() < 0.5
                && hit.rel_volume >= 2.0
        ...
        Preset::MegaRangeTrendDayUpMidYearLowHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct > 0.0
                && hit.hod_dist_pct.abs() < 0.5
                && hit.rel_volume >= 2.0
        ...
        Preset::MegaRangeTrendDayUpJustOffYearHighHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct > 0.0
                && hit.hod_dist_pct.abs() < 0.5
                && hit.rel_volume >= 2.0
        ...
        Preset::MegaRangeTrendDayUpJustOffYearLowHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct > 0.0
                && hit.hod_dist_pct.abs() < 0.5
                && hit.rel_volume >= 2.0
        ...
        Preset::MegaRangeTrendDayDownNearYearHighHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct < 0.0
                && hit.lod_dist_pct.abs() < 0.5
                && hit.rel_volume >= 2.0
        ...
        Preset::MegaRangeTrendDayDownNearYearLowHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct < 0.0
                && hit.lod_dist_pct.abs() < 0.5
                && hit.rel_volume >= 2.0
        ...
        Preset::MegaRangeTrendDayDownConfirmedAboveYearHighHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct < 0.0
                && hit.lod_dist_pct.abs() < 0.5
                && hit.rel_volume >= 2.0
        ...
        Preset::MegaRangeTrendDayDownConfirmedBelowYearLowHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct < 0.0
                && hit.lod_dist_pct.abs() < 0.5
                && hit.rel_volume >= 2.0
        ...
        Preset::MegaRangeTrendDayDownDeepBelowYearHighHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct < 0.0
                && hit.lod_dist_pct.abs() < 0.5
                && hit.rel_volume >= 2.0
        ...
        Preset::MegaRangeTrendDayDownDeepAboveYearLowHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct < 0.0
                && hit.lod_dist_pct.abs() < 0.5
                && hit.rel_volume >= 2.0
        ...
        Preset::MegaRangeTrendDayDownMidYearHighHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct < 0.0
                && hit.lod_dist_pct.abs() < 0.5
                && hit.rel_volume >= 2.0
        ...
        Preset::MegaRangeTrendDayDownMidYearLowHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct < 0.0
                && hit.lod_dist_pct.abs() < 0.5
                && hit.rel_volume >= 2.0
        ...
        Preset::MegaRangeTrendDayDownJustOffYearHighHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct < 0.0
                && hit.lod_dist_pct.abs() < 0.5
                && hit.rel_volume >= 2.0
        ...
        Preset::MegaRangeTrendDayDownJustOffYearLowHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct < 0.0
                && hit.lod_dist_pct.abs() < 0.5
                && hit.rel_volume >= 2.0
        ...
        Preset::MegaRangeUpperThirdNearYearHighHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.hod_dist_pct.abs() * 2.0 < hit.lod_dist_pct.abs()
                && hit.hod_dist_pct.abs() > 0.3
                && hit.rel_volume >= 2.0
        ...
        Preset::MegaRangeUpperThirdNearYearLowHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.hod_dist_pct.abs() * 2.0 < hit.lod_dist_pct.abs()
                && hit.hod_dist_pct.abs() > 0.3
                && hit.rel_volume >= 2.0
        ...
        Preset::MegaRangeUpperThirdConfirmedAboveYearHighHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.hod_dist_pct.abs() * 2.0 < hit.lod_dist_pct.abs()
                && hit.hod_dist_pct.abs() > 0.3
                && hit.rel_volume >= 2.0
        ...
        Preset::MegaRangeUpperThirdDeepBelowYearHighHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.hod_dist_pct.abs() * 2.0 < hit.lod_dist_pct.abs()
                && hit.hod_dist_pct.abs() > 0.3
                && hit.rel_volume >= 2.0
        ...
        Preset::MegaRangeUpperThirdDeepAboveYearLowHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.hod_dist_pct.abs() * 2.0 < hit.lod_dist_pct.abs()
                && hit.hod_dist_pct.abs() > 0.3
                && hit.rel_volume >= 2.0
        ...
        Preset::MegaRangeUpperThirdJustOffYearHighHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.hod_dist_pct.abs() * 2.0 < hit.lod_dist_pct.abs()
                && hit.hod_dist_pct.abs() > 0.3
                && hit.rel_volume >= 2.0
        ...
        Preset::MegaRangeUpperThirdJustOffYearLowHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.hod_dist_pct.abs() * 2.0 < hit.lod_dist_pct.abs()
                && hit.hod_dist_pct.abs() > 0.3
                && hit.rel_volume >= 2.0
        ...
        Preset::MegaRangeLowerThirdNearYearHighHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.lod_dist_pct.abs() * 2.0 < hit.hod_dist_pct.abs()
                && hit.lod_dist_pct.abs() > 0.3
                && hit.rel_volume >= 2.0
        ...
        Preset::MegaRangeLowerThirdNearYearLowHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.lod_dist_pct.abs() * 2.0 < hit.hod_dist_pct.abs()
                && hit.lod_dist_pct.abs() > 0.3
                && hit.rel_volume >= 2.0
        ...
        Preset::MegaRangeLowerThirdConfirmedAboveYearHighHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.lod_dist_pct.abs() * 2.0 < hit.hod_dist_pct.abs()
                && hit.lod_dist_pct.abs() > 0.3
                && hit.rel_volume >= 2.0
        ...
        Preset::MegaRangeLowerThirdConfirmedBelowYearLowHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.lod_dist_pct.abs() * 2.0 < hit.hod_dist_pct.abs()
                && hit.lod_dist_pct.abs() > 0.3
                && hit.rel_volume >= 2.0
        ...
        Preset::MegaRangeLowerThirdDeepBelowYearHighHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.lod_dist_pct.abs() * 2.0 < hit.hod_dist_pct.abs()
                && hit.lod_dist_pct.abs() > 0.3
                && hit.rel_volume >= 2.0
        ...
        Preset::MegaRangeLowerThirdDeepAboveYearLowHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.lod_dist_pct.abs() * 2.0 < hit.hod_dist_pct.abs()
                && hit.lod_dist_pct.abs() > 0.3
                && hit.rel_volume >= 2.0
        ...
        Preset::MegaRangeLowerThirdMidYearLowHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.lod_dist_pct.abs() * 2.0 < hit.hod_dist_pct.abs()
                && hit.lod_dist_pct.abs() > 0.3
                && hit.rel_volume >= 2.0
        ...
        Preset::MegaRangeMidpointCloseAtYearHighHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.hod_dist_pct.abs() > 0.5
                && hit.lod_dist_pct.abs() > 0.5
                && (hit.hod_dist_pct.abs() - hit.lod_dist_pct.abs()).abs() < 0.5
        ...
        Preset::MegaRangeMidpointCloseAtYearLowHotVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.hod_dist_pct.abs() > 0.5
                && hit.lod_dist_pct.abs() > 0.5
                && (hit.hod_dist_pct.abs() - hit.lod_dist_pct.abs()).abs() < 0.5
        ...
        Preset::MegaRangeUpNearYearHighQuintupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct > 0.0
                && hit.rel_volume >= 5.0
                && hit.year_high_pct < 2.0
        }
        Preset::MegaRangeDownNearYearLowQuintupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct < 0.0
                && hit.rel_volume >= 5.0
                && hit.year_low_pct < 2.0
        ...
        Preset::MegaRangeUpDeepBelowYearHighQuintupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct > 0.0
                && hit.rel_volume >= 5.0
                && hit.year_high_pct >= 20.0
        }
        Preset::MegaRangeDownDeepAboveYearLowQuintupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct < 0.0
                && hit.rel_volume >= 5.0
                && hit.year_low_pct >= 20.0
        }
        Preset::MegaRangeUpMidYearHighQuintupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct > 0.0
                && hit.rel_volume >= 5.0
                && hit.year_high_pct >= 5.0
        ...
        Preset::MegaRangeDownMidYearLowQuintupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.change_pct < 0.0
                && hit.rel_volume >= 5.0
                && hit.year_low_pct >= 5.0
        ...
        Preset::MegaRangeMidpointCloseNearYearHighQuintupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.hod_dist_pct.abs() > 0.5
                && hit.lod_dist_pct.abs() > 0.5
                && (hit.hod_dist_pct.abs() - hit.lod_dist_pct.abs()).abs() < 0.5
        ...
        Preset::MegaRangeMidpointCloseNearYearLowQuintupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.hod_dist_pct.abs() > 0.5
                && hit.lod_dist_pct.abs() > 0.5
                && (hit.hod_dist_pct.abs() - hit.lod_dist_pct.abs()).abs() < 0.5
        ...
        Preset::MegaRangeMidpointCloseConfirmedAboveYearHighQuintupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.hod_dist_pct.abs() > 0.5
                && hit.lod_dist_pct.abs() > 0.5
                && (hit.hod_dist_pct.abs() - hit.lod_dist_pct.abs()).abs() < 0.5
        ...
        Preset::MegaRangeMidpointCloseConfirmedBelowYearLowQuintupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.hod_dist_pct.abs() > 0.5
                && hit.lod_dist_pct.abs() > 0.5
                && (hit.hod_dist_pct.abs() - hit.lod_dist_pct.abs()).abs() < 0.5
        ...
        Preset::MegaRangeMidpointCloseDeepBelowYearHighQuintupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.hod_dist_pct.abs() > 0.5
                && hit.lod_dist_pct.abs() > 0.5
                && (hit.hod_dist_pct.abs() - hit.lod_dist_pct.abs()).abs() < 0.5
        ...
        Preset::MegaRangeMidpointCloseDeepAboveYearLowQuintupledVol => {
            hit.hod_dist_pct.abs() + hit.lod_dist_pct.abs() > 10.0
                && hit.hod_dist_pct.abs() > 0.5
                && hit.lod_dist_pct.abs() > 0.5
                && (hit.hod_dist_pct.abs() - hit.lod_dist_pct.abs()).abs() < 0.5
...
pub fn preset_label(p: Preset) -> &'static str {
    match p {
        ...
        Preset::GapUpWideRangeDeepBelowYearHighHotVol => "Gap Up (>2 %) + Wide Intraday Range (>8 %) + Hot Vol + Far below 52w High (>=20 %) (Gap-and-fight Deep in Pullback Territory: Overnight Gap up Followed by a Wide Trading Range Well below the Prior Peak with Elevated Participation; Recovery-attempt-vs-rejection Battle Where Catalyst Tries to Reverse the Extended Decline)",
        Preset::GapDownWideRangeDeepAboveYearLowHotVol => "Gap Down (<-2 %) + Wide Intraday Range (>8 %) + Hot Vol + Far above 52w Low (>=20 %) (Gap-and-fight Deep in Advance Territory: Overnight Gap down Followed by a Wide Trading Range Well above the Prior Trough with Elevated Participation; Top-attempt-vs-rebound Battle Where Catalyst Tries to Reverse the Extended Advance)",
        Preset::GapUpWideRangeMidYearHighHotVol => "Gap Up (>2 %) + Wide Intraday Range (>8 %) + Hot Vol + Mid-range from High (5-20 %) (Gap-and-fight in Mid-cycle Pullback Zone: Overnight Gap up Followed by a Wide Trading Range in the Proper Consolidation Range below the Prior Peak with Elevated Participation; Mid-cycle Consolidation-fight Where Catalyst Meets Resistance from Prior Consolidation Supply)",
        ...
        Preset::GapDownTightRangeDeepAboveYearLowHotVol => "Gap Down (<-2 %) + Tight Intraday Range (<1 %) + Hot Vol + Far above 52w Low (>=20 %) (Institutional Distribution at Elevated-level Catalyst: Overnight Gap down Positions Price Deep in Advance Territory then Regular Session Prints a Tight Trading Range with Heavy Participation; Gap-and-park Signal at the Top-fishing Zone Where Smart Money Quietly Exits without Selling Further)",
        Preset::GapUpTightRangeMidYearHighHotVol => "Gap Up (>2 %) + Tight Intraday Range (<1 %) + Hot Vol + Mid-range from High (5-20 %) (Institutional Absorption in Mid-cycle Pullback Zone: Overnight Gap up Positions Price in the Proper Consolidation Range below the Prior Peak then Regular Session Prints a Tight Trading Range with Heavy Participation; Gap-and-park Signal at Mid-cycle Where Smart Money Quietly Accumulates without Breakout Chase)",
        Preset::GapDownTightRangeMidYearLowHotVol => "Gap Down (<-2 %) + Tight Intraday Range (<1 %) + Hot Vol + Mid-range from Low (5-20 %) (Institutional Absorption in Mid-cycle Recovery Zone: Overnight Gap down Positions Price in the Proper Consolidation Range above the Prior Trough then Regular Session Prints a Tight Trading Range with Heavy Participation; Gap-and-park Signal at Mid-cycle Where Smart Money Quietly Distributes without Breakdown Follow-through)",
        Preset::GapUpTightRangeJustOffYearHighHotVol => "Gap Up (>2 %) + Tight Intraday Range (<1 %) + Hot Vol + Just off 52w High (2-5 %) (Institutional Absorption Just off the Year Peak: Overnight Gap up Positions Price Immediately after a Shallow Pullback from the 52w High then Regular Session Prints a Tight Trading Range with Heavy Participation; Gap-and-park Signal at the Post-tag Re-test Where Smart Money Positions before Another High Attempt)",
        Preset::GapDownTightRangeJustOffYearLowHotVol => "Gap Down (<-2 %) + Tight Intraday Range (<1 %) + Hot Vol + Just off 52w Low (2-5 %) (Institutional Absorption Just off the Year Trough: Overnight Gap down Positions Price Immediately after a Shallow Bounce from the 52w Low then Regular Session Prints a Tight Trading Range with Heavy Participation; Gap-and-park Signal at the Post-tag Re-test Where Smart Money Positions before Another Low Attempt)",
        ...
        Preset::DecupledVolUpJustOffYearHighHotVol => "Decupled Vol (>=10) + Big Up Move (>3 %) + Just off 52w High (2-5 %) (Unprecedented Catalyst Just off the Year Peak: Vol Is 10x Average and Price Prints a Significant Up Move Immediately after a Shallow Pullback from the 52w High; Tier-0 Once-per-year Catalyst at Post-tag Zone = Decisive Break-or-fail Event for the Breakout Thesis)",
        Preset::DecupledVolDownJustOffYearLowHotVol => "Decupled Vol (>=10) + Big Down Move (<-3 %) + Just off 52w Low (2-5 %) (Unprecedented Catalyst Just off the Year Trough: Vol Is 10x Average and Price Prints a Significant Down Move Immediately after a Shallow Bounce from the 52w Low; Tier-0 Once-per-year Catalyst at Post-tag Zone = Decisive Break-or-fail Event for the Breakdown Thesis)",
        Preset::DecupledVolCloseAtHodHotVol => "Decupled Vol (>=10) + Close Pinned to HOD + Green Close (Tier-0 Institutional Rally with No End-of-day Fade: Vol Is 10x Average and Close Pins to the Day's High with Positive Change; Rarest Possible Bull-conviction Close across the Entire Participation-tier Framework, Once-per-year-per-name Event with Full Close-strength)",
        ...
        Preset::HexaConfluenceHighHotVol => "Gap Up + Intraday Up + Close Pinned to HOD + Close Pinned to 52w High + Green Close + Quintupled Vol (Hexa-confluence New-high Day: Overnight Gap Aligned with Intraday Rally, Intraday Close at Day's High, Fresh 52w High, Quintupled Participation All Aligned Bull-direction; Six Orthogonal Axes Confirming Break with Both Overnight and Intraday Contributing to the Move at Tier-1 Vol Level)",
        ...
        Preset::CloseInUpperThirdDeepBelowYearHighHotVol => "Close in Upper Third + Far below 52w High (>=20 %) + Hot Vol (Soft Recovery-bias Tilt Deep in Pullback Territory: Close in Top Third of Intraday Range While Price Remains Well below the Prior Peak; Early-recovery Signal Where Buyers Start Gaining Control after Extended Decline but without Rigid HOD Pin)",
        ...
        Preset::BullRecoveryWickDeepBelowYearHighHotVol => "Long Lower Wick + Green Close + Not Pinned to HOD + Far below 52w High (>=20 %) + Hot Vol (Partial Bull-recovery Wick Deep in Pullback: Price Probed Lower Well below the Prior Peak then Recovered Green but Didn't Pin to HOD; Turnaround-forming Signal in Extended-decline Territory Where Buyers Begin to Absorb without Full Conviction)",
        Preset::BearRejectionWickDeepAboveYearLowHotVol => "Long Upper Wick + Red Close + Not Pinned to LOD + Far above 52w Low (>=20 %) + Hot Vol (Partial Bear-rejection Wick Deep in Advance: Price Probed Higher Well above the Prior Trough then Rejected Red but Didn't Pin to LOD; Top-forming Signal in Extended-advance Territory Where Sellers Begin to Distribute without Full Conviction)",
        Preset::BullRecoveryWickMidYearHighHotVol => "Long Lower Wick + Green Close + Not Pinned to HOD + Mid-range from High (5-20 %) + Hot Vol (Partial Bull-recovery Wick in Mid-cycle Pullback: Price Probed Lower in the Proper Consolidation Zone then Recovered Green but Didn't Pin to HOD; Mid-cycle Accumulation Signal Where Buyers Begin to Absorb in the Consolidation Phase without Full Conviction)",
        ...
        Preset::MegaRangeMidpointCloseJustOffYearHighHotVol => "Mega Intraday Range (>10 %) + Midpoint Close + Just off 52w High (2-5 %) + Doubled Vol (Post-tag Wide-range Standoff: Extreme Two-way Intraday Battle Immediately after a Shallow Pullback from the 52w High Ends with Balanced Midpoint Close; Post-tag Re-test Indecision Day with Maximum-energy Two-way Commitment)",
        Preset::MegaRangeMidpointCloseJustOffYearLowHotVol => "Mega Intraday Range (>10 %) + Midpoint Close + Just off 52w Low (2-5 %) + Doubled Vol (Post-tag Wide-range Standoff: Extreme Two-way Intraday Battle Immediately after a Shallow Bounce from the 52w Low Ends with Balanced Midpoint Close; Post-tag Re-test Indecision Day with Maximum-energy Two-way Commitment)",
        Preset::MegaRangeMidpointCloseAtYearHighDecupledVol => "Mega Intraday Range (>10 %) + Midpoint Close + At 52w High (<2 %) + Decupled Vol (Climactic Peak-day Wide-range Standoff: Extreme Two-way Intraday Battle at the 52w Peak with 10x Volume Ends with Balanced Midpoint Close; Institutional-scale Exhaustion-or-continuation Showdown at the Absolute High-water Mark)",
        ...
        Preset::MegaRangeMidpointCloseMidYearHighDecupledVol => "Mega Intraday Range (>10 %) + Midpoint Close + Mid-range from High (5-20 %) + Decupled Vol (Mid-cycle Wide-range Standoff: Extreme Two-way Intraday Battle in the Proper Consolidation Zone below the Prior Peak with 10x Volume Ends with Balanced Midpoint Close; Institutional-scale Pause-or-pivot Showdown in Mid-cycle Pullback Territory)",
        Preset::MegaRangeMidpointCloseMidYearLowDecupledVol => "Mega Intraday Range (>10 %) + Midpoint Close + Mid-range from Low (5-20 %) + Decupled Vol (Mid-cycle Wide-range Standoff: Extreme Two-way Intraday Battle in the Proper Consolidation Zone above the Prior Trough with 10x Volume Ends with Balanced Midpoint Close; Institutional-scale Pause-or-pivot Showdown in Mid-cycle Recovery Territory)",
        Preset::MegaRangeMidpointCloseJustOffYearHighDecupledVol => "Mega Intraday Range (>10 %) + Midpoint Close + Just off 52w High (2-5 %) + Decupled Vol (Post-tag Wide-range Standoff: Extreme Two-way Intraday Battle Immediately after a Shallow Pullback from the 52w High with 10x Volume Ends with Balanced Midpoint Close; Institutional-scale Re-test Indecision Day with Maximum-energy Two-way Commitment in the Post-tag Retracement Zone)",
        ...
        Preset::MegaRangeUpJustOffYearHighDecupledVol => "Mega Intraday Range (>10 %) + Green Close + Decupled Vol + Just off 52w High (2-5 %) (Institutional-scale Post-tag Volcanic Recovery: Extreme Intraday Range with Bull-direction Outcome Immediately after a Shallow Pullback from the 52w High with 10x Volume; Once-per-year Wide-range Push back toward the Breakout Level with Maximum-conviction Institutional Commitment to Re-test the Prior Peak)",
        Preset::MegaRangeDownJustOffYearLowDecupledVol => "Mega Intraday Range (>10 %) + Red Close + Decupled Vol + Just off 52w Low (2-5 %) (Institutional-scale Post-tag Volcanic Rejection: Extreme Intraday Range with Bear-direction Outcome Immediately after a Shallow Bounce from the 52w Low with 10x Volume; Once-per-year Wide-range Push back toward the Breakdown Level with Maximum-conviction Institutional Commitment to Re-test the Prior Trough)",
        Preset::MegaRangeUpperThirdNearYearHighDecupledVol => "Mega Intraday Range (>10 %) + Upper-third Close (Not Pinned to HOD) + Decupled Vol + At/near 52w High (<2 %) (Institutional-scale Partial-bull Breakout Day: Extreme Intraday Range with Close 2x Closer to HOD than LOD at the 52w Peak with 10x Volume; Once-per-year Regime-defining Event Marking Partial Bull-control at the High-water Mark without Rigid HOD Pinning)",
        Preset::MegaRangeUpperThirdNearYearLowDecupledVol => "Mega Intraday Range (>10 %) + Upper-third Close (Not Pinned to HOD) + Decupled Vol + At/near 52w Low (<2 %) (Institutional-scale Partial-bull Bottoming-bounce Day: Extreme Intraday Range with Close 2x Closer to HOD than LOD at the 52w Trough with 10x Volume; Once-per-year Regime-defining Event Marking Partial Bull-control at the Low-water Mark, Classic Capitulation-bottom Marker without Rigid HOD Pinning)",
        Preset::MegaRangeUpperThirdConfirmedAboveYearHighDecupledVol => "Mega Intraday Range (>10 %) + Upper-third Close (Not Pinned to HOD) + Decupled Vol + Confirmed-breakout Zone (1-3 % past 52w High) (Institutional-scale Partial-bull Follow-through Extension: Extreme Intraday Range with Close 2x Closer to HOD than LOD Just past Validated Breakout Level with 10x Volume; Once-per-year Maximum-conviction Continuation Marker Confirming Institutional Trend Acceleration without Rigid HOD Pinning)",
        ...
        Preset::MegaRangeUpperThirdJustOffYearHighDecupledVol => "Mega Intraday Range (>10 %) + Upper-third Close (Not Pinned to HOD) + Decupled Vol + Just off 52w High (2-5 %) (Institutional-scale Post-tag Partial-bull Retracement-recovery: Extreme Intraday Range with Close 2x Closer to HOD than LOD Immediately after a Shallow Pullback from the 52w High with 10x Volume; Once-per-year Re-test Marker with Maximum-conviction Institutional Commitment to Reclaim the Breakout Level without Rigid HOD Pinning)",
        Preset::MegaRangeUpperThirdJustOffYearLowDecupledVol => "Mega Intraday Range (>10 %) + Upper-third Close (Not Pinned to HOD) + Decupled Vol + Just off 52w Low (2-5 %) (Institutional-scale Post-tag Partial-bull Bounce-continuation: Extreme Intraday Range with Close 2x Closer to HOD than LOD Immediately after a Shallow Bounce from the 52w Low with 10x Volume; Once-per-year Re-test Marker Confirming the Bottom Hold with Maximum-conviction Institutional Bull Commitment without Rigid HOD Pinning)",
        Preset::MegaRangeLowerThirdNearYearHighDecupledVol => "Mega Intraday Range (>10 %) + Lower-third Close (Not Pinned to LOD) + Decupled Vol + At/near 52w High (<2 %) (Institutional-scale Partial-bear Failed-breakout Distribution Day: Extreme Intraday Range with Close 2x Closer to LOD than HOD at the 52w Peak with 10x Volume; Once-per-year Regime-defining Event Marking Partial Bear-control at the High-water Mark — Classic Bull-trap Distribution Marker Where Sellers Seized Control at the Peak without Rigid LOD Pinning)",
        Preset::MegaRangeLowerThirdNearYearLowDecupledVol => "Mega Intraday Range (>10 %) + Lower-third Close (Not Pinned to LOD) + Decupled Vol + At/near 52w Low (<2 %) (Institutional-scale Partial-bear Breakdown-continuation Day: Extreme Intraday Range with Close 2x Closer to LOD than HOD at the 52w Trough with 10x Volume; Once-per-year Regime-defining Event Marking Partial Bear-control at the Low-water Mark — Capitulation-extension Marker Where Sellers Maintained Control at the Trough without Rigid LOD Pinning)",
        Preset::MegaRangeLowerThirdConfirmedAboveYearHighDecupledVol => "Mega Intraday Range (>10 %) + Lower-third Close (Not Pinned to LOD) + Decupled Vol + Confirmed-breakout Zone (1-3 % past 52w High) (Institutional-scale Failed-confirmed-breakout Reversal: Extreme Intraday Range with Close 2x Closer to LOD than HOD Just past Validated Breakout Level with 10x Volume; Once-per-year Bear-reclaim Reversal Day Where Institutional Sellers Immediately Reasserted Control after the Breakout — Classic Bull-trap Marker at the Breakout Extension Where the Breakout Failed under Heavy Institutional Selling)",
        Preset::MegaRangeLowerThirdConfirmedBelowYearLowDecupledVol => "Mega Intraday Range (>10 %) + Lower-third Close (Not Pinned to LOD) + Decupled Vol + Confirmed-breakdown Zone (1-3 % past 52w Low) (Institutional-scale Partial-bear Breakdown Follow-through Extension: Extreme Intraday Range with Close 2x Closer to LOD than HOD Just past Validated Breakdown Level with 10x Volume; Once-per-year Maximum-conviction Continuation Marker Confirming Institutional Trend Deceleration without Rigid LOD Pinning)",
        Preset::MegaRangeLowerThirdDeepBelowYearHighDecupledVol => "Mega Intraday Range (>10 %) + Lower-third Close (Not Pinned to LOD) + Decupled Vol + Far below 52w High (≥20 %) (Institutional-scale Partial-bear Deep-pullback Capitulation-continuation: Extreme Intraday Range with Close 2x Closer to LOD than HOD Deep in Pullback Territory with 10x Volume; Once-per-year Capitulation Day Where Institutional Sellers Maintained Partial Control Deep in Extended Decline — Bear-trend Acceleration Marker in Late-stage Selloff without Rigid LOD Pinning)",
        Preset::MegaRangeLowerThirdDeepAboveYearLowDecupledVol => "Mega Intraday Range (>10 %) + Lower-third Close (Not Pinned to LOD) + Decupled Vol + Far above 52w Low (≥20 %) (Institutional-scale Partial-bear Waterfall-top Reversal: Extreme Intraday Range with Close 2x Closer to LOD than HOD Deep in Advance Territory with 10x Volume; Once-per-year Reversal Day Where Institutional Sellers Seized Partial Control after Absorbing Massive Buying — Historically a Marker for Major Highs in Extended Advances with Maximum-conviction Commitment but without Rigid LOD Pinning)",
        Preset::MegaRangeLowerThirdMidYearHighDecupledVol => "Mega Intraday Range (>10 %) + Lower-third Close (Not Pinned to LOD) + Decupled Vol + Mid-range from High (5-20 %) (Institutional-scale Mid-cycle Partial-bear Pullback-continuation: Extreme Intraday Range with Close 2x Closer to LOD than HOD in Proper Consolidation Zone below the Prior Peak with 10x Volume; Once-per-year Swing-trade Marker for Institutional Distribution-acceleration from Mid-cycle Pullback Top without Rigid LOD Pinning)",
        Preset::MegaRangeLowerThirdMidYearLowDecupledVol => "Mega Intraday Range (>10 %) + Lower-third Close (Not Pinned to LOD) + Decupled Vol + Mid-range from Low (5-20 %) (Institutional-scale Mid-cycle Partial-bear Recovery-rejection: Extreme Intraday Range with Close 2x Closer to LOD than HOD in Proper Consolidation Zone above the Prior Trough with 10x Volume; Once-per-year Swing-trade Marker for Institutional Recovery-failure from Mid-cycle Bounce Top without Rigid LOD Pinning)",
        Preset::MegaRangeLowerThirdJustOffYearHighDecupledVol => "Mega Intraday Range (>10 %) + Lower-third Close (Not Pinned to LOD) + Decupled Vol + Just off 52w High (2-5 %) (Institutional-scale Post-tag Partial-bear Retracement-continuation: Extreme Intraday Range with Close 2x Closer to LOD than HOD Immediately after a Shallow Pullback from the 52w High with 10x Volume; Once-per-year Re-test Marker with Maximum-conviction Institutional Bear Commitment Confirming the Rejection from the Breakout Level without Rigid LOD Pinning)",
        Preset::MegaRangeLowerThirdJustOffYearLowDecupledVol => "Mega Intraday Range (>10 %) + Lower-third Close (Not Pinned to LOD) + Decupled Vol + Just off 52w Low (2-5 %) (Institutional-scale Post-tag Partial-bear Bounce-rejection: Extreme Intraday Range with Close 2x Closer to LOD than HOD Immediately after a Shallow Bounce from the 52w Low with 10x Volume; Once-per-year Re-test Marker Confirming Failed-bounce Rejection back toward the Breakdown Level with Maximum-conviction Institutional Bear Commitment without Rigid LOD Pinning)",
        Preset::MegaRangeTrendDayUpNearYearHighDecupledVol => "Mega Intraday Range (>10 %) + Close Pinned to HOD (<0.5 %) + Green Close + Decupled Vol + At/near 52w High (<2 %) (Institutional-scale Trend-day-up Volcanic Breakout: Extreme Intraday Range with Bull-direction Outcome Closing Rigidly Pinned to HOD at the 52w Peak with 10x Volume; Once-per-year Maximum-domination Peak Event — Full Institutional Bull Control from Open to Close Marking a Fresh High without Any Meaningful Intraday Retrace)",
        Preset::MegaRangeTrendDayUpNearYearLowDecupledVol => "Mega Intraday Range (>10 %) + Close Pinned to HOD (<0.5 %) + Green Close + Decupled Vol + At/near 52w Low (<2 %) (Institutional-scale Trend-day-up V-bottom Capitulation-reversal: Extreme Intraday Range with Bull-direction Outcome Closing Rigidly Pinned to HOD at the 52w Trough with 10x Volume; Once-per-year Maximum-domination Capitulation-bottom Event — Full Institutional Bull Reversal from Open to Close Marking the Absolute Low-water Mark with HOD-pinned Conviction)",
        Preset::MegaRangeTrendDayUpConfirmedAboveYearHighDecupledVol => "Mega Intraday Range (>10 %) + Close Pinned to HOD (<0.5 %) + Green Close + Decupled Vol + Confirmed-breakout Zone (1-3 % past 52w High) (Institutional-scale Trend-day-up Follow-through Extension: Extreme Intraday Range with HOD-pinned Close Just past the Validated Breakout Level with 10x Volume; Once-per-year Maximum-domination Continuation Event — Full Institutional Bull Control Extending past the Cleared Prior Peak, the Strongest Possible Follow-through Signature)",
        Preset::MegaRangeTrendDayUpConfirmedBelowYearLowDecupledVol => "Mega Intraday Range (>10 %) + Close Pinned to HOD (<0.5 %) + Green Close + Decupled Vol + Confirmed-breakdown Zone (1-3 % past 52w Low) (Institutional-scale Trend-day-up Failed-breakdown Bull-reclaim Reversal: Extreme Intraday Range with HOD-pinned Close Just past the Validated Breakdown Level with 10x Volume; Once-per-year Maximum-conviction Bear-trap Reversal Event — Full Institutional Bull Reversal from Open to Close after the Breakdown Failed, the Strongest Possible Bear-trap Signature)",
        Preset::MegaRangeTrendDayUpDeepBelowYearHighDecupledVol => "Mega Intraday Range (>10 %) + Close Pinned to HOD (<0.5 %) + Green Close + Decupled Vol + Far below 52w High (≥20 %) (Institutional-scale Trend-day-up Maximum-conviction V-bottom Reversal: Extreme Intraday Range with HOD-pinned Close Deep in Pullback Territory with 10x Volume; Once-per-year Reversal Day Where Institutional Buyers Completely Absorbed Massive Selling and Held the High to Close — Historically the Strongest Possible Major-low Marker in Extended Declines with Full HOD-pinned Domination)",
        ...
        Preset::MegaRangeTrendDayDownNearYearHighDecupledVol => "Mega Intraday Range (>10 %) + Close Pinned to LOD (<0.5 %) + Red Close + Decupled Vol + At/near 52w High (<2 %) (Institutional-scale Trend-day-down Failed-peak Distribution: Extreme Intraday Range with Bear-direction Outcome Closing Rigidly Pinned to LOD at the 52w Peak with 10x Volume; Once-per-year Maximum-domination Peak-failure Event — Full Institutional Bear Control from Open to Close Marking a Failed-peak Reversal with LOD-pinned Conviction, the Strongest Possible Distribution-day Signature)",
        Preset::MegaRangeTrendDayDownNearYearLowDecupledVol => "Mega Intraday Range (>10 %) + Close Pinned to LOD (<0.5 %) + Red Close + Decupled Vol + At/near 52w Low (<2 %) (Institutional-scale Trend-day-down Volcanic Breakdown: Extreme Intraday Range with Bear-direction Outcome Closing Rigidly Pinned to LOD at the 52w Trough with 10x Volume; Once-per-year Maximum-domination Trough Event — Full Institutional Bear Control from Open to Close Marking a Fresh Low with LOD-pinned Conviction, the Strongest Possible Volcanic-breakdown Signature)",
        Preset::MegaRangeTrendDayDownConfirmedAboveYearHighDecupledVol => "Mega Intraday Range (>10 %) + Close Pinned to LOD (<0.5 %) + Red Close + Decupled Vol + Confirmed-breakout Zone (1-3 % past 52w High) (Institutional-scale Trend-day-down Failed-confirmed-breakout Reversal: Extreme Intraday Range with LOD-pinned Close Just past the Validated Breakout Level with 10x Volume; Once-per-year Full Bear-reclaim Event — Full Institutional Bear Reversal from Open to Close after the Breakout Failed, the Strongest Possible Bull-trap Signature with LOD-pinned Conviction)",
        Preset::MegaRangeTrendDayDownConfirmedBelowYearLowDecupledVol => "Mega Intraday Range (>10 %) + Close Pinned to LOD (<0.5 %) + Red Close + Decupled Vol + Confirmed-breakdown Zone (1-3 % past 52w Low) (Institutional-scale Trend-day-down Confirmed-breakdown Follow-through Extension: Extreme Intraday Range with LOD-pinned Close Just past the Validated Breakdown Level with 10x Volume; Once-per-year Maximum-domination Continuation Event — Full Institutional Bear Control Extending past the Cleared Prior Trough, the Strongest Possible Bear-follow-through Signature)",
        Preset::MegaRangeTrendDayDownDeepBelowYearHighDecupledVol => "Mega Intraday Range (>10 %) + Close Pinned to LOD (<0.5 %) + Red Close + Decupled Vol + Far below 52w High (≥20 %) (Institutional-scale Trend-day-down Deep-pullback Capitulation Extension: Extreme Intraday Range with LOD-pinned Close Deep in Pullback Territory with 10x Volume; Once-per-year Maximum-conviction Capitulation Event — Full Institutional Bear Control from Open to Close Deep in Extended Decline, the Strongest Possible Bear-trend Acceleration Signature in Late-stage Selloff with LOD-pinned Domination)",
        Preset::MegaRangeTrendDayDownDeepAboveYearLowDecupledVol => "Mega Intraday Range (>10 %) + Close Pinned to LOD (<0.5 %) + Red Close + Decupled Vol + Far above 52w Low (≥20 %) (Institutional-scale Trend-day-down Waterfall-top Full Reversal: Extreme Intraday Range with LOD-pinned Close Deep in Advance Territory with 10x Volume; Once-per-year Maximum-conviction Reversal Event — Full Institutional Bear Reversal from Open to Close after Absorbing Massive Buying — Historically the Strongest Possible Major-high Marker in Extended Advances with LOD-pinned Domination)",
        Preset::MegaRangeTrendDayDownMidYearHighDecupledVol => "Mega Intraday Range (>10 %) + Close Pinned to LOD (<0.5 %) + Red Close + Decupled Vol + Mid-range from High (5-20 %) (Institutional-scale Trend-day-down Mid-cycle Pullback-continuation: Extreme Intraday Range with LOD-pinned Close in Proper Consolidation Zone below the Prior Peak with 10x Volume; Once-per-year Swing-trade Marker — Strongest Possible Mid-cycle Distribution-acceleration Signature from Pullback Top with Full LOD-pinned Institutional Bear Conviction)",
        Preset::MegaRangeTrendDayDownMidYearLowDecupledVol => "Mega Intraday Range (>10 %) + Close Pinned to LOD (<0.5 %) + Red Close + Decupled Vol + Mid-range from Low (5-20 %) (Institutional-scale Trend-day-down Mid-cycle Recovery-rejection: Extreme Intraday Range with LOD-pinned Close in Proper Consolidation Zone above the Prior Trough with 10x Volume; Once-per-year Swing-trade Marker — Strongest Possible Mid-cycle Recovery-failure Signature from Bounce Top with Full LOD-pinned Institutional Bear Conviction)",
        Preset::MegaRangeTrendDayDownJustOffYearHighDecupledVol => "Mega Intraday Range (>10 %) + Close Pinned to LOD (<0.5 %) + Red Close + Decupled Vol + Just off 52w High (2-5 %) (Institutional-scale Trend-day-down Post-tag Breakdown-rejection: Extreme Intraday Range with LOD-pinned Close Immediately after a Shallow Pullback from the 52w High with 10x Volume; Once-per-year Re-test Rejection Event — Strongest Possible Re-test Rejection Signature Confirming the Prior Peak Has Failed with Full LOD-pinned Institutional Bear Conviction Pushing back toward the Breakdown Level)",
        Preset::MegaRangeTrendDayDownJustOffYearLowDecupledVol => "Mega Intraday Range (>10 %) + Close Pinned to LOD (<0.5 %) + Red Close + Decupled Vol + Just off 52w Low (2-5 %) (Institutional-scale Trend-day-down Post-tag Bottom-failure: Extreme Intraday Range with LOD-pinned Close Immediately after a Shallow Bounce from the 52w Low with 10x Volume; Once-per-year Re-test Breakdown Event — Strongest Possible Re-test Breakdown Signature Confirming the Prior Trough Has Failed with Full LOD-pinned Institutional Bear Conviction Pushing back toward the Breakdown Level)",
        ...
        Preset::MegaRangeTrendDayUpConfirmedAboveYearHighHotVol => "Mega Intraday Range (>10 %) + Close Pinned to HOD (<0.5 %) + Green Close + Doubled Vol + Confirmed-breakout Zone (1-3 % past 52w High) (Mid-tier Trend-day-up Follow-through Extension: Extreme Intraday Range with HOD-pinned Close Just past the Validated Breakout Level with Doubled Volume; Once-per-month Maximum-domination Continuation Event — Full Bull Control Extending past the Cleared Prior Peak, Mid-tier Follow-through Signature)",
        Preset::MegaRangeTrendDayUpConfirmedBelowYearLowHotVol => "Mega Intraday Range (>10 %) + Close Pinned to HOD (<0.5 %) + Green Close + Doubled Vol + Confirmed-breakdown Zone (1-3 % past 52w Low) (Mid-tier Trend-day-up Failed-breakdown Bull-reclaim Reversal: Extreme Intraday Range with HOD-pinned Close Just past the Validated Breakdown Level with Doubled Volume; Once-per-month Maximum-conviction Bear-trap Reversal Event — Full Bull Reversal from Open to Close after the Breakdown Failed, Mid-tier Bear-trap Signature)",
        Preset::MegaRangeTrendDayUpDeepBelowYearHighHotVol => "Mega Intraday Range (>10 %) + Close Pinned to HOD (<0.5 %) + Green Close + Doubled Vol + Far below 52w High (≥20 %) (Mid-tier Trend-day-up V-bottom Reversal: Extreme Intraday Range with HOD-pinned Close Deep in Pullback Territory with Doubled Volume; Once-per-month Reversal Day Where Buyers Completely Absorbed Massive Selling and Held the High to Close — Mid-tier Major-low Marker in Extended Declines with HOD-pinned Domination)",
        Preset::MegaRangeTrendDayUpDeepAboveYearLowHotVol => "Mega Intraday Range (>10 %) + Close Pinned to HOD (<0.5 %) + Green Close + Doubled Vol + Far above 52w Low (≥20 %) (Mid-tier Trend-day-up Late-stage Advance Momentum-continuation: Extreme Intraday Range with HOD-pinned Close Deep in Advance Territory with Doubled Volume; Once-per-month Continuation Day Where Buyers Maintained Full Control from Open to Close Deep in Extended Advance — Mid-tier Momentum-acceleration Marker in Late-stage Rally with HOD-pinned Conviction)",
        Preset::MegaRangeTrendDayUpMidYearHighHotVol => "Mega Intraday Range (>10 %) + Close Pinned to HOD (<0.5 %) + Green Close + Doubled Vol + Mid-range from High (5-20 %) (Mid-tier Trend-day-up Mid-cycle Pullback-recovery: Extreme Intraday Range with HOD-pinned Close in Proper Consolidation Zone below the Prior Peak with Doubled Volume; Once-per-month Swing-trade Marker — Mid-tier Trend-resumption Signature from Pullback Bottom with HOD-pinned Conviction)",
        Preset::MegaRangeTrendDayUpMidYearLowHotVol => "Mega Intraday Range (>10 %) + Close Pinned to HOD (<0.5 %) + Green Close + Doubled Vol + Mid-range from Low (5-20 %) (Mid-tier Trend-day-up Mid-cycle Recovery-continuation: Extreme Intraday Range with HOD-pinned Close in Proper Consolidation Zone above the Prior Trough with Doubled Volume; Once-per-month Swing-trade Marker — Mid-tier Recovery-acceleration Signature from Bounce Base with HOD-pinned Conviction)",
        Preset::MegaRangeTrendDayUpJustOffYearHighHotVol => "Mega Intraday Range (>10 %) + Close Pinned to HOD (<0.5 %) + Green Close + Doubled Vol + Just off 52w High (2-5 %) (Mid-tier Trend-day-up Post-tag Breakout-reclaim: Extreme Intraday Range with HOD-pinned Close Immediately after a Shallow Pullback from the 52w High with Doubled Volume; Once-per-month Re-test Reclaim Event — Mid-tier Re-test Reclaim Signature of the Prior Peak with HOD-pinned Conviction Restoring the Breakout Level)",
        Preset::MegaRangeTrendDayUpJustOffYearLowHotVol => "Mega Intraday Range (>10 %) + Close Pinned to HOD (<0.5 %) + Green Close + Doubled Vol + Just off 52w Low (2-5 %) (Mid-tier Trend-day-up Post-tag Bottom-hold: Extreme Intraday Range with HOD-pinned Close Immediately after a Shallow Bounce from the 52w Low with Doubled Volume; Once-per-month Re-test Bottom-confirmation Event — Mid-tier Re-test Bottom-hold Signature Confirming the Prior Trough Has Held with HOD-pinned Bull Conviction)",
        Preset::MegaRangeTrendDayDownNearYearHighHotVol => "Mega Intraday Range (>10 %) + Close Pinned to LOD (<0.5 %) + Red Close + Doubled Vol + At/near 52w High (<2 %) (Mid-tier Trend-day-down Failed-peak Distribution: Extreme Intraday Range with Bear-direction Outcome Closing Rigidly Pinned to LOD at the 52w Peak with Doubled Volume; Once-per-month Maximum-domination Peak-failure Event — Full Bear Control from Open to Close Marking a Failed-peak Reversal with LOD-pinned Conviction, Mid-tier Distribution-day Signature)",
        Preset::MegaRangeTrendDayDownNearYearLowHotVol => "Mega Intraday Range (>10 %) + Close Pinned to LOD (<0.5 %) + Red Close + Doubled Vol + At/near 52w Low (<2 %) (Mid-tier Trend-day-down Volcanic Breakdown: Extreme Intraday Range with Bear-direction Outcome Closing Rigidly Pinned to LOD at the 52w Trough with Doubled Volume; Once-per-month Maximum-domination Trough Event — Full Bear Control from Open to Close Marking a Fresh Low with LOD-pinned Conviction, Mid-tier Volcanic-breakdown Signature)",
        Preset::MegaRangeTrendDayDownConfirmedAboveYearHighHotVol => "Mega Intraday Range (>10 %) + Close Pinned to LOD (<0.5 %) + Red Close + Doubled Vol + Confirmed-breakout Zone (1-3 % past 52w High) (Mid-tier Trend-day-down Failed-confirmed-breakout Reversal: Extreme Intraday Range with LOD-pinned Close Just past the Validated Breakout Level with Doubled Volume; Once-per-month Full Bear-reclaim Event — Full Bear Reversal from Open to Close after the Breakout Failed, Mid-tier Bull-trap Signature with LOD-pinned Conviction)",
        Preset::MegaRangeTrendDayDownConfirmedBelowYearLowHotVol => "Mega Intraday Range (>10 %) + Close Pinned to LOD (<0.5 %) + Red Close + Doubled Vol + Confirmed-breakdown Zone (1-3 % past 52w Low) (Mid-tier Trend-day-down Confirmed-breakdown Follow-through Extension: Extreme Intraday Range with LOD-pinned Close Just past the Validated Breakdown Level with Doubled Volume; Once-per-month Maximum-domination Continuation Event — Full Bear Control Extending past the Cleared Prior Trough, Mid-tier Bear-follow-through Signature)",
        Preset::MegaRangeTrendDayDownDeepBelowYearHighHotVol => "Mega Intraday Range (>10 %) + Close Pinned to LOD (<0.5 %) + Red Close + Doubled Vol + Far below 52w High (≥20 %) (Mid-tier Trend-day-down Deep-pullback Capitulation Extension: Extreme Intraday Range with LOD-pinned Close Deep in Pullback Territory with Doubled Volume; Once-per-month Maximum-conviction Capitulation Event — Full Bear Control from Open to Close Deep in Extended Decline, Mid-tier Bear-trend Acceleration Signature in Late-stage Selloff with LOD-pinned Domination)",
        Preset::MegaRangeTrendDayDownDeepAboveYearLowHotVol => "Mega Intraday Range (>10 %) + Close Pinned to LOD (<0.5 %) + Red Close + Doubled Vol + Far above 52w Low (≥20 %) (Mid-tier Trend-day-down Waterfall-top Full Reversal: Extreme Intraday Range with LOD-pinned Close Deep in Advance Territory with Doubled Volume; Once-per-month Maximum-conviction Reversal Event — Full Bear Reversal from Open to Close after Absorbing Massive Buying — Mid-tier Major-high Marker in Extended Advances with LOD-pinned Domination)",
        Preset::MegaRangeTrendDayDownMidYearHighHotVol => "Mega Intraday Range (>10 %) + Close Pinned to LOD (<0.5 %) + Red Close + Doubled Vol + Mid-range from High (5-20 %) (Mid-tier Trend-day-down Mid-cycle Pullback-continuation: Extreme Intraday Range with LOD-pinned Close in Proper Consolidation Zone below the Prior Peak with Doubled Volume; Once-per-month Swing-trade Marker — Mid-tier Distribution-acceleration Signature from Pullback Top with LOD-pinned Bear Conviction)",
        Preset::MegaRangeTrendDayDownMidYearLowHotVol => "Mega Intraday Range (>10 %) + Close Pinned to LOD (<0.5 %) + Red Close + Doubled Vol + Mid-range from Low (5-20 %) (Mid-tier Trend-day-down Mid-cycle Recovery-rejection: Extreme Intraday Range with LOD-pinned Close in Proper Consolidation Zone above the Prior Trough with Doubled Volume; Once-per-month Swing-trade Marker — Mid-tier Recovery-failure Signature from Bounce Top with LOD-pinned Bear Conviction)",
        Preset::MegaRangeTrendDayDownJustOffYearHighHotVol => "Mega Intraday Range (>10 %) + Close Pinned to LOD (<0.5 %) + Red Close + Doubled Vol + Just off 52w High (2-5 %) (Mid-tier Trend-day-down Post-tag Breakdown-rejection: Extreme Intraday Range with LOD-pinned Close Immediately after a Shallow Pullback from the 52w High with Doubled Volume; Once-per-month Re-test Rejection Event — Mid-tier Re-test Rejection Signature Confirming the Prior Peak Has Failed with LOD-pinned Bear Conviction Pushing back toward the Breakdown Level)",
        Preset::MegaRangeTrendDayDownJustOffYearLowHotVol => "Mega Intraday Range (>10 %) + Close Pinned to LOD (<0.5 %) + Red Close + Doubled Vol + Just off 52w Low (2-5 %) (Mid-tier Trend-day-down Post-tag Bottom-failure: Extreme Intraday Range with LOD-pinned Close Immediately after a Shallow Bounce from the 52w Low with Doubled Volume; Once-per-month Re-test Breakdown Event — Mid-tier Re-test Breakdown Signature Confirming the Prior Trough Has Failed with LOD-pinned Bear Conviction Pushing back toward the Breakdown Level)",
        Preset::MegaRangeUpperThirdNearYearHighHotVol => "Mega Intraday Range (>10 %) + Upper-third Close (Not Pinned to HOD) + Doubled Vol + At/near 52w High (<2 %) (Mid-tier Partial-bull Breakout Day: Extreme Intraday Range with Close 2x Closer to HOD than LOD at the 52w Peak with Doubled Volume; Once-per-month Regime-defining Event Marking Partial Bull-control at the High-water Mark without Rigid HOD Pinning)",
        Preset::MegaRangeUpperThirdNearYearLowHotVol => "Mega Intraday Range (>10 %) + Upper-third Close (Not Pinned to HOD) + Doubled Vol + At/near 52w Low (<2 %) (Mid-tier Partial-bull Bottoming-bounce Day: Extreme Intraday Range with Close 2x Closer to HOD than LOD at the 52w Trough with Doubled Volume; Once-per-month Regime-defining Event Marking Partial Bull-control at the Low-water Mark, Mid-tier Capitulation-bottom Marker without Rigid HOD Pinning)",
        Preset::MegaRangeUpperThirdConfirmedAboveYearHighHotVol => "Mega Intraday Range (>10 %) + Upper-third Close (Not Pinned to HOD) + Doubled Vol + Confirmed-breakout Zone (1-3 % past 52w High) (Mid-tier Partial-bull Follow-through Extension: Extreme Intraday Range with Close 2x Closer to HOD than LOD Just past Validated Breakout Level with Doubled Volume; Once-per-month Continuation Marker Confirming Mid-tier Trend Acceleration without Rigid HOD Pinning)",
        ...
        Preset::MegaRangeUpperThirdJustOffYearHighHotVol => "Mega Intraday Range (>10 %) + Upper-third Close (Not Pinned to HOD) + Doubled Vol + Just off 52w High (2-5 %) (Mid-tier Post-tag Partial-bull Retracement-recovery: Extreme Intraday Range with Close 2x Closer to HOD than LOD Immediately after a Shallow Pullback from the 52w High with Doubled Volume; Once-per-month Re-test Marker Confirming Partial-bull Commitment to Reclaim the Breakout Level without Rigid HOD Pinning)",
        Preset::MegaRangeUpperThirdJustOffYearLowHotVol => "Mega Intraday Range (>10 %) + Upper-third Close (Not Pinned to HOD) + Doubled Vol + Just off 52w Low (2-5 %) (Mid-tier Post-tag Partial-bull Bounce-continuation: Extreme Intraday Range with Close 2x Closer to HOD than LOD Immediately after a Shallow Bounce from the 52w Low with Doubled Volume; Once-per-month Re-test Marker Confirming the Bottom Hold with Partial-bull Commitment without Rigid HOD Pinning)",
        Preset::MegaRangeLowerThirdNearYearHighHotVol => "Mega Intraday Range (>10 %) + Lower-third Close (Not Pinned to LOD) + Doubled Vol + At/near 52w High (<2 %) (Mid-tier Partial-bear Failed-breakout Distribution Day: Extreme Intraday Range with Close 2x Closer to LOD than HOD at the 52w Peak with Doubled Volume; Once-per-month Regime-defining Event Marking Partial Bear-control at the High-water Mark — Bull-trap Distribution Marker without Rigid LOD Pinning)",
        Preset::MegaRangeLowerThirdNearYearLowHotVol => "Mega Intraday Range (>10 %) + Lower-third Close (Not Pinned to LOD) + Doubled Vol + At/near 52w Low (<2 %) (Mid-tier Partial-bear Breakdown-continuation Day: Extreme Intraday Range with Close 2x Closer to LOD than HOD at the 52w Trough with Doubled Volume; Once-per-month Regime-defining Event Marking Partial Bear-control at the Low-water Mark — Capitulation-extension Marker without Rigid LOD Pinning)",
        Preset::MegaRangeLowerThirdConfirmedAboveYearHighHotVol => "Mega Intraday Range (>10 %) + Lower-third Close (Not Pinned to LOD) + Doubled Vol + Confirmed-breakout Zone (1-3 % past 52w High) (Mid-tier Failed-confirmed-breakout Reversal: Extreme Intraday Range with Close 2x Closer to LOD than HOD Just past Validated Breakout Level with Doubled Volume; Once-per-month Bear-reclaim Reversal Day Where Sellers Reasserted Control after the Breakout — Mid-tier Bull-trap Marker at the Breakout Extension)",
        Preset::MegaRangeLowerThirdConfirmedBelowYearLowHotVol => "Mega Intraday Range (>10 %) + Lower-third Close (Not Pinned to LOD) + Doubled Vol + Confirmed-breakdown Zone (1-3 % past 52w Low) (Mid-tier Partial-bear Breakdown Follow-through Extension: Extreme Intraday Range with Close 2x Closer to LOD than HOD Just past Validated Breakdown Level with Doubled Volume; Once-per-month Continuation Marker Confirming Mid-tier Trend Deceleration without Rigid LOD Pinning)",
        Preset::MegaRangeLowerThirdDeepBelowYearHighHotVol => "Mega Intraday Range (>10 %) + Lower-third Close (Not Pinned to LOD) + Doubled Vol + Far below 52w High (≥20 %) (Mid-tier Partial-bear Deep-pullback Capitulation-continuation: Extreme Intraday Range with Close 2x Closer to LOD than HOD Deep in Pullback Territory with Doubled Volume; Once-per-month Capitulation Day Where Sellers Maintained Partial Control Deep in Extended Decline — Mid-tier Bear-trend Acceleration Marker in Late-stage Selloff without Rigid LOD Pinning)",
        ...
        Preset::MegaRangeMidpointCloseAtYearHighHotVol => "Mega Intraday Range (>10 %) + Midpoint Close + Doubled Vol + At 52w High (<2 %) (Mid-tier Peak-day Wide-range Standoff: Extreme Two-way Intraday Battle at the 52w Peak with Doubled Volume Ends with Balanced Midpoint Close; Once-per-month Exhaustion-or-continuation Showdown at the Absolute High-water Mark)",
        ...
        Preset::MegaRangeDownConfirmedBelowYearLowQuintupledVol => "Mega Intraday Range (>10 %) + Red Close + Quintupled Vol + Confirmed-breakdown Zone (1-3 % past 52w Low) (Institutional-tier Volcanic Extension past Validated Breakdown: Extreme Intraday Range with Bear-direction Outcome Extending past the Already-cleared Prior Trough with 5x Volume; Quarterly-rare Follow-through Day Confirming Institutional Trend Acceleration)",
        Preset::MegaRangeUpDeepBelowYearHighQuintupledVol => "Mega Intraday Range (>10 %) + Green Close + Quintupled Vol + Far below 52w High (≥20 %) (Institutional-tier V-bottom Signal: Extreme Intraday Range with Bull-direction Outcome Deep in Pullback Territory with 5x Volume; Quarterly-rare Reversal Day Where Institutional Buyers Absorbed Massive Selling and Reversed it — Historically a Marker for Major Lows in Extended Declines)",
        ...
        Preset::MegaRangeDownJustOffYearLowQuintupledVol => "Mega Intraday Range (>10 %) + Red Close + Quintupled Vol + Just off 52w Low (2-5 %) (Institutional-tier Post-tag Volcanic Rejection: Extreme Intraday Range with Bear-direction Outcome Immediately after a Shallow Bounce from the 52w Low with 5x Volume; Quarterly-rare Wide-range Push back toward the Breakdown Level with Strong Institutional Commitment to Re-test the Prior Trough)",
        Preset::MegaRangeMidpointCloseNearYearHighQuintupledVol => "Mega Intraday Range (>10 %) + Midpoint Close + Quintupled Vol + At/near 52w High (<2 %) (Institutional-tier Peak-day Wide-range Standoff: Extreme Two-way Intraday Battle at the 52w Peak with 5x Volume Ends with Balanced Midpoint Close; Quarterly-rare Exhaustion-or-continuation Showdown at the Absolute High-water Mark with Strong Institutional Commitment on Both Sides)",
        Preset::MegaRangeMidpointCloseNearYearLowQuintupledVol => "Mega Intraday Range (>10 %) + Midpoint Close + Quintupled Vol + At/near 52w Low (<2 %) (Institutional-tier Trough-day Wide-range Standoff: Extreme Two-way Intraday Battle at the 52w Trough with 5x Volume Ends with Balanced Midpoint Close; Quarterly-rare Capitulation-or-reversal Showdown at the Absolute Low-water Mark with Strong Institutional Commitment on Both Sides)",
        Preset::MegaRangeMidpointCloseConfirmedAboveYearHighQuintupledVol => "Mega Intraday Range (>10 %) + Midpoint Close + Quintupled Vol + Confirmed-breakout Zone (1-3 % past 52w High) (Institutional-tier Post-breakout Wide-range Standoff: Extreme Two-way Intraday Battle Just past the Validated Breakout Level with 5x Volume Ends with Balanced Midpoint Close; Quarterly-rare Follow-through-or-fade Showdown in the Confirmed Expansion Zone with Strong Institutional Commitment)",
        Preset::MegaRangeMidpointCloseConfirmedBelowYearLowQuintupledVol => "Mega Intraday Range (>10 %) + Midpoint Close + Quintupled Vol + Confirmed-breakdown Zone (1-3 % past 52w Low) (Institutional-tier Post-breakdown Wide-range Standoff: Extreme Two-way Intraday Battle Just past the Validated Breakdown Level with 5x Volume Ends with Balanced Midpoint Close; Quarterly-rare Follow-through-or-fade Showdown in the Confirmed Contraction Zone with Strong Institutional Commitment)",
        Preset::MegaRangeMidpointCloseDeepBelowYearHighQuintupledVol => "Mega Intraday Range (>10 %) + Midpoint Close + Quintupled Vol + Far below 52w High (≥20 %) (Institutional-tier Exhaustion-zone Wide-range Standoff: Extreme Two-way Intraday Battle Deep below the Prior Peak with 5x Volume Ends with Balanced Midpoint Close; Quarterly-rare Capitulation-or-reversal Turning-point Indecision in Deep-pullback Territory with Strong Institutional Commitment)",
        Preset::MegaRangeMidpointCloseDeepAboveYearLowQuintupledVol => "Mega Intraday Range (>10 %) + Midpoint Close + Quintupled Vol + Far above 52w Low (≥20 %) (Institutional-tier Deep-recovery Wide-range Standoff: Extreme Two-way Intraday Battle Deep above the Prior Trough with 5x Volume Ends with Balanced Midpoint Close; Quarterly-rare Euphoria-or-pullback Turning-point Indecision in Deep-recovery Territory with Strong Institutional Commitment)",
    }
}
