...
pub fn price(
    ...
    if exercise_dates_years
        .iter()
        .any(|t| !t.is_finite() || *t <= 0.0 || *t > time_to_expiry)
    {
        return None;
    }
    ...
    let mut exercise_steps: Vec<usize> = exercise_dates_years
        .iter()
        .map(|t| ((t / dt).round() as isize).clamp(1, n_steps as isize) as usize)
        .collect();
    exercise_steps.sort();
    exercise_steps.dedup();
    let exercise_at_step = |s: usize| -> bool { exercise_steps.binary_search(&s).is_ok() };
...
mod tests {
    ...
    fn full_grid_exercise_approximates_american() {
        ...
        .unwrap();
        let european = price(100.0, 100.0, 1.0, 0.05, 0.0, 0.20, n, &[], OptionKind::Put).unwrap();
