...
use crate::indicators::{
    adx, bollinger, classic_pivots, closes, highs, lows, macd, rsi, sma, stochastic, Macd, Pivots,
    Stoch,
};
...
pub fn analyze(symbol: &str, bars: &[PriceBar]) -> SignalReport {
    ...
    let pivots = if n >= 2 {
        ...
        Some(classic_pivots(
            crate::indicators::highs(bars)[pi],
    ...
    if let (Some(a), Some(pdi), Some(mdi)) = (
        last(&adx14.adx),
        last(&adx14.plus_di),
        last(&adx14.minus_di),
    ) {
    ...
    if n >= 2 {
        ...
        if let (Some(pk), Some(pd), Some(k), Some(d)) = (pk, pd, last(&stoch_k), last(&stoch_d)) {
            if pk <= pd && k > d && k < 30.0 {
                signals.push(Signal {
                    name: "Stochastic bullish (oversold)",
                    side: "buy",
                    weight: 1,
                ...
                signals.push(Signal {
                    name: "Stochastic bearish (overbought)",
                    side: "sell",
                    weight: -1,
    ...
    if n >= 5 {
        ...
        if hi_window.iter().all(|x| *x <= center_hi)
            && hi_window[2] > hi_window[0]
            && hi_window[2] > hi_window[4]
        {
            signals.push(Signal {
                name: "Pivot Top",
                side: "sell",
        ...
        if lo_window.iter().all(|x| *x >= center_lo)
            && lo_window[2] < lo_window[0]
            && lo_window[2] < lo_window[4]
        {
            signals.push(Signal {
                name: "Pivot Bottom",
                side: "buy",
...
mod tests {
    ...
    fn analyze_summary_is_one_of_three_labels() {
        ...
        assert!(
            matches!(r.summary, "buy" | "sell" | "hold"),
            "summary must be buy/sell/hold, got: {}",
            r.summary
        );
    }
    ...
    fn analyze_uptrend_yields_positive_or_neutral_score() {
        ...
        assert!(
            r.score >= -2,
            "uptrend should not score deeply bearish, got {}",
            r.score
        );
    }
