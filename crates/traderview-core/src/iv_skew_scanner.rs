...
impl Default for Config {
    fn default() -> Self {
        Self {
            min_strikes_in_wing: 3,
            min_abs_risk_reversal: 0.02,
        }
    }
}
...
pub fn scan(strips: &[SymbolIvStrip], cfg: &Config) -> Vec<SkewHit> {
    ...
    for strip in strips {
        if !strip.spot.is_finite() || strip.spot <= 0.0 || strip.strikes.is_empty() {
            continue;
        }
        ...
        let (Some(atm), Some(c25), Some(p25)) = (atm, d25_call, d25_put) else {
            continue;
        };
        let Some(atm_iv) = atm.call_iv.or(atm.put_iv) else {
            continue;
        };
        let Some(c25_iv) = c25.call_iv else { continue };
        let Some(p25_iv) = p25.put_iv else { continue };
        if !atm_iv.is_finite() || !c25_iv.is_finite() || !p25_iv.is_finite() {
            continue;
        }
        ...
        // [0.85, 1.0] · spot.
        let wing_pts: Vec<(f64, f64)> = strip
            .strikes
            .iter()
            .filter_map(|s| {
                let iv = s.put_iv?;
        ...
        if rr.abs() < cfg.min_abs_risk_reversal {
            continue;
        }
        hits.push(SkewHit {
...
mod tests {
    ...
    fn butterfly_positive_for_smile() {
        ...
        assert!(
            h.butterfly_25d > 0.0,
            "smile should yield positive butterfly, got {}",
            h.butterfly_25d
        );
    }
    ...
    fn put_wing_slope_computed_when_enough_strikes() {
        ...
        assert!(
            hits[0].put_wing_slope < 0.0,
            "put-wing slope should be negative, got {}",
            hits[0].put_wing_slope
        );
    }
}
