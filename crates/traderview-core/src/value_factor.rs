...
pub fn scan(symbols: &[SymbolFundamentals]) -> Option<ValueFactorReport> {
    ...
    for s in symbols {
        if !s.book_value_per_share.is_finite()
            || !s.price_per_share.is_finite()
            || s.book_value_per_share <= 0.0
            || s.price_per_share <= 0.0
        {
            continue;
        }
    ...
    let n = ratios.len();
    let mut all_ranked: Vec<ValueHit> = ratios
        .iter()
        .enumerate()
        .map(|(i, (sym, bm))| {
            let pct = (i + 1) as f64 / n as f64 * 100.0;
            let bucket = if pct >= 90.0 {
                ValueBucket::ValueDecile
            } else if pct <= 10.0 {
                ValueBucket::GrowthDecile
            } else {
                ValueBucket::Neutral
            };
            ValueHit {
    ...
    all_ranked.sort_by(|a, b| {
        b.book_to_market
            .partial_cmp(&a.book_to_market)
            .unwrap_or(std::cmp::Ordering::Equal)
    });
    let value_decile: Vec<ValueHit> = all_ranked
        .iter()
        .filter(|h| h.bucket == ValueBucket::ValueDecile)
        .cloned()
        .collect();
    let growth_decile: Vec<ValueHit> = all_ranked
        .iter()
        .filter(|h| h.bucket == ValueBucket::GrowthDecile)
        .cloned()
        .collect();
    Some(ValueFactorReport {
        value_decile,
        growth_decile,
        all_ranked,
    })
...
mod tests {
    ...
    fn top_10_pct_goes_to_value_decile() {
        ...
        let symbols: Vec<_> = (1..=10)
            .map(|i| s(&format!("S{i}"), i as f64, 1.0))
            .collect();
        let r = scan(&symbols).unwrap();
    ...
    fn percentiles_in_unit_range() {
        let symbols: Vec<_> = (1..=20)
            .map(|i| s(&format!("S{i:02}"), i as f64, 1.0))
            .collect();
        let r = scan(&symbols).unwrap();
