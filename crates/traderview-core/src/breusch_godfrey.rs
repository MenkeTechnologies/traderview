...
pub fn test(x: &[f64], y: &[f64], lag_order: usize) -> Option<BreuschGodfreyReport> {
    let n = x.len();
    if n < lag_order + 8 || y.len() != n || lag_order == 0 {
        return None;
    }
...
mod tests {
    ...
    fn iid_residuals_do_not_reject() {
        ...
        assert!(
            !r.reject_at_5pct,
            "IID residuals shouldn't reject, LM = {}, p = {}",
    ...
    fn ar1_residuals_reject() {
        ...
        assert!(
            r.reject_at_5pct,
            "AR(1) residuals should reject, LM = {}",
            r.lm_statistic
        );
    }
