...
pub fn solve(
    ...
    max_iter: usize,
    tolerance: f64,
) -> Option<RiskParityReport> {
    let n = covariance.len();
    if n < 2 || covariance.iter().any(|row| row.len() != n) {
        return None;
    }
    if covariance.iter().any(|row| row.iter().any(|c| !c.is_finite())) {
        return None;
    }
    ...
    for _ in 0..max_iter {
        ...
        let new_port_vol: f64 = new_w.iter().zip(new_sigma_w.iter())
            .map(|(a, b)| a * b).sum::<f64>().max(0.0).sqrt();
        if new_port_vol <= 0.0 { return None; }
        let contributions: Vec<f64> = new_w.iter().zip(new_sigma_w.iter())
            .map(|(a, b)| a * b / new_port_vol)
            .collect();
        let target = contributions.iter().sum::<f64>() / n as f64;
        max_dev = contributions.iter()
            .map(|c| (c - target).abs())
    ...
    let contributions: Vec<f64> = w.iter().zip(sigma_w.iter())
        .map(|(a, b)| if port_vol > 0.0 { a * b / port_vol } else { 0.0 })
        .collect();
...
mod tests {
    ...
    fn high_vol_asset_gets_lower_weight() {
        ...
        assert!((ratio - 3.0).abs() < 0.01, "expected ratio ≈ 3, got {ratio}");
    }
