...
pub fn compute(
    ...
    if !(0.0..=1.0).contains(&up_prob) {
        return None;
    }
    if eta_up <= 1.0 || eta_down <= 0.0 {
        return None;
    }
    if steps < 1 || paths < 1 {
        return None;
    ...
    let kappa =
        up_prob * eta_up / (eta_up - 1.0) + (1.0 - up_prob) * eta_down / (eta_down + 1.0) - 1.0;
...
mod tests {
    ...
    fn asymmetric_p_yields_jump_count_asymmetry() {
        ...
        let r = compute(100.0, 0.0, 0.10, 5.0, 0.1, 10.0, 5.0, 0.01, 100, 200, 7).unwrap();
    ...
    fn down_skewed_distribution_yields_negative_skew() {
        ...
        let r = compute(100.0, 0.0, 0.10, 5.0, 0.1, 20.0, 3.0, 0.01, 252, 5000, 7).unwrap();
