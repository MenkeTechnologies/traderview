...
pub fn compute(
    ...
    if window < 2 || n < window
        ...
        || !risk_free_per_period.is_finite() { return out; }
    if returns.iter().any(|x| !x.is_finite()) { return out; }
    ...
    for (i, slot) in out.iter_mut().enumerate().skip(window - 1) {
        ...
        let var: f64 = win.iter().map(|r| (r - risk_free_per_period - mean).powi(2))
            .sum::<f64>() / (n_f - 1.0);
...
mod tests {
    ...
    fn risk_free_subtracted() {
        ...
        let r: Vec<f64> = (0..50).map(|i| 0.001 + ((i as f64 * 0.3).sin()) * 0.005).collect();
        ...
        assert!(last_no_rf > last_rf,
            "no-rf Sharpe {} should exceed rf=0.5% Sharpe {}", last_no_rf, last_rf);
    }
}
