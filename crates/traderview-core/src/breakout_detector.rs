...
mod tests {
    ...
    fn buffer_filters_marginal_breakouts() {
        ...
        assert!(
            r.events.is_empty(),
            "marginal breakout should be filtered by buffer, got {} events",
            r.events.len()
        );
    }
    ...
    fn close_only_filters_wick_false_signals() {
        ...
        assert_eq!(
            intraday.events.len(),
            1,
            "wick should trigger intraday-mode breakout"
        );
        let closes = detect(
            &bars,
            &BreakoutConfig {
        ...
        assert!(
            closes.events.is_empty(),
            "wick should NOT trigger close-only breakout"
        );
    }
