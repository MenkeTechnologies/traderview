...
impl Default for AnalyzerConfig {
    fn default() -> Self {
        Self { grid_low_pct_of_strike: 0.6, grid_high_pct_of_strike: 1.4, grid_points: 81 }
    }
}
...
pub fn analyze(spread: &CalendarSpread, cfg: &AnalyzerConfig) -> Option<CalendarReport> {
    if !spread.strike.is_finite() || spread.strike <= 0.0
        || !spread.front_premium.is_finite() || spread.front_premium < 0.0
        || !spread.back_premium.is_finite()  || spread.back_premium < 0.0
        || !spread.back_time_after_front_expiry.is_finite()
        || spread.back_time_after_front_expiry <= 0.0
        || !spread.sigma.is_finite() || spread.sigma <= 0.0
        || !spread.multiplier.is_finite() || spread.multiplier <= 0.0
        || spread.contracts == 0
        || cfg.grid_points < 3
        || !cfg.grid_low_pct_of_strike.is_finite() || cfg.grid_low_pct_of_strike <= 0.0
    ...
    let kind_sign = match spread.kind { OptionKind::Call => 1.0, OptionKind::Put => -1.0 };
    ...
    for k in 0..cfg.grid_points {
        ...
        let back_value = bs_price(
            s, spread.strike, spread.back_time_after_front_expiry,
            spread.risk_free, spread.dividend_yield, spread.sigma, spread.kind,
        );
        ...
        if pnl > max_profit { max_profit = pnl; max_profit_at = s; }
    ...
    for w in grid.windows(2) {
        let (a, b) = (w[0], w[1]);
        if (a.pnl == 0.0) || (a.pnl.signum() != b.pnl.signum() && a.pnl.is_finite() && b.pnl.is_finite()) {
            if a.pnl == 0.0 {
    ...
    Some(CalendarReport {
        net_debit, pnl_grid: grid, breakevens, max_profit, max_profit_at, max_loss,
    })
}
...
mod tests {
    ...
    fn cs(strike: f64, kind: OptionKind, front: f64, back: f64) -> CalendarSpread {
        CalendarSpread {
            strike, kind,
            front_premium: front, back_premium: back,
            back_time_after_front_expiry: 0.25,
            risk_free: 0.05, dividend_yield: 0.0,
            sigma: 0.25,
            contracts: 1, multiplier: 100.0,
        }
    }
    ...
    fn classic_at_the_money_calendar_has_max_profit_near_strike() {
        ...
        let r = analyze(&cs(100.0, OptionKind::Call, 2.0, 5.0), &AnalyzerConfig::default()).unwrap();
        assert!(r.net_debit > 0.0);
        // Max profit should be at or very near the strike.
        assert!((r.max_profit_at - 100.0).abs() / 100.0 < 0.10,
            "max-profit spot should be near 100, got {}", r.max_profit_at);
    ...
    fn far_otm_pnl_approaches_negative_net_debit() {
        ...
        assert!(far_itm.pnl < r.max_profit, "wings should lose vs max profit");
    }
    ...
    fn put_calendar_supported() {
        let r = analyze(&cs(100.0, OptionKind::Put, 2.0, 5.0), &AnalyzerConfig::default()).unwrap();
        assert!(r.max_profit > 0.0);
    }
    ...
    fn config_validation() {
        ...
        let cfg = AnalyzerConfig { grid_points: 2, ..Default::default() };
        assert!(analyze(&s, &cfg).is_none());
        let cfg = AnalyzerConfig { grid_low_pct_of_strike: 1.5, grid_high_pct_of_strike: 0.5, ..Default::default() };
        assert!(analyze(&s, &cfg).is_none());
    }
}
