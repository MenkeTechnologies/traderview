...
pub enum SwaptionKind { Payer, Receiver }
...
pub fn price(
    ...
    if !forward_swap_rate.is_finite() || forward_swap_rate <= 0.0
        || !strike_rate.is_finite() || strike_rate <= 0.0
        || !sigma.is_finite() || sigma <= 0.0
        || !time_to_expiry.is_finite() || time_to_expiry <= 0.0
        || !annuity_pv01.is_finite() || annuity_pv01 <= 0.0
...
mod tests {
    ...
    fn deep_itm_payer_approaches_intrinsic() {
        ...
        let r = price(0.07, 0.04, 0.25, 5.0, 7.5, 10_000_000.0, SwaptionKind::Payer).unwrap();
    ...
    fn deep_otm_payer_approaches_zero() {
        ...
        let r = price(0.01, 0.08, 0.25, 0.1, 7.5, 10_000_000.0, SwaptionKind::Payer).unwrap();
        assert!(r.price < 1_000.0);
    }

    #[test]
    fn higher_vol_inflates_atm_swaption() {
        let r_low = price(0.04, 0.04, 0.05, 5.0, 7.5, 10_000_000.0, SwaptionKind::Payer).unwrap();
        let r_high = price(0.04, 0.04, 0.50, 5.0, 7.5, 10_000_000.0, SwaptionKind::Payer).unwrap();
        assert!(r_high.price > r_low.price);
    }

    #[test]
    fn longer_expiry_inflates_atm_swaption() {
        let r_short = price(0.04, 0.04, 0.25, 0.5, 7.5, 10_000_000.0, SwaptionKind::Payer).unwrap();
        let r_long  = price(0.04, 0.04, 0.25, 5.0, 7.5, 10_000_000.0, SwaptionKind::Payer).unwrap();
