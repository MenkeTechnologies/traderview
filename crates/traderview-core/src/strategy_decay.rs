...
impl Default for DecayConfig {
    fn default() -> Self {
        Self { broken_below: 0.5, eroding_slope: -0.05, acceleration_floor: -0.10 }
    }
}
...
pub fn analyze(rolling_sharpe: &[f64], cfg: &DecayConfig) -> DecayReport {
    ...
    if n == 0 {
        return DecayReport { note: "empty input".into(), ..Default::default() };
    }
    let latest = rolling_sharpe[n - 1];
    let mut peak = f64::NEG_INFINITY;
    let mut peak_idx = 0;
    for (i, &s) in rolling_sharpe.iter().enumerate() {
        if s > peak { peak = s; peak_idx = i; }
    }
    if n < 3 {
        return DecayReport {
            verdict: if latest < cfg.broken_below { DecayVerdict::Broken } else { DecayVerdict::Healthy },
    ...
    let note = match verdict {
        DecayVerdict::Broken if latest < cfg.broken_below =>
            format!("latest Sharpe {latest:.2} is below the {:.2} floor", cfg.broken_below),
        DecayVerdict::Broken =>
            format!("decay is accelerating — acceleration {acceleration:.3} past the {:.3} threshold", cfg.acceleration_floor),
        DecayVerdict::Eroding =>
            format!("rolling Sharpe falling at {slope:.3} per period"),
        DecayVerdict::Healthy =>
            "rolling Sharpe trend is healthy".into(),
    };
    DecayReport {
        verdict, slope_per_period: slope, acceleration,
        latest_sharpe: latest, peak_sharpe: peak, peak_index: peak_idx, note,
    }
}
...
mod tests {
    ...
    fn slow_steady_erosion_is_eroding() {
        ...
        let r = analyze(&[2.0, 1.8, 1.6, 1.4, 1.2, 1.0, 0.8], &DecayConfig::default());
        assert!(matches!(r.verdict, DecayVerdict::Eroding),
            "expected Eroding for steady decline, got {:?} (slope={})", r.verdict, r.slope_per_period);
    ...
    fn accelerating_decay_classifies_as_broken() {
        ...
        let r = analyze(&[2.0, 1.99, 1.98, 1.97, 1.9, 1.5, 1.0, 0.6], &DecayConfig::default());
        assert!(matches!(r.verdict, DecayVerdict::Broken),
            "expected Broken for cliff, got {:?}", r.verdict);
