...
pub fn compute(
    ...
    if !spot.is_finite() || spot <= 0.0
        ...
        || chain.len() < 3 { return None; }
    if chain.iter().any(|q| !q.strike.is_finite() || q.strike <= 0.0
        || !q.mid_price.is_finite() || q.mid_price < 0.0) {
        return None;
    }
    ...
    for i in 0..n {
        let dk = if n == 1 { 0.0 }
            else if i == 0 { otm[1].0 - otm[0].0 }
            else if i == n - 1 { otm[n - 1].0 - otm[n - 2].0 }
            else { 0.5 * (otm[i + 1].0 - otm[i - 1].0) };
...
mod tests {
    ...
    fn put(k: f64, p: f64) -> OptionQuote {
        OptionQuote { strike: k, option_type: OptionType::Put, mid_price: p }
    }
    fn call(k: f64, p: f64) -> OptionQuote {
        OptionQuote { strike: k, option_type: OptionType::Call, mid_price: p }
    }
    ...
    fn vol_strike_is_sqrt_of_variance_strike() {
        let chain = vec![
            put(80.0, 0.20), put(95.0, 1.50),
            put(99.0, 2.50), call(101.0, 2.50),
            call(105.0, 1.40), call(120.0, 0.15),
        ];
        let r = compute(100.0, 0.05, 0.25, &chain).unwrap();
    ...
    fn richer_otm_chain_yields_higher_var_strike() {
        ...
        let cheap_otm = vec![
            put(80.0, 0.10), put(95.0, 0.50),
            put(99.0, 1.00), call(101.0, 1.00),
            call(105.0, 0.50), call(120.0, 0.10),
        ];
        let rich_otm = vec![
            put(80.0, 2.00), put(95.0, 4.00),
            put(99.0, 5.00), call(101.0, 5.00),
            call(105.0, 4.00), call(120.0, 2.00),
        ];
