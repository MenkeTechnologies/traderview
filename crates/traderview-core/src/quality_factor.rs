...
pub enum QualityBucket { QualityDecile, JunkDecile, Neutral }
...
pub fn scan(symbols: &[SymbolQualityInputs]) -> Option<QualityFactorReport> {
    ...
    let valid: Vec<&SymbolQualityInputs> = symbols.iter()
        .filter(|s| s.return_on_equity.is_finite()
    ...
    let mean_stdev = |getter: &dyn Fn(&SymbolQualityInputs) -> f64| -> Option<(f64, f64)> {
        ...
        if stdev > 0.0 { Some((mean, stdev)) } else { None }
    };
    ...
    let mut all_ranked: Vec<QualityHit> = valid.iter().map(|s| {
    ...
    all_ranked.sort_by(|a, b| a.quality_score.partial_cmp(&b.quality_score)
        .unwrap_or(std::cmp::Ordering::Equal));
    for (i, h) in all_ranked.iter_mut().enumerate() {
        h.percentile = (i + 1) as f64 / n as f64 * 100.0;
        h.bucket = if h.percentile >= 90.0 { QualityBucket::QualityDecile }
            else if h.percentile <= 10.0 { QualityBucket::JunkDecile }
            else { QualityBucket::Neutral };
    }
    all_ranked.sort_by(|a, b| b.quality_score.partial_cmp(&a.quality_score)
        .unwrap_or(std::cmp::Ordering::Equal));
    let quality_decile: Vec<QualityHit> = all_ranked.iter()
        .filter(|h| h.bucket == QualityBucket::QualityDecile).cloned().collect();
    let junk_decile: Vec<QualityHit> = all_ranked.iter()
        .filter(|h| h.bucket == QualityBucket::JunkDecile).cloned().collect();
    Some(QualityFactorReport { quality_decile, junk_decile, all_ranked })
}
...
mod tests {
    ...
    fn ranked_descending_by_quality_score() {
        let symbols: Vec<_> = (1..=15).map(|i| s(&format!("S{i:02}"),
            0.05 + i as f64 * 0.02, 1.0 - i as f64 * 0.05, 0.1)).collect();
        let r = scan(&symbols).unwrap();
