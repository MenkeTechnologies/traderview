...
impl Default for Config {
    fn default() -> Self {
        Self { volume_per_bucket: 50_000.0, window_buckets: 50, return_window: 100 }
    }
}
...
pub fn compute(ticks: &[Tick], cfg: &Config) -> VpinReport {
    ...
    if cfg.volume_per_bucket <= 0.0 || !cfg.volume_per_bucket.is_finite()
        || cfg.window_buckets == 0 || cfg.return_window == 0
        || ticks.is_empty()
    {
        return report;
    }
    ...
    for i in 1..n {
        if ticks[i].price.is_finite() && ticks[i - 1].price.is_finite()
            && ticks[i].price > 0.0 && ticks[i - 1].price > 0.0
        {
    ...
    for (i, t) in ticks.iter().enumerate() {
        ...
        let omega = if i == 0 {
            ...
            let z = if sigmas[i] > 0.0 { log_returns[i] / sigmas[i] } else { 0.0 };
            norm_cdf(z)
        };
        ...
        while remaining_total > 0.0 {
            ...
            if space <= 0.0 {
                ...
                buy = 0.0; sell = 0.0; bucket_vol = 0.0;
                continue;
            }
    ...
    let imbalances: Vec<f64> = (0..nbuckets)
        .map(|i| (report.bucket_buy_volume[i] - report.bucket_sell_volume[i]).abs()
...
mod tests {
    ...
    fn t(p: f64, v: f64) -> Tick { Tick { price: p, volume: v } }
    ...
    fn invalid_config_returns_default() {
        ...
        for cfg in [
            Config { volume_per_bucket: 0.0, ..Default::default() },
            Config { volume_per_bucket: f64::NAN, ..Default::default() },
            Config { window_buckets: 0, ..Default::default() },
            Config { return_window: 0, ..Default::default() },
        ] {
            assert!(compute(&ticks, &cfg).vpin.is_empty()
    ...
    fn buckets_aggregate_to_target_volume() {
        ...
        let ticks: Vec<Tick> = (0..100).map(|i| t(100.0 + i as f64 * 0.01, 100.0)).collect();
        let cfg = Config { volume_per_bucket: 1_000.0, window_buckets: 3, return_window: 10 };
        ...
        for (b, s) in r.bucket_buy_volume.iter().zip(r.bucket_sell_volume.iter()) {
            assert!((b + s - 1_000.0).abs() < 1e-6, "bucket sum should equal target");
        }
    }
    ...
    fn monotonic_uptrend_produces_high_vpin() {
        ...
        let cfg = Config { volume_per_bucket: 500.0, window_buckets: 5, return_window: 20 };
        let r = compute(&ticks, &cfg);
        // The trailing VPIN should be high (toxic — informed buying).
        let last = r.vpin.iter().rev().find_map(|x| *x).expect("populated");
        assert!(last > 0.5, "monotonic uptrend should yield high VPIN, got {last}");
    }
    ...
    fn nan_tick_skipped_safely() {
        ...
        let cfg = Config { volume_per_bucket: 500.0, window_buckets: 3, return_window: 10 };
        let r = compute(&ticks, &cfg);
