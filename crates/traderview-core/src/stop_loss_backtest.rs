...
pub fn simulate(trades: &[TradeOutcome], params: &StopParams, side_long: bool) -> MethodResult {
    ...
    for t in trades {
        ...
        let stop_price = match params.method {
            StopMethod::None        => f64::NEG_INFINITY,
            StopMethod::FixedDollar => if side_long { t.entry - stop_level } else { t.entry + stop_level },
            StopMethod::FixedPct    => if side_long { t.entry * (1.0 - params.value) } else { t.entry * (1.0 + params.value) },
            StopMethod::AtrMultiple => if side_long { t.entry - stop_level } else { t.entry + stop_level },
        };
        // Determine if stop would have been hit (MAE breaches stop?).
        let mae_price = if side_long { t.entry - t.mae } else { t.entry + t.mae };
        let hit_stop = if side_long { mae_price <= stop_price } else { mae_price >= stop_price };
        let realized = if hit_stop {
            stopped += 1;
            if side_long { stop_price - t.entry } else { t.entry - stop_price }
        } else {
            if side_long { t.actual_exit - t.entry } else { t.entry - t.actual_exit }
        };
...
pub fn best_of(trades: &[TradeOutcome], candidates: &[StopParams], side_long: bool)
    -> Vec<MethodResult>
{
    let mut results: Vec<MethodResult> = candidates.iter()
        .map(|c| simulate(trades, c, side_long))
        .collect();
    results.sort_by(|a, b| b.total_realized.partial_cmp(&a.total_realized)
...
mod tests {
    ...
    fn t(entry: f64, mae: f64, mfe: f64, exit: f64) -> TradeOutcome {
        TradeOutcome { entry, mae, mfe, actual_exit: exit }
    }

    #[test]
    fn empty_trades_zero_result() {
        let r = simulate(&[], &StopParams { method: StopMethod::None, value: 0.0, atr: 0.0 }, true);
    ...
    fn no_stop_uses_actual_exit() {
        ...
        let r = simulate(&[t(100.0, 5.0, 20.0, 110.0)],
            &StopParams { method: StopMethod::None, value: 0.0, atr: 0.0 }, true);
    ...
    fn fixed_dollar_stop_triggers_on_mae() {
        ...
        let r = simulate(&[t(100.0, 5.0, 20.0, 110.0)],
            &StopParams { method: StopMethod::FixedDollar, value: 3.0, atr: 0.0 }, true);
    ...
    fn fixed_pct_stop_triggers_on_mae() {
        ...
        let r = simulate(&[t(100.0, 10.0, 20.0, 110.0)],
            &StopParams { method: StopMethod::FixedPct, value: 0.05, atr: 0.0 }, true);
        assert_eq!(r.stopped_out_count, 1);
    ...
    fn atr_multiple_stop_triggers_on_mae() {
        ...
        let r = simulate(&[t(100.0, 10.0, 20.0, 110.0)],
            &StopParams { method: StopMethod::AtrMultiple, value: 3.0, atr: 2.0 }, true);
        assert_eq!(r.stopped_out_count, 1);
    ...
    fn stop_not_triggered_when_mae_above_stop() {
        ...
        let r = simulate(&[t(100.0, 1.0, 20.0, 110.0)],
            &StopParams { method: StopMethod::FixedDollar, value: 3.0, atr: 0.0 }, true);
    ...
    fn short_trade_stop_inverts() {
        ...
        let r = simulate(&[t(100.0, 5.0, 20.0, 95.0)],
            &StopParams { method: StopMethod::FixedDollar, value: 3.0, atr: 0.0 }, false);
        assert_eq!(r.stopped_out_count, 1);
        assert_eq!(r.total_realized, -3.0);
    }
    ...
    fn best_of_picks_highest_total_realized() {
        ...
        let candidates = vec![
            StopParams { method: StopMethod::FixedDollar, value: 0.5, atr: 0.0 },    // too tight
            StopParams { method: StopMethod::FixedDollar, value: 5.0, atr: 0.0 },    // loose
            StopParams { method: StopMethod::None, value: 0.0, atr: 0.0 },
        ];
    ...
    fn wins_counted_correctly() {
        ...
        let r = simulate(&trades, &StopParams { method: StopMethod::None, value: 0.0, atr: 0.0 }, true);
