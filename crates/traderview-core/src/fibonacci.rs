...
pub enum SwingDirection { Up, Down }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum FibKind { Retracement, Extension }
...
pub fn compute(input: &FibInput) -> FibReport {
    ...
    FibReport { retracements, extensions, range }
}
...
mod tests {
    ...
    fn up_swing_50pct_retracement_at_midpoint() {
        ...
        let r = compute(&FibInput {
            swing_high: 200.0, swing_low: 100.0, direction: SwingDirection::Up,
        });
    ...
    fn down_swing_50pct_retracement_at_midpoint() {
        ...
        let r = compute(&FibInput {
            swing_high: 200.0, swing_low: 100.0, direction: SwingDirection::Down,
        });
        let fifty = r.retracements.iter().find(|l| l.ratio == 0.5).unwrap();
        assert_eq!(fifty.price, 150.0);
    }
    ...
    fn up_swing_786_retracement_near_low() {
        ...
        let level = r.retracements.iter().find(|l| (l.ratio - 0.786).abs() < 1e-6).unwrap();
    ...
    fn up_swing_extension_161_above_high() {
        ...
        let ext = r.extensions.iter().find(|l| (l.ratio - 1.618).abs() < 1e-6).unwrap();
    ...
    fn zero_range_swing_collapses_all_levels_to_same_price() {
        // Degenerate: high == low.
        let r = compute(&FibInput {
            swing_high: 100.0, swing_low: 100.0, direction: SwingDirection::Up,
        });
    ...
    fn swing_high_lower_than_low_uses_abs_range() {
        ...
        let r = compute(&FibInput {
            swing_high: 100.0, swing_low: 200.0, direction: SwingDirection::Up,
        });
