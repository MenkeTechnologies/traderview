...
pub fn test(sample_x: &[f64], sample_y: &[f64]) -> Option<WilcoxonReport> {
    ...
    if sample_x.iter().any(|v| !v.is_finite()) || sample_y.iter().any(|v| !v.is_finite()) {
        return None;
    }
    // Pairwise differences; drop zeros.
    let diffs: Vec<f64> = sample_x
        .iter()
        .zip(sample_y.iter())
        .map(|(x, y)| x - y)
        .filter(|d| *d != 0.0)
    ...
    while i < n {
        let mut j = i;
        while j + 1 < n && abs_diffs[idx[j + 1]] == abs_diffs[idx[i]] {
            j += 1;
        }
...
mod tests {
    ...
    fn systematic_positive_shift_rejects() {
        ...
        assert!(
            r.reject_at_5pct,
            "x > y consistently → should reject, z = {}",
            r.z_statistic
        );
    }
    ...
    fn ties_handled_with_zeros_dropped() {
        ...
        let y = vec![
            1.0, 1.5, 3.5, 4.0, 4.5, 6.5, 7.5, 8.5, 9.5, 10.0, 11.5, 12.5,
        ];
        let r = test(&x, &y).unwrap();
