...
pub fn price(
    ...
    let p_m = 1.0 - p_u - p_d;
    if !(0.0..=1.0).contains(&p_u) || !(0.0..=1.0).contains(&p_d)
        || !(0.0..=1.0).contains(&p_m)
    {
        return None;
    }
    ...
    for step in (0..n_steps).rev() {
        ...
        for j in 0..node_count {
            let continuation = disc * (
                p_u * values[j] + p_m * values[j + 1] + p_d * values[j + 2]
            );
    ...
    Some(TrinomialReport { price: values[0], n_steps })
}
...
mod tests {
    ...
    fn invalid_inputs_return_none() {
        for bad in [0.0, -1.0, f64::NAN] {
            ...
            assert!(price(100.0, 100.0, bad, 0.05, 0.0, 0.2, 100, OptionKind::Call, true).is_none());
            assert!(price(100.0, 100.0, 0.5, 0.05, 0.0, bad, 100, OptionKind::Call, true).is_none());
        }
    ...
    fn european_call_close_to_black_scholes() {
        ...
        let r = price(100.0, 100.0, 1.0, 0.05, 0.0, 0.20, 200, OptionKind::Call, false).unwrap();
        // BS ATM call ≈ 10.45.
    ...
    fn american_put_above_european_put() {
        let r_amer = price(100.0, 100.0, 1.0, 0.05, 0.0, 0.20, 200, OptionKind::Put, true).unwrap();
        let r_euro = price(100.0, 100.0, 1.0, 0.05, 0.0, 0.20, 200, OptionKind::Put, false).unwrap();
    ...
    fn american_call_on_non_div_equals_european_to_within_steps() {
        ...
        let r_amer = price(100.0, 100.0, 1.0, 0.05, 0.0, 0.20, 200, OptionKind::Call, true).unwrap();
        let r_euro = price(100.0, 100.0, 1.0, 0.05, 0.0, 0.20, 200, OptionKind::Call, false).unwrap();
    ...
    fn deep_itm_american_put_intrinsic_floor() {
        ...
        let r = price(50.0, 100.0, 0.5, 0.05, 0.0, 0.20, 200, OptionKind::Put, true).unwrap();
        assert!(r.price >= 49.99);
    }

    #[test]
    fn convergence_with_more_steps() {
        let r_50 = price(100.0, 100.0, 1.0, 0.05, 0.02, 0.25, 50, OptionKind::Put, true).unwrap();
        let r_500 = price(100.0, 100.0, 1.0, 0.05, 0.02, 0.25, 500, OptionKind::Put, true).unwrap();
        assert!((r_50.price - r_500.price).abs() / r_500.price < 0.03);
    }

    #[test]
    fn higher_vol_inflates_atm_european() {
        let r_low  = price(100.0, 100.0, 0.5, 0.05, 0.0, 0.10, 100, OptionKind::Call, false).unwrap();
        let r_high = price(100.0, 100.0, 0.5, 0.05, 0.0, 0.40, 100, OptionKind::Call, false).unwrap();
