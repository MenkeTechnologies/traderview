...
pub fn analyze(features: &[TradeFeature], k: usize, max_iters: usize) -> ClusterReport {
    ...
    for _ in 0..max_iters {
        ...
        for (j, centroid) in centroids.iter_mut().enumerate().take(k) {
            let members: Vec<&TradeFeature> = features
                .iter()
                .enumerate()
    ...
    for (j, centroid) in centroids.iter().enumerate().take(k) {
        let idxs: Vec<usize> = (0..features.len())
        ...
        let wins = idxs
            .iter()
            .filter(|i| features[**i].r_multiple > 0.0)
            .count();
        let win_rate = if size > 0 {
            wins as f64 / size as f64
        } else {
            0.0
        };
...
mod tests {
    ...
    fn f(em: f64, hd: f64, r: f64) -> TradeFeature {
        TradeFeature {
            entry_minute_of_day: em,
            hold_duration_minutes: hd,
            r_multiple: r,
        }
    }
    ...
    fn two_distinct_clusters_separated() {
        ...
        for i in 0..3 {
            assert_eq!(r.assignments[i], first_assignment);
        }
        for i in 3..6 {
            assert_eq!(r.assignments[i], other);
        }
    }
