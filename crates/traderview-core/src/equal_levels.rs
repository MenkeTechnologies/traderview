...
impl Default for EqualLevelsConfig {
    fn default() -> Self { Self { cluster_tolerance: 0.10, min_cluster_size: 2 } }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum LevelKind { EqualHighs, EqualLows }
...
pub fn detect(swings: &[SwingPoint], cfg: &EqualLevelsConfig) -> LevelsReport {
    ...
    let highs: Vec<&SwingPoint> = swings.iter().filter(|s| matches!(s.kind, SwingKind::High)).collect();
    let lows:  Vec<&SwingPoint> = swings.iter().filter(|s| matches!(s.kind, SwingKind::Low)).collect();
    ...
    LevelsReport { clusters, n_equal_highs: n_high, n_equal_lows: n_low }
}
...
mod tests {
    ...
    fn min_cluster_size_three_filters_out_pairs() {
        let cfg = EqualLevelsConfig { cluster_tolerance: 0.10, min_cluster_size: 3 };
        let swings = vec![
    ...
    fn invalid_config_returns_empty() {
        ...
        let cfg = EqualLevelsConfig { cluster_tolerance: -1.0, min_cluster_size: 2 };
        let swings = vec![sp(5, 100.0, SwingKind::High), sp(20, 100.0, SwingKind::High)];
        let r = detect(&swings, &cfg);
        assert!(r.clusters.is_empty());
        // min_cluster_size < 2 also returns empty.
        let cfg2 = EqualLevelsConfig { cluster_tolerance: 0.10, min_cluster_size: 1 };
