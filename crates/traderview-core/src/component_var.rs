...
pub fn compute(
    ...
    let cvar: Vec<f64> = weights
        .iter()
        .zip(mvar.iter())
        .map(|(w, m)| w * m)
        .collect();
    let pct: Vec<f64> = cvar
        .iter()
        .map(|c| if var.abs() > 1e-18 { c / var } else { 0.0 })
        .collect();
...
mod tests {
    ...
    fn components_sum_to_portfolio_var() {
        ...
        assert!(
            (sum - r.portfolio_var).abs() < 1e-9,
            "components {} should sum to portfolio_var {}",
            sum,
            r.portfolio_var
        );
    }
    ...
    fn negative_weight_can_yield_negative_component_var() {
        ...
        let r = compute(&w, &cov, 0.95).unwrap();
        assert!(
            r.component_var[1] < 0.0,
            "short hedge CVaR should be negative, got {}",
            r.component_var[1]
        );
    }
}
