...
pub fn detect(bars: &[Bar]) -> Vec<PatternHit> {
    ...
    for i in 1..bars.len() {
        ...
        if prev.is_bear() && cur.is_bull() && cur.open <= prev.close && cur.close >= prev.open {
            out.push(PatternHit {
                bar_index: i,
        ...
        // Bearish engulfing.
        if prev.is_bull() && cur.is_bear() && cur.open >= prev.close && cur.close <= prev.open {
            out.push(PatternHit {
                bar_index: i,
...
mod tests {
    ...
    fn zero_range_bar_skipped() {
        ...
        assert!(!hits
            .iter()
            .any(|h| matches!(h.pattern, Pattern::Hammer | Pattern::ShootingStar)));
    }
}
