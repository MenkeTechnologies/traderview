...
pub fn compute(
    ...
    if !strike.is_finite()
        ...
        || !time_short_years.is_finite()
        || time_short_years <= 0.0
        || !time_long_years.is_finite()
        || time_long_years <= time_short_years
        || ![
            call_short_price,
            call_long_price,
            put_short_price,
    ...
    let market = (call_long_price - call_short_price) + (put_short_price - put_long_price);
...
mod tests {
    ...
    fn fair_jelly_roll_yields_zero_mispricing() {
        ...
        let put_long = 4.0;
        let result = compute(
            strike, r, t_s, t_l, call_short, call_long, put_short, put_long, 50.0,
        )
        .unwrap();
        assert!(
            result.mispricing.abs() < 1e-9,
            "fair jelly-roll should yield 0 mispricing, got {}",
            result.mispricing
        );
    ...
    fn overpriced_jelly_roll_flagged_arbitrage() {
        ...
        let put_long = 4.0;
        let result = compute(
            strike, r, t_s, t_l, call_short, call_long, put_short, put_long, 1.0,
        )
        .unwrap();
    ...
    fn arbitrage_threshold_honored() {
        ...
        let put_long = 4.0;
        let result = compute(
            strike, r, t_s, t_l, call_short, call_long, put_short, put_long, 100.0,
        )
        .unwrap();
        assert!(!result.is_arbitrage_opportunity);
    }
