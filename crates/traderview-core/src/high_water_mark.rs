...
pub fn compute(input: &PeriodInput, rates: &FeeRates) -> PeriodFee {
    ...
    let (incentive_fee, earned) = if excess > Decimal::ZERO {
        let pct =
            Decimal::from_str(&format!("{}", rates.incentive_fee_pct)).unwrap_or(Decimal::ZERO);
        (excess * pct, true)
    } else {
        (Decimal::ZERO, false)
    };
    ...
    // never down.
    let end_hwm = if net > input.start_hwm {
        net
    } else {
        input.start_hwm
    };
...
mod tests {
    ...
    fn winning_period_above_hwm_charges_incentive() {
        ...
        assert!(
            (to_f64(r.incentive_fee) - 1963.333333).abs() < 0.5,
            "got incentive_fee = {}",
            r.incentive_fee
        );
    ...
    fn hwm_advances_between_periods_and_incentive_compounds() {
        ...
        assert!(
            r1.end_hwm > d("107000") && r1.end_hwm < d("108000"),
            "HWM after p1 should be net equity ~107,853 but was {}",
            r1.end_hwm
        );
        let r2 = compute(
            &PeriodInput {
        ...
        assert!(
            (to_f64(r2.incentive_fee) - 2389.333333).abs() < 1.0,
            "got incentive_fee = {}",
            r2.incentive_fee
        );
        // HWM keeps climbing.
        assert!(
            r2.end_hwm > r1.end_hwm,
