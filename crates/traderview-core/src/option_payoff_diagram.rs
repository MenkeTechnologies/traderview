...
pub fn compute(legs: &[Leg], spot_min: f64, spot_max: f64, steps: usize) -> Option<Report> {
    if legs.is_empty() || steps < 2 { return None; }
    if !spot_min.is_finite() || !spot_max.is_finite() { return None; }
    if spot_min >= spot_max { return None; }
    for l in legs {
    ...
    for i in 1..points.len() {
        let a = &points[i - 1];
        let b = &points[i];
        if (a.pnl == 0.0) && !breakevens.contains(&a.spot) { breakevens.push(a.spot); }
    ...
    Some(Report { points, max_profit: max_p, max_loss: max_l, breakevens })
}
...
mod tests {
    ...
    fn long_put_breakeven_at_strike_minus_premium() {
        let legs = vec![Leg { kind: LegKind::Put, strike: 100.0, premium: 5.0, qty: 1.0 }];
    ...
    fn bull_call_spread_has_capped_profit_and_loss() {
        ...
        let legs = vec![
            ...
            Leg { kind: LegKind::Call, strike: 110.0, premium: 2.0, qty: -1.0 },
        ];
        let r = compute(&legs, 80.0, 130.0, 501).unwrap();
    ...
    fn straddle_has_two_breakevens() {
        ...
        let legs = vec![
            Leg { kind: LegKind::Call, strike: 100.0, premium: 5.0, qty: 1.0 },
            Leg { kind: LegKind::Put, strike: 100.0, premium: 5.0, qty: 1.0 },
        ];
