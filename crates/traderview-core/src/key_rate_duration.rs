...
pub fn compute(
    ...
    if tenors.iter().any(|t| !t.time_years.is_finite() || !t.spot_rate.is_finite()
        || t.time_years < 0.0) { return None; }
    ...
    Some(KrdReport { present_value: pv0, krd_per_tenor: krds, total_duration: total })
}
...
fn interp_rate(t: f64, tenors: &[KeyTenor]) -> Option<f64> {
    if tenors.is_empty() { return None; }
    if t <= tenors[0].time_years { return Some(tenors[0].spot_rate); }
...
mod tests {
    ...
    fn flat_curve(rate: f64) -> Vec<KeyTenor> {
        vec![
            KeyTenor { time_years: 0.5, spot_rate: rate },
            KeyTenor { time_years: 2.0, spot_rate: rate },
            KeyTenor { time_years: 5.0, spot_rate: rate },
            KeyTenor { time_years: 10.0, spot_rate: rate },
        ]
    }
    ...
    fn empty_or_invalid_returns_none() {
        ...
        assert!(compute(&cf, &[KeyTenor { time_years: 1.0, spot_rate: 0.05 }], 1.0).is_none());
    ...
    fn non_monotonic_tenors_rejected() {
        let bad = vec![
            KeyTenor { time_years: 5.0, spot_rate: 0.05 },
            KeyTenor { time_years: 2.0, spot_rate: 0.05 },
        ];
        let cf = ten_year_bond();
    ...
    fn nan_tenor_rejected() {
        let bad = vec![
            KeyTenor { time_years: 0.5, spot_rate: f64::NAN },
            KeyTenor { time_years: 2.0, spot_rate: 0.05 },
        ];
        let cf = ten_year_bond();
        assert!(compute(&cf, &bad, 1.0).is_none());
    ...
    fn total_duration_approximates_macaulay() {
        ...
        assert!(r.total_duration > 7.0 && r.total_duration < 9.0,
            "total duration {} outside expected ~8y for 10y bond", r.total_duration);
    }
    ...
    fn all_krds_non_negative_for_vanilla_bond() {
        ...
        for (i, k) in r.krd_per_tenor.iter().enumerate() {
            assert!(*k >= -1e-6, "KRD[{i}] = {k} unexpectedly negative for vanilla bond");
        }
    }
    ...
    fn longest_tenor_dominates_for_10y_bond() {
        ...
        let cf = ten_year_bond();
        let r = compute(&cf, &flat_curve(0.05), 1.0).unwrap();
        let max_idx = r.krd_per_tenor.iter().enumerate()
            .max_by(|a, b| a.1.partial_cmp(b.1).unwrap())
            .map(|(i, _)| i).unwrap();
        assert_eq!(max_idx, 3, "10y bond should have highest KRD at 10y key tenor");
    }

    #[test]
    fn zero_coupon_concentrates_krd() {
        let cf = vec![CashFlow { time_years: 5.0, amount: 100.0 }];
        let r = compute(&cf, &flat_curve(0.05), 1.0).unwrap();
