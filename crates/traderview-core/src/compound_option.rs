...
pub fn price(
    ...
    if !spot.is_finite()
        || spot <= 0.0
        || !strike_outer.is_finite()
        || strike_outer <= 0.0
        || !strike_inner.is_finite()
        || strike_inner <= 0.0
        || !t1.is_finite()
        || t1 <= 0.0
        || !t2.is_finite()
...
fn bivar_norm_cdf(h: f64, k: f64, rho: f64) -> f64 {
    ...
    let xs = [0.04691008, 0.23076534, 0.5, 0.76923466, 0.95308992];
    let ws = [
        0.018854042,
        0.038088059,
        0.045270692,
...
mod tests {
    ...
    fn compound_call_cheaper_than_inner_call() {
        ...
        assert!(
            coc.price <= inner_call,
            "CoC ({}) should be ≤ inner call ({})",
            coc.price,
            inner_call
        );
    }
