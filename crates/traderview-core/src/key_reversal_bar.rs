...
pub fn detect(bars: &[Bar], cfg: &Config) -> Vec<KeyReversalEvent> {
    let n = bars.len();
    let mut out = Vec::new();
    if n < cfg.prior_trend_lookback + 2 || cfg.prior_trend_lookback < 1 {
        return out;
    }
    if bars.iter().any(|b| {
    ...
    for i in (cfg.prior_trend_lookback + 1)..n {
        ...
        let downtrend =
            (1..=cfg.prior_trend_lookback).all(|k| bars[i - k].low < bars[i - k - 1].low);
        let uptrend =
            (1..=cfg.prior_trend_lookback).all(|k| bars[i - k].high > bars[i - k - 1].high);
        if downtrend && cur.open < prev.close && cur.low < prev.low && cur.close > prev.high {
        ...
        } else if uptrend && cur.open > prev.close && cur.high > prev.high && cur.close < prev.low {
            out.push(KeyReversalEvent {
