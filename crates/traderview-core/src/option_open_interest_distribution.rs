...
pub fn compute(strikes: &[StrikeOi]) -> Option<OptionOiReport> {
    ...
    if strikes.iter().any(|s| !s.strike.is_finite() || s.strike <= 0.0
        || !s.call_oi.is_finite() || s.call_oi < 0.0
        || !s.put_oi.is_finite() || s.put_oi < 0.0) {
        return None;
    }
    ...
    let per_strike: Vec<StrikeSummary> = sorted.iter().map(|s| {
        let total = s.call_oi + s.put_oi;
        let cp_ratio = if s.put_oi > 0.0 { s.call_oi / s.put_oi } else { f64::INFINITY };
        StrikeSummary {
            strike: s.strike,
            total_oi: total,
    ...
    let weighted_avg = if total_all > 0.0 {
        sorted.iter().map(|s| s.strike * (s.call_oi + s.put_oi)).sum::<f64>() / total_all
    } else { 0.0 };
    let mut by_total: Vec<&StrikeSummary> = per_strike.iter().collect();
    by_total.sort_by(|a, b| b.total_oi.partial_cmp(&a.total_oi)
...
mod tests {
    ...
    fn s(strike: f64, call: f64, put: f64) -> StrikeOi {
        StrikeOi { strike, call_oi: call, put_oi: put }
    }
