...
pub struct Bar {
    pub high: f64, pub low: f64, pub close: f64,
}
...
pub enum VortexTrend { Up, Down, Crossover, Neutral }
...
pub fn classify(prev: VortexPoint, cur: VortexPoint) -> VortexTrend {
    ...
    if was_up != is_up { return VortexTrend::Crossover; }
    if is_up { VortexTrend::Up } else { VortexTrend::Down }
}
...
mod tests {
    ...
    fn under_period_zeros() {
        ...
        for p in &out { assert_eq!(p.vi_plus, 0.0); }
    }
    ...
    fn crossover_detected() {
        let prev = VortexPoint { vi_plus: 0.6, vi_minus: 0.8 };
        let cur  = VortexPoint { vi_plus: 0.9, vi_minus: 0.7 };
        assert_eq!(classify(prev, cur), VortexTrend::Crossover);
    }

    #[test]
    fn up_when_vi_plus_stays_above_vi_minus() {
        let prev = VortexPoint { vi_plus: 0.9, vi_minus: 0.7 };
        let cur  = VortexPoint { vi_plus: 1.0, vi_minus: 0.6 };
        assert_eq!(classify(prev, cur), VortexTrend::Up);
    }

    #[test]
    fn down_when_vi_minus_stays_above_vi_plus() {
        let prev = VortexPoint { vi_plus: 0.5, vi_minus: 0.9 };
        let cur  = VortexPoint { vi_plus: 0.4, vi_minus: 1.0 };
