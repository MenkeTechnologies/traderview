...
pub fn test(values: &[f64], threshold: f64) -> Option<RunsTestReport> {
    if values.is_empty() || !threshold.is_finite() {
        return None;
    }
...
mod tests {
    ...
    fn alternating_pattern_yields_low_p_value() {
        ...
        assert!(
            r.reject_at_5pct,
            "alternating pattern should reject randomness, z = {}",
            r.z_statistic
        );
    ...
    fn clustered_pattern_yields_low_p_value() {
        ...
        assert!(
            r.reject_at_5pct,
            "clustered pattern should reject randomness, z = {}",
            r.z_statistic
        );
    }
    ...
    fn random_sequence_does_not_reject() {
        ...
        assert!(
            !r.reject_at_5pct,
            "iid sequence shouldn't reject, z = {}",
            r.z_statistic
        );
    }
