...
mod tests {
    ...
    fn accelerating_uptrend_stc_at_or_above_50() {
        ...
        let closes: Vec<f64> = (1..=80).map(|i| 100.0 + (i as f64).powi(2) * 0.5).collect();
        let out = compute(&closes, 23, 50, 10);
        assert!(
            out[79] >= 50.0,
            "accelerating uptrend should yield STC ≥ 50, got {}",
            out[79]
        );
    }
    ...
    fn accelerating_downtrend_stc_at_or_below_50() {
        let closes: Vec<f64> = (1..=80)
            .map(|i| 2000.0 - (i as f64).powi(2) * 0.5)
            .collect();
        let out = compute(&closes, 23, 50, 10);
        assert!(out[79] <= 50.0);
    }
    ...
    fn flat_series_stc_50() {
        ...
        // After full warmup.
        assert!(
            (out[79] - 50.0).abs() < 0.01,
            "flat series should produce STC ≈ 50, got {}",
            out[79]
        );
    }
