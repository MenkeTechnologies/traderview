...
fn is_bullish_mat_hold(b1: Bar, b2: Bar, b3: Bar, b4: Bar, b5: Bar) -> bool {
    ...
    if range2 <= 0.0 || body2 <= 0.0 || body2 > 0.5 * range2 {
        return false;
    }
    if b2.low <= b1.close {
        return false;
    ...
    if !(small_close_above(b3, b1.close) && small_close_above(b4, b1.close)) {
        return false;
    }
...
fn is_bearish_mat_hold(b1: Bar, b2: Bar, b3: Bar, b4: Bar, b5: Bar) -> bool {
    ...
    if !(small_close_below(b3, b1.close) && small_close_below(b4, b1.close)) {
        return false;
    }
