...
#[derive(Debug, Clone, Copy, Serialize, Deserialize)]
pub struct FixedFractionalParams { pub risk_pct: f64 }

#[derive(Debug, Clone, Copy, Serialize, Deserialize)]
pub struct RBasedParams { pub risk_dollars: f64 }
...
pub fn fixed_fractional(i: Inputs, p: FixedFractionalParams) -> Option<Sizing> {
    ...
    if rps <= 0.0 || i.equity <= 0.0 { return None; }
    ...
    Some(finalize("fixed_fractional", i, rps, raw,
        format!("risking {:.2}% of equity (${:.2})", p.risk_pct * 100.0, dollar_risk)))
}

pub fn r_based(i: Inputs, p: RBasedParams) -> Option<Sizing> {
    let rps = risk_per_share(i.side, i.entry, i.stop);
    if rps <= 0.0 || p.risk_dollars <= 0.0 { return None; }
    let raw = (p.risk_dollars / rps).floor() as i64;
    Some(finalize("r_based", i, rps, raw,
...
pub fn kelly(i: Inputs, p: KellyParams) -> Option<Sizing> {
    let rps = risk_per_share(i.side, i.entry, i.stop);
    if rps <= 0.0 || i.equity <= 0.0 { return None; }
    if !(0.0..=1.0).contains(&p.win_rate) { return None; }
    ...
    let note = if f_star <= 0.0 {
        ...
        format!("Kelly f* = {:.3}, applying {:.0}%-Kelly → risking ${:.2}",
                f_star, p.fractional_kelly * 100.0, dollar_risk)
    };
    Some(finalize("kelly", i, rps, raw, note))
}

fn finalize(method: &'static str, i: Inputs, rps: f64, raw_shares: i64, mut note: String) -> Sizing {
    ...
    let pct          = if i.equity > 0.0 { notional / i.equity } else { 0.0 };
    Sizing {
        method, shares, risk_per_share: rps,
...
pub fn report(
    ...
    Report {
        inputs: i, risk_per_share: rps,
        fixed_fractional, r_based, kelly, recommended,
        warnings,
    }
}
...
mod tests {
    ...
    fn fixed_fractional_long_basic() {
        ...
        // shares = 500.
        let i = Inputs { side: Side::Long, entry: 50.0, stop: 48.0,
                         equity: 100_000.0, correlation_drag: 0.0,
    ...
    fn correlation_drag_halves_size_at_correlation_one() {
        ...
        let i = Inputs { side: Side::Long, entry: 50.0, stop: 48.0,
                         equity: 100_000.0, correlation_drag: 1.0,
                         max_position_pct: 0.0 };
        let s = fixed_fractional(i, FixedFractionalParams { risk_pct: 0.01 }).unwrap();
    ...
    fn kelly_no_edge_returns_zero() {
        ...
        let i = Inputs { side: Side::Long, entry: 10.0, stop: 9.0,
                         equity: 10_000.0, correlation_drag: 0.0,
                         max_position_pct: 0.0 };
        let k = KellyParams { win_rate: 0.4, avg_win: 1.0, avg_loss: 1.0,
    ...
    fn kelly_with_edge_sizes_positive() {
        ...
        let i = Inputs { side: Side::Long, entry: 10.0, stop: 9.0,
                         equity: 10_000.0, correlation_drag: 0.0,
                         max_position_pct: 0.0 };
        let k = KellyParams { win_rate: 0.6, avg_win: 2.0, avg_loss: 1.0,
