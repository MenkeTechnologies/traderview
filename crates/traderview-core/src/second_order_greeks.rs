...
pub fn compute(
    ...
    let charm_common =
        exp_q_t * pdf_d1 * (2.0 * (r - q) * t - d2 * sigma * sqrt_t) / (2.0 * t * sigma * sqrt_t);
    let nd1 = norm_cdf(d1);
    ...
    let veta =
        -s * exp_q_t * pdf_d1 * (q + (r - q) * d1 / (sigma * sqrt_t) - (1.0 + d1 * d2) / (2.0 * t));
    let g = Greeks2 {
        vanna,
        charm,
        vomma,
    ...
    if [g.vanna, g.charm, g.vomma, g.veta]
        .iter()
        .all(|x| x.is_finite())
    {
        Some(g)
...
mod tests {
    ...
    fn invalid_inputs_return_none() {
        for bad in [f64::NAN, f64::INFINITY, 0.0, -1.0] {
            assert!(
                compute(bad, 100.0, 0.25, 0.05, 0.0, 0.20, OptionKind::Call).is_none(),
                "s={bad}"
            );
            assert!(
                compute(100.0, bad, 0.25, 0.05, 0.0, 0.20, OptionKind::Call).is_none(),
                "k={bad}"
            );
            assert!(
                compute(100.0, 100.0, bad, 0.05, 0.0, 0.20, OptionKind::Call).is_none(),
                "t={bad}"
            );
            assert!(
                compute(100.0, 100.0, 0.25, 0.05, 0.0, bad, OptionKind::Call).is_none(),
                "sigma={bad}"
            );
        }
    }
    ...
    fn deep_otm_call_vanna_positive() {
        ...
        assert!(
            g.vanna > 0.0,
            "OTM call vanna should be positive, got {}",
            g.vanna
        );
    }
