...
pub fn compute(
    ...
    if equity_curve.len() < 2 || period_returns.is_empty()
    ...
    // Annualized return.
    let clean_rets: Vec<f64> = period_returns.iter().copied()
        .filter(|x| x.is_finite()).collect();
    if clean_rets.is_empty() { return None; }
    ...
    let ratio = if pain > 0.0 { excess / pain }
        else if excess.abs() < 1e-12 { 0.0 }
        else { f64::INFINITY * excess.signum() };
    Some(PainReport {
        pain_index: pain,
        pain_ratio: ratio,
