...
pub fn compute(series: &[SymbolReturns], window: usize) -> Option<CorrelationReport> {
    ...
    for end in (window - 1)..t {
        ...
        for i in 0..n_sym {
            for j in (i + 1)..n_sym {
                if !corr[i][j].is_finite() { all_valid = false; break; }
                sum += corr[i][j];
                count += 1;
            }
            if !all_valid { break; }
...
fn pearson(a: &[f64], b: &[f64]) -> Option<f64> {
    ...
    let denom = ((sxx - sx * sx / n_f) * (syy - sy * sy / n_f)).max(0.0).sqrt();
...
mod tests {
    ...
    fn s(sym: &str, r: Vec<f64>) -> SymbolReturns {
        SymbolReturns { symbol: sym.into(), returns: r }
    }
    ...
    fn perfectly_correlated_pair_yields_one() {
        ...
        assert!((c - 1.0).abs() < 1e-9, "linear A↔B should have corr=1, got {c}");
    }
    ...
    fn nan_returns_skipped_per_pair() {
        ...
        let r = compute(&[
            s("A", a),
            s("B", (0..50).map(|i| i as f64).collect()),
        ], 30).unwrap();
