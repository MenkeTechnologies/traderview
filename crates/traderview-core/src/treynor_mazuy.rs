...
mod tests {
    ...
    fn perfect_market_tracker_yields_zero_gamma() {
        ...
        assert!((report.beta - 1.0).abs() < 0.01,
            "beta should be ≈ 1.0, got {}", report.beta);
        assert!(report.gamma.abs() < 0.05,
            "gamma should be ≈ 0 for pure tracker, got {}", report.gamma);
    }
    ...
    fn convex_timing_strategy_yields_positive_gamma() {
        ...
        assert!(report.gamma > 1.0,
            "expected large positive gamma, got {}", report.gamma);
        assert!(report.gamma_tstat > 2.0,
            "expected significant t-stat, got {}", report.gamma_tstat);
    }
    ...
    fn anti_timing_strategy_yields_negative_gamma() {
        ...
        let report = analyze(&p, &m, &rf).unwrap();
        assert!(report.gamma < -1.0, "expected negative gamma, got {}", report.gamma);
    }
