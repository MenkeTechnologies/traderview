...
pub fn price(
    spot: f64, strike_trigger: f64, strike_settlement: f64,
    time_to_expiry: f64,
    ...
    if !spot.is_finite() || spot <= 0.0
        || !strike_trigger.is_finite() || strike_trigger <= 0.0
        || !strike_settlement.is_finite() || strike_settlement <= 0.0
...
mod tests {
    ...
    fn gap_option_can_be_negative_with_high_settlement() {
        ...
        assert!(r.price < 0.0, "expected negative gap call price, got {}", r.price);
    }
    ...
    fn put_call_parity_extension_holds() {
        ...
        let s = 100.0; let k_trig = 105.0; let k_set = 100.0;
