...
pub enum ButterflyDirection { #[default] Bullish, Bearish }
...
pub fn detect(pivots: &[Pivot], tolerance: f64) -> Vec<ButterflyMatch> {
    ...
    for w in pivots.windows(5) {
        ...
        let cd_ratio = cd / bc;
        let ad_ratio = ad / xa;
        if (ab_ratio - 0.786).abs() > tolerance { continue; }
        ...
        if !(1.272 - tolerance..=1.618 + tolerance).contains(&ad_ratio) { continue; }
        let direction = if x.is_high { ButterflyDirection::Bearish }
            else { ButterflyDirection::Bullish };
