...
pub enum VolBucket { LowVolDecile, HighVolDecile, Neutral }
...
pub fn scan(symbols: &[SymbolPriceHistory], lookback_days: usize) -> Option<LowVolFactorReport> {
    if symbols.is_empty() || lookback_days < 5 { return None; }
    ...
    for s in symbols {
        ...
        if rets.len() < 5 { continue; }
        let m: f64 = rets.iter().sum::<f64>() / rets.len() as f64;
        let var: f64 = rets.iter().map(|r| (r - m).powi(2)).sum::<f64>()
            / (rets.len() as f64 - 1.0);
    ...
    let n = vols.len();
    let mut all_ranked: Vec<LowVolHit> = vols.iter().enumerate()
        .map(|(i, (sym, v))| {
            let pct = (i + 1) as f64 / n as f64 * 100.0;
            let bucket = if pct <= 10.0 { VolBucket::LowVolDecile }
                else if pct >= 90.0 { VolBucket::HighVolDecile }
                else { VolBucket::Neutral };
            LowVolHit {
                symbol: sym.clone(),
        ...
        .collect();
    // Lowest vol first.
    let low_vol_decile: Vec<LowVolHit> = all_ranked.iter()
        .filter(|h| h.bucket == VolBucket::LowVolDecile).cloned().collect();
    let high_vol_decile: Vec<LowVolHit> = all_ranked.iter()
        .filter(|h| h.bucket == VolBucket::HighVolDecile).cloned().collect();
    // Sort all_ranked descending by vol (high-to-low) for display convention.
    all_ranked.sort_by(|a, b| b.annualized_volatility.partial_cmp(&a.annualized_volatility)
        .unwrap_or(std::cmp::Ordering::Equal));
    Some(LowVolFactorReport { low_vol_decile, high_vol_decile, all_ranked })
}
...
mod tests {
    ...
    fn s(sym: &str, closes: Vec<f64>) -> SymbolPriceHistory {
        SymbolPriceHistory { symbol: sym.into(), daily_closes: closes }
    }
