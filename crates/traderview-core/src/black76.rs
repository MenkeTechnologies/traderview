...
pub fn price(
    ...
    let (price, delta) = match kind {
        OptionKind::Call => (dr * (forward * nd1 - strike * nd2), dr * nd1),
        OptionKind::Put => (
    ...
    let theta = match kind {
        OptionKind::Call => {
            -dr * forward * pdf_d1 * sigma / (2.0 * sqrt_t)
    ...
    if [price, delta, gamma, vega, theta]
        .iter()
        .all(|x| x.is_finite())
    {
        Some(Black76Output {
...
mod tests {
    ...
    fn invalid_inputs_return_none() {
        for bad in [0.0, -1.0, f64::NAN, f64::INFINITY] {
            assert!(
                price(bad, 100.0, 0.25, 0.05, 0.2, OptionKind::Call).is_none(),
                "F={bad}"
            );
            assert!(
                price(100.0, bad, 0.25, 0.05, 0.2, OptionKind::Call).is_none(),
                "K={bad}"
            );
            assert!(
                price(100.0, 100.0, bad, 0.05, 0.2, OptionKind::Call).is_none(),
                "T={bad}"
            );
            assert!(
                price(100.0, 100.0, 0.25, 0.05, bad, OptionKind::Call).is_none(),
                "σ={bad}"
            );
        }
    }
    ...
    fn vega_matches_finite_difference() {
        ...
        assert!(
            (analytic - fd).abs() / analytic.abs() < 1e-4,
