...
pub fn compute(bars: &[Bar]) -> Vec<bool> {
    ...
    for i in 2..n {
        ...
        if b2.close >= b2.open { continue; }
        if b2.open <= b1.close { continue; }
        // Bar 3: bearish, opens above bar 2's open, closes below bar 2's
        // close, but still above bar 1's close (gap not yet filled).
        if b3.close >= b3.open { continue; }
        if b3.open <= b2.open { continue; }
        if b3.close >= b2.close { continue; }
        if b3.close <= b1.close { continue; }
        out[i] = true;
    }
    out
}
