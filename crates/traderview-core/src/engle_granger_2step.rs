...
fn augmented_dickey_fuller(series: &[f64], lags: usize) -> Option<f64> {
    let n = series.len();
    if n < lags + 5 { return None; }
    // Δε_t = γ · ε_{t-1} + Σ_{j=1..lags} φ_j · Δε_{t-j} + ν_t
    let mut delta = vec![0.0_f64; n];
    for i in 1..n { delta[i] = series[i] - series[i - 1]; }
    let p = 1 + lags;
    let start = lags + 1;
    let n_obs = n - start;
    if n_obs < p + 2 { return None; }
    let mut xtx = vec![vec![0.0_f64; p]; p];
    let mut xty = vec![0.0_f64; p];
    for t in start..n {
        let mut row = vec![0.0_f64; p];
        row[0] = series[t - 1];
        for j in 1..=lags { row[j] = delta[t - j]; }
        let y = delta[t];
        for j in 0..p {
            xty[j] += row[j] * y;
    ...
    for t in start..n {
        ...
        for j in 1..=lags { yhat += coef[j] * delta[t - j]; }
...
mod tests {
    ...
    fn cointegrated_series_reject_no_cointegration() {
        ...
        assert!((r.cointegrating_beta - 2.0).abs() < 0.2,
            "β ≈ 2.0 expected, got {}", r.cointegrating_beta);
        assert!(r.reject_no_cointegration_5pct,
            "ADF stat {}, expected to reject no-cointegration", r.adf_statistic);
    }
    ...
    fn independent_random_walks_do_not_reject() {
        ...
        let r = test(&y, &x, 2).unwrap();
        assert!(!r.reject_no_cointegration_1pct,
            "independent RWs shouldn't reject at 1%, ADF = {}", r.adf_statistic);
    }
