...
pub fn analyze(input: &StressInput) -> StressReport {
    ...
    let new_dte_years = ((input.legs.iter()
        .map(|l| l.days_to_expiry).fold(f64::INFINITY, f64::min)
        - input.time_decay_days) / 365.0).max(0.0);
        ...
        for &iv_shock in &input.iv_shocks_pct {
            ...
            for leg in &input.legs {
                ...
                let greeks = price_and_greeks(
                    leg.kind, shocked_spot, leg.strike, t_years, shocked_iv,
            ...
            grid.push(StressCell {
                price_shock_pct: p_shock, iv_shock_pct: iv_shock,
                pnl_dollars: pnl,
    ...
    let worst = grid.iter()
        .min_by(|a, b| a.pnl_dollars.partial_cmp(&b.pnl_dollars).unwrap_or(std::cmp::Ordering::Equal))
        .cloned()
        .unwrap_or_default();
    let best = grid.iter()
        .max_by(|a, b| a.pnl_dollars.partial_cmp(&b.pnl_dollars).unwrap_or(std::cmp::Ordering::Equal))
        .cloned()
        .unwrap_or_default();

    StressReport { grid, worst_case: worst, best_case: best }
}
...
mod tests {
    ...
    fn long_atm_call() -> OptionLeg {
        OptionLeg {
            symbol: "SPY".into(), kind: OptKind::Call,
            spot: 500.0, strike: 500.0, days_to_expiry: 30.0,
            implied_vol: 0.20, contracts: 1, multiplier: 100.0,
            entry_price: 5.0,
        }
    }
    ...
    fn long_call_makes_money_when_underlying_rallies() {
        ...
        let r = analyze(&flat_input(vec![long_atm_call()]));
        let up5 = r.grid.iter().find(|c| (c.price_shock_pct - 0.05).abs() < 1e-9 && c.iv_shock_pct == 0.0).unwrap();
        let dn5 = r.grid.iter().find(|c| (c.price_shock_pct + 0.05).abs() < 1e-9 && c.iv_shock_pct == 0.0).unwrap();
        assert!(up5.pnl_dollars > dn5.pnl_dollars,
    ...
    fn worst_and_best_case_are_extrema_of_the_grid() {
        let r = analyze(&flat_input(vec![long_atm_call()]));
        let min = r.grid.iter().map(|c| c.pnl_dollars).fold(f64::INFINITY, f64::min);
        let max = r.grid.iter().map(|c| c.pnl_dollars).fold(f64::NEG_INFINITY, f64::max);
    ...
    fn iv_crush_hurts_a_long_premium_position() {
        ...
        assert!(crushed.pnl_dollars < flat.pnl_dollars,
