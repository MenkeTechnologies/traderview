...
static TABLE: Lazy<HashMap<&'static str, &'static StateSoiRule>> = Lazy::new(|| {
    ...
    static RULES: &[StateSoiRule] = &[
        ...
        rule("CA", "California", StatewideFull, Some(2019), "Cal. Gov. Code § 12955 (SB 329)"),
        rule("CO", "Colorado", StatewideFull, Some(2020), "C.R.S. § 24-34-502.2"),
        rule(
            "CT",
            "Connecticut",
            StatewideFull,
        ...
        rule("DE", "Delaware", StatewideFull, Some(2020), "6 Del. C. § 4604"),
        rule("FL", "Florida", NoProtection, None, "no statewide statute"),
        rule("GA", "Georgia", NoProtection, None, "no statewide; Atlanta local ordinance"),
        ...
        rule("IL", "Illinois", StatewideFull, Some(2023), "775 ILCS 5/3-102 (statewide 2023)"),
        ...
        rule("LA", "Louisiana", NoProtection, None, "no statewide statute"),
        rule(
            "MA",
            "Massachusetts",
            StatewideFull,
        ...
        rule("MD", "Maryland", StatewideFull, Some(2020), "Md. Code Real Prop. § 20-705"),
        rule("ME", "Maine", StatewideFull, Some(2009), "5 M.R.S. § 4581-A"),
        rule("MI", "Michigan", NoProtection, None, "no statewide statute"),
        rule("MN", "Minnesota", StatewideFull, Some(2023), "Minn. Stat. § 363A.09"),
        rule("MO", "Missouri", NoProtection, None, "no statewide statute"),
        rule("MS", "Mississippi", NoProtection, None, "no statewide statute"),
        rule("MT", "Montana", NoProtection, None, "no statewide statute"),
        rule("NC", "North Carolina", NoProtection, None, "no statewide statute"),
        rule("ND", "North Dakota", NoProtection, None, "no statewide statute"),
        rule("NE", "Nebraska", NoProtection, None, "no statewide statute"),
        rule("NH", "New Hampshire", NoProtection, None, "no statewide statute"),
        rule("NJ", "New Jersey", StatewideFull, Some(2002), "N.J.S.A. § 10:5-12"),
        rule("NM", "New Mexico", StatewideFull, Some(2023), "NMSA § 28-1-7"),
        ...
        rule("OR", "Oregon", StatewideFull, Some(2014), "ORS § 659A.421 (first statewide post-2010)"),
        rule("PA", "Pennsylvania", NoProtection, None, "no statewide; Philadelphia + Pittsburgh local"),
        rule("RI", "Rhode Island", StatewideFull, Some(1996), "R.I.G.L. § 34-37-4.3"),
        rule("SC", "South Carolina", NoProtection, None, "no statewide statute"),
        rule("SD", "South Dakota", NoProtection, None, "no statewide statute"),
        rule("TN", "Tennessee", NoProtection, None, "no statewide; Memphis local"),
        rule("TX", "Texas", NoProtection, None, "no statewide; Austin local ordinance"),
        rule(
            "UT",
            "Utah",
            StatewidePartialOrChallenged,
        ...
        rule("VA", "Virginia", StatewideFull, Some(2020), "Va. Code § 36-96.3"),
        rule("VT", "Vermont", StatewideFull, Some(1989), "9 V.S.A. § 4503"),
        rule("WA", "Washington", StatewideFull, Some(2018), "RCW § 59.18.255"),
        rule("WI", "Wisconsin", NoProtection, None, "no statewide; Madison + Dane County local"),
        rule("WV", "West Virginia", NoProtection, None, "no statewide statute"),
...
mod tests {
    ...
    fn local_only_states_flagged_in_citation() {
        ...
        for code in ["GA", "PA", "TN", "TX", "WI"] {
            ...
            assert!(r.citation.contains("local"), "{code} citation should mention local ordinance");
        }
    }
