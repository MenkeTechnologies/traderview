...
import { GLOBAL_ITEMS, EDITING_ITEMS, SYMBOL_ITEMS, ALL_SCOPED_ITEMS } from '../js/_context_menu.js';
...
test('every ctxmenu item actionKey has a window event handler', () => {
    const handlers = collectHandlers();
    const missing = [];
    for (const it of [...GLOBAL_ITEMS, ...EDITING_ITEMS, ...SYMBOL_ITEMS, ...SCOPED_ROW_ITEMS]) {
