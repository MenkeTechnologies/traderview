...
import { GLOBAL_ITEMS, EDITING_ITEMS, SYMBOL_ITEMS, TRADE_ROW_ITEMS, WATCHLIST_ROW_ITEMS, POSITION_ROW_ITEMS } from '../js/_context_menu.js';
...
test('every ctxmenu item actionKey has a window event handler', () => {
    const handlers = collectHandlers();
    const missing = [];
    for (const it of [...GLOBAL_ITEMS, ...EDITING_ITEMS, ...SYMBOL_ITEMS, ...TRADE_ROW_ITEMS, ...WATCHLIST_ROW_ITEMS, ...POSITION_ROW_ITEMS]) {
