...
// validateInputs routes through t() — the vitest setup-i18n.js preloads the
// en catalog, so we assert against the translated strings (not raw keys).

test('validate rejects too few returns', () => {
    expect(validateInputs([1, 2, 3], [2])).toMatch(/at least 10 returns/);
});
...
test('validate requires at least 2 chunk sizes', () => {
    expect(validateInputs(Array(20).fill(0.01), [10])).toMatch(/at least 2 chunk sizes/);
});

test('validate rejects non-integer / < 2 chunk sizes', () => {
    expect(validateInputs(Array(20).fill(0.01), [10, 1])).toMatch(/every chunk size/);
    expect(validateInputs(Array(20).fill(0.01), [10, 5.5])).toMatch(/every chunk size/);
});

test('validate rejects chunk sizes larger than series', () => {
    expect(validateInputs(Array(20).fill(0.01), [10, 25])).toMatch(/chunk sizes must be ≤ series length/);
});
