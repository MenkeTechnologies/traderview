...
import { GLOBAL_ITEMS, EDITING_ITEMS, SYMBOL_ITEMS, TRADE_ROW_ITEMS, SYMBOL_AWARE_SCOPES } from '../js/_context_menu.js';
...
test('every non-separator ctxmenu item has a labelKey', () => {
    const items = [...GLOBAL_ITEMS, ...EDITING_ITEMS, ...SYMBOL_ITEMS, ...TRADE_ROW_ITEMS].filter(it => it.kind !== 'separator');
...
test('every ctxmenu navTo target maps to a router case in app.js', () => {
    const items = [...GLOBAL_ITEMS, ...EDITING_ITEMS, ...SYMBOL_ITEMS, ...TRADE_ROW_ITEMS].filter(it => it.navTo);
