...
export function validateInputs(input) {
    if (!Array.isArray(input.closes))                       return t('view.bbpb.validate.closes_array');
    if (!Number.isInteger(input.period))                    return t('view.bbpb.validate.period_int');
    if (input.period < MIN_PERIOD || input.period > MAX_PERIOD)
                                                             return t('view.bbpb.validate.period_range', { min: MIN_PERIOD, max: MAX_PERIOD });
    if (!Number.isFinite(input.n_stdev) || input.n_stdev <= 0)
                                                             return t('view.bbpb.validate.n_stdev_pos');
    if (input.closes.length < input.period)                 return t('view.bbpb.validate.closes_min_period', { period: input.period });
    for (let i = 0; i < input.closes.length; i++) {
        if (!Number.isFinite(input.closes[i]))              return t('view.bbpb.validate.close_not_finite', { i });
    }
    return null;
}
