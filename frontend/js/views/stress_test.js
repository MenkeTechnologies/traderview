...
export async function renderStressTest(mount, _appState) {
    ...
    mount.innerHTML = `
        ...
        <div class="chart-panel">
            <h2 data-i18n="view.stress_test.h2.vega_chart">Portfolio vega ($) vs price shock (per IV shock)</h2>
            <div id="st-vega-chart" style="width:100%;height:220px"></div>
            <p data-i18n="view.stress_test.hint.vega_chart" class="muted small">Orthogonal to the P&L curve: how the portfolio's dollar vega shifts across the same shock grid. Reveals whether the position becomes more or less vol-sensitive in the tails — critical for sizing under a vol regime change.</p>
        </div>
...
function renderVegaChart(report) {
    const el = document.getElementById('st-vega-chart');
    if (!el || !window.uPlot) return;
    el.innerHTML = '';
    const grid = (report && report.grid) || [];
    if (grid.length < 1) {
        el.innerHTML = `<div class="muted" data-i18n="view.stress_test.empty_vega_chart">${esc(t('view.stress_test.empty_vega_chart'))}</div>`;
        return;
    }
    const matrix = pivotGrid(grid, state.priceShocks, state.ivShocks);
    const sortedIdx = state.priceShocks
    ...
    state.ivShocks.forEach((iv, ii) => {
        const ys = sortedIdx.map(([, pi]) => {
            const c = matrix[pi] && matrix[pi][ii];
            return c && Number.isFinite(Number(c.portfolio_vega_dollars)) ? Number(c.portfolio_vega_dollars) : null;
        });
        series.push({
            label: t('view.stress_test.chart.iv_label', { iv: (iv * 100).toFixed(0) + '%' }),
            stroke: palette[ii % palette.length],
