...
const SECTIONS = [
    ...
    },
    {
        ...
        body: `
            ...
            <dl class="tut-defs">
                ...
                <dd data-i18n-html="view.tutorial.live.live_scanner">6-panel real-time scanner (Gappers, Gainers, Losers, HOD, Volume, Ross 5-Pillar). Needs a free <a href="https://finnhub.io" target="_blank">Finnhub</a> API key + a comma-sep symbol universe (25 syms / WS connection, chunks automatically).</dd>
                <dt><button data-i18n="view.tutorial.btn.halts" class="link" data-go="halts">⏸ Halts</button></dt>
                <dd data-i18n-html="view.tutorial.live.halts">Polls Nasdaq trade-halt RSS every 3s. Voice alerts spell tickers letter-by-letter so TTS pronounces SPCE, not "space".</dd>
                <dt><button data-i18n="view.tutorial.btn.catalysts" class="link" data-go="catalysts">📰 Catalysts</button></dt>
                <dd data-i18n-html="view.tutorial.live.catalysts">SEC EDGAR (6s) + Business Wire / PR Newswire / GlobeNewswire / AccessWire (30s). Ticker NER extracts <code>$SYM</code>, <code>(SYM)</code>, and <code>NYSE:</code>/<code>NASDAQ:</code> prefixes. Voice fires when a watchlist symbol appears.</dd>
                <dt><button data-i18n="view.tutorial.btn.pre_market" class="link" data-go="premarket">🌅 Pre-market</button></dt>
                <dd data-i18n-html="view.tutorial.live.premarket">Cross-asset opening drive — index futures, commodities, crypto, FX, plus today's high-importance economic events. ATR-multiple shows how outsized the overnight move is.</dd>
            </dl>`,
    },
    {
        id: 'trading',
        ...
        body: `
            <dl class="tut-defs">
                ...
                <dd data-i18n-html="view.tutorial.trading.webull"><strong>Read-only.</strong> Order entry is intentionally not implemented. Open webull.com → DevTools → Network → copy <code>did</code>, <code>access_token</code>, <code>t_token</code> from any <code>tradeapi.webullbroker.com</code> request. Paste into the form. Tokens live in process memory only — never written to disk. Positions/orders/account poll every 5s.</dd>
                <dt><button data-i18n="view.tutorial.btn.live_positions" class="link" data-go="live">💰 Live Positions</button></dt>
                <dd data-i18n-html="view.tutorial.trading.live_positions">Open positions from imported trades, re-priced with fresh quotes. Shows unrealized P/L, day P/L, and total cost basis.</dd>
                <dt><button data-i18n="view.tutorial.btn.paper_trade" class="link" data-go="paper">📝 Paper Trade</button></dt>
                <dd data-i18n-html="view.tutorial.trading.paper">Simulated execution against the same quote pipeline as live. Order types: market, limit, stop, stop-limit. Reset starting cash anytime.</dd>
                <dt><button data-i18n="view.tutorial.btn.pre_trade_plans" class="link" data-go="plans">🎯 Pre-trade Plans</button></dt>
                <dd data-i18n-html="view.tutorial.trading.plans">Write the setup before you take the trade. Link a plan to an execution post-fill — the plan vs. outcome lands in your discipline scorecard.</dd>
                <dt><button data-i18n="view.tutorial.btn.risk_gate" class="link" data-go="risk-gate">🛡 Risk Gate</button></dt>
                <dd data-i18n-html="view.tutorial.trading.risk_gate">Pre-trade rules that BLOCK bad trades before they reach the broker. Define max loss per trade / per day, max consecutive losses, cool-down after a loss, max open positions, blocked symbols, require-plan, require-stop. Every new trade form runs these rules first and refuses to submit if any Block-severity rule fires (warnings get a confirm-then-proceed prompt).</dd>
                <dt><button data-i18n="view.tutorial.btn.position_size" class="link" data-go="sizing">🧮 Position Size</button></dt>
                <dd data-i18n-html="view.tutorial.trading.sizing">Three modes: Kelly fraction (from your historical win-rate), fixed-fractional (% of equity per trade), R-based (max loss in dollars / stop distance). Correlation-aware: caps concentration when symbols co-move.</dd>
            </dl>`,
    },
    {
        id: 'journal',
        ...
        body: `
            <dl class="tut-defs">
                ...
                <dd data-i18n-html="view.tutorial.journal.journal">Per-trade, daily, and general notes. Each scope has its own templates editable under <button data-i18n="view.tutorial.btn.settings" class="link" data-go="settings">Settings</button> → Notes Templates.</dd>
                <dt><button data-i18n="view.tutorial.btn.ai_journal" class="link" data-go="ai">🧠 AI Journal</button></dt>
                <dd data-i18n-html="view.tutorial.journal.ai">GPT-assisted post-mortem on individual trades. Requires an OpenAI key in <button data-i18n="view.tutorial.btn.ai_settings" class="link" data-go="ai">AI settings</button>. Output is cached so re-opening a trade is instant.</dd>
                <dt><button data-i18n="view.tutorial.btn.trade_reviews" class="link" data-go="reviews">🔁 Trade Reviews</button></dt>
                <dd data-i18n-html="view.tutorial.journal.reviews">Forced reflection on |R| ≥ 2 trades. Tracks plan-quality, execution-quality, emotion. Surfaces "reviews needed" count on the dashboard.</dd>
                <dt><button data-i18n="view.tutorial.btn.discipline" class="link" data-go="discipline">🛡 Discipline</button></dt>
                <dd data-i18n-html="view.tutorial.journal.discipline">Rule-violation tracker. Defines hard limits (max R per trade, max losses per day, no FOMO entries) and counts breaches.</dd>
                <dt><button data-i18n="view.tutorial.btn.mood_analytics" class="link" data-go="mood">🌡 Mood Analytics</button></dt>
                <dd data-i18n-html="view.tutorial.journal.mood">Self-rated mood entered with each daily journal correlates against same-day P/L. Surfaces "you trade worse when…" patterns.</dd>
            </dl>`,
    },
    {
        id: 'research',
        ...
        body: `
            <dl class="tut-defs">
                ...
                <dd data-i18n-html="view.tutorial.research.charts">OHLC + indicator overlays. Draw lines/arrows/rectangles; drawings persist per symbol per user.</dd>
                <dt><button data-i18n="view.tutorial.btn.research" class="link" data-go="research">🔎 Research</button></dt>
                <dd data-i18n-html="view.tutorial.research.research">Per-symbol dossier: quote, fundamentals, news, earnings history, recommendations, insider trades, dividends, holders.</dd>
                ...
                <dd data-i18n-html="view.tutorial.research.discovery">Three layers of discovery. Screener = manual criteria. Scanners = 24 Warrior/Zendoo presets (gap-and-go, opening drive, low-float surge…). Top Signals = ranked live signal leaderboard.</dd>
                ...
                <dd data-i18n-html="view.tutorial.research.dark_pool">Off-exchange % print rankings and FINRA Reg-SHO daily short volume. Use to flag squeezable setups.</dd>
                ...
                <dd data-i18n-html="view.tutorial.research.options">Full options chain with Greeks. Vol surface plots the IV grid across strikes × expiries. Earnings IV scans pre-event IV crush and computes straddle EV.</dd>
            </dl>`,
    },
    {
        id: 'reports',
    ...
    },
    {
        ...
        body: `
            <dl class="tut-defs">
                ...
                <dd data-i18n-html="view.tutorial.strategy.backtest">stryke-JIT backtester. Define entry/exit on any indicator combination. Save presets, fork community presets, share via slug.</dd>
                <dt><button data-i18n="view.tutorial.btn.walk_forward" class="link" data-go="walk-forward">🧱 Walk-forward</button></dt>
                <dd data-i18n-html="view.tutorial.strategy.walk_forward">Rolling in-sample / out-of-sample sweep. Reveals overfit parameters that look great in-sample but fall apart OOS.</dd>
                <dt><button data-i18n="view.tutorial.btn.indicators" class="link" data-go="custom-indicators">∇ Indicators</button></dt>
                <dd data-i18n-html="view.tutorial.strategy.indicators">Custom indicator editor. Define your own MA/RSI/ATR variants. Evaluable against any symbol from the Research view.</dd>
                ...
                <dd data-i18n-html="view.tutorial.strategy.alerts">Price/threshold alerts vs. compound AND/OR/NOT strategy rules. Both fire voice + Notification + optional webhook.</dd>
                <dt><button data-i18n="view.tutorial.btn.webhooks" class="link" data-go="webhooks">🪝 Webhooks</button></dt>
                <dd data-i18n-html="view.tutorial.strategy.webhooks">Outbound to Discord / Slack / generic URL. Use as a kill-switch trigger, mobile push, journal mirror, etc.</dd>
            </dl>`,
    },
    {
        id: 'data',
        ...
        body: `
            ...
            <ul class="tut-list">
                <li data-i18n-html="view.tutorial.data.quotes"><strong>Quotes / fundamentals / charts:</strong> Yahoo Finance v8/v10. Cached 60s per symbol in Postgres.</li>
                <li data-i18n-html="view.tutorial.data.live_ticks"><strong>Live ticks:</strong> Finnhub WebSocket. Free tier = 25 syms / connection; chunked automatically.</li>
                <li data-i18n-html="view.tutorial.data.halts"><strong>Halts:</strong> nasdaqtrader.com RSS, 3s poll.</li>
                <li data-i18n-html="view.tutorial.data.catalysts"><strong>Catalysts:</strong> SEC EDGAR Atom (6s), BusinessWire/PRNewswire/GlobeNewswire/AccessWire RSS (30s).</li>
                <li data-i18n-html="view.tutorial.data.short_interest"><strong>Short interest:</strong> FINRA Reg-SHO daily files.</li>
                <li data-i18n-html="view.tutorial.data.sentiment"><strong>Sentiment:</strong> Reddit r/wallstreetbets JSON, StockTwits public stream.</li>
                <li data-i18n-html="view.tutorial.data.crypto"><strong>Crypto:</strong> CoinGecko + blockchain.com.</li>
                <li data-i18n-html="view.tutorial.data.webull"><strong>Webull:</strong> session tokens you paste from your own browser. Read-only.</li>
            </ul>
        ...
        body: `
            ...
            <ol class="tut-steps">
                <li><strong>Install a preset</strong> from the <button data-i18n="view.tutorial.btn.risk_gate_2" class="link" data-go="risk-gate">Risk Gate</button> view. Three packs:
                    <ul class="tut-list">
                        <li data-i18n-html="view.tutorial.rg.beginner"><strong>Beginner</strong> — 7 rules. 1% max per trade, 3% per day, 3 consec losses stop you, 15-min cool-down, 25% max position, plan + stop required. Strictest.</li>
                        <li data-i18n-html="view.tutorial.rg.intermediate"><strong>Intermediate</strong> — 5 rules. Same per-trade cap, 5% daily, 4 streak, 5-min cool-down, stop required (plan optional).</li>
                        <li data-i18n-html="view.tutorial.rg.aggressive"><strong>Aggressive</strong> — 2 rules. Daily-loss cap + cool-down only. Assumes you manage per-trade risk yourself.</li>
                    </ul>
                </li>
                <li data-i18n-html="view.tutorial.rg.tune"><strong>Tune</strong> — toggle individual rules off via the checkbox column. Delete with the link. Add custom rules with the form below the list.</li>
                <li data-i18n-html="view.tutorial.rg.dry_run"><strong>Dry-run</strong> a hypothetical trade in the bottom panel to see exactly what the gate would say. Same call the new-trade form makes on submit.</li>
                <li data-i18n-html="view.tutorial.rg.compliance"><strong>Compliance snapshot</strong> at top — synthetic probe that surfaces which rules would fire on ANY entry right now (max daily loss already hit, in cool-down after the last loss, etc).</li>
                <li data-i18n-html="view.tutorial.rg.paper"><strong>Paper trades also gate.</strong> Paper trading is where you build the habit; if rules only fired live, you'd practice rule-breaking.</li>
                <li data-i18n-html="view.tutorial.rg.webhook"><strong>Webhook on Block.</strong> Every Block-severity veto fires to every enabled webhook (Discord / Slack / generic) so you get a public record of every rule save.</li>
            </ol>
    ...
    },
    {
        ...
        body: `
            <ul class="tut-list">
                <li data-i18n-html="view.tutorial.trouble.launch_fail"><strong>App fails to launch with "pg_ctl: another server might be running":</strong> An orphan Postgres from a hard kill. Cleanup runs automatically on next launch — just re-open. If it persists, kill any stray <code>postgres</code> process owned by you.</li>
                <li data-i18n-html="view.tutorial.trouble.loading_forever"><strong>Widgets show "loading…" forever:</strong> Check <code>~/Library/Application Support/traderview/traderview.log</code> — look for <code>pool timed out</code> (DB pool exhausted) or <code>ERROR</code> lines.</li>
                <li data-i18n-html="view.tutorial.trouble.ws_silent"><strong>WebSocket-driven views (Halts / Live Scanner / Catalysts / Webull) silent:</strong> The connect status dot (next to the view title) turns green on connect, red on error, gray on disconnected. Disconnected auto-reconnects in 4s.</li>
                <li data-i18n-html="view.tutorial.trouble.voice_silent"><strong>Voice alerts silent:</strong> Voice toggle is per-view. Browser SpeechSynthesis is throttled by macOS; if no voice fires for several events in a row, switch the system voice in System Settings → Accessibility → Spoken Content.</li>
                <li data-i18n-html="view.tutorial.trouble.stale"><strong>Premarket / markets data 60s stale:</strong> That's the in-process cache TTL. Yahoo throttles aggressive polling; restart to force-fresh.</li>
            </ul>`,
    },
];
...
export async function renderTutorial(mount, _state) {
    ...
    mount.innerHTML = `
        <h1 class="view-title"><span data-i18n="view.tutorial.title">// TUTORIAL</span>
            <input id="tut-q" type="search" data-shortcut="focus_search" placeholder="search…" data-i18n-placeholder="common.placeholder.search" autocomplete="off">
        </h1>
