...
export async function renderMarketProfile(mount, _appState) {
    ...
    mount.innerHTML = `
        <h1 class="view-title">// MARKET PROFILE · TPO HISTOGRAM</h1>

        <div class="chart-panel">
            <h2>Brackets (one per time period — typically 30-min RTH brackets)</h2>
            ...
            <div class="inline-form">
                <label>Tick size <input id="mp-tick" type="number" step="any" min="0" value="${state.tickSize}"></label>
                <button id="mp-demo" class="secondary" type="button">Load demo (13-bracket normal day)</button>
                <button id="mp-clear" class="secondary" type="button">Clear</button>
                <button id="mp-run" class="primary" type="button">Build TPO</button>
            </div>
        </div>
        ...
        <div class="chart-panel">
            ...
            <p class="muted">Yellow row = POC (most time). Cyan rows = Value Area (70%).
...
function renderSummary(r) {
    ...
    document.getElementById('mp-summary').innerHTML = [
        card('POC',           fmtN(r.poc_price), 'pos'),
        card('VAH',           fmtN(r.value_area_high)),
        card('VAL',           fmtN(r.value_area_low)),
        card('VA width',      fmtN(vaWidth)),
        card('Total TPOs',    fmtInt(r.total_tpos)),
        card('Price levels',  String((r.levels || []).length)),
        card('Single prints', String(counts.single), counts.single > 0 ? 'neg' : ''),
        card('Tick size',     fmtN(r.tick_size, 4)),
    ].join('');
}
