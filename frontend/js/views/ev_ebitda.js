...
export async function renderEvEbitda(mount, _appState) {
    ...
    mount.innerHTML = `
        ...
        <div class="chart-panel">
            ...
            <div id="evebitda-tools" class="ce-toolbar"></div>
            <button type="button" id="evebitda-sens-btn" class="ce-tool" data-i18n="calc.enh.sens.run">▦ Sensitivity</button>
            <div id="evebitda-sens" class="ce-sens"></div>
        </div>
    ...
    enh.mountToolbar(mount.querySelector('#evebitda-tools'), { viewId: VIEW, getInputs: () => lastBody || readBody(), getRows: () => reportRows(lastReport), filename: 'ev-ebitda.csv' });
    mount.querySelector('#evebitda-sens-btn').addEventListener('click', () => runSens(mount, readBody(), tok));
...
function reportRows(r) {
    ...
    return [
        ...
        ['ev_sales', r.ev_sales == null ? '' : r.ev_sales],
        ['ebitda_margin_pct', r.ebitda_margin_pct == null ? '' : r.ebitda_margin_pct],
        ['net_debt_usd', r.net_debt_usd],
    ];
}

async function renderResult(mount, r, body, tok) {
    const el = mount.querySelector('#evebitda-result');
    // Line chart: EV/EBITDA as EBITDA sweeps 0.5x -> 2x (EV fixed -> the multiple compresses).
    const base = body.ebitda_usd || 1;
    const xs = enh.linspace(base * 0.5, base * 2, 13);
    const pts = await Promise.all(xs.map(async (e) => {
        const rr = await api.calcEvEbitda({ ...body, ebitda_usd: e });
        return { x: e / 1000000, y: rr && rr.ev_ebitda != null ? rr.ev_ebitda : NaN };
    }));
    if (!viewIsCurrent(tok)) return;
    const chart = enh.svgLineChart(pts, { xlabel: 'EBITDA $M', ylabel: 'EV/EBITDA' });
...
async function runSens(mount, base, tok) {
    const panel = mount.querySelector('#evebitda-sens');
    panel.innerHTML = `<p class="muted small" data-i18n="calc.enh.sens.running">Computing…</p>`; applyUiI18n(panel);
    // x: EBITDA 0.5x -> 2x; y: total debt 0 -> 2x. Output: EV/EBITDA multiple.
    const eb = base.ebitda_usd || 1;
    const debt = base.total_debt_usd || 1000000;
    const xVals = enh.linspace(eb * 0.5, eb * 2, 5);
    const yVals = enh.linspace(0, debt * 2, 5);
    const { cells } = await enh.runSensitivity({ base, xKey: 'ebitda_usd', yKey: 'total_debt_usd', xVals, yVals, compute: (b) => api.calcEvEbitda(b), pick: (r) => (r ? r.ev_ebitda : null) });
    if (!viewIsCurrent(tok)) return;
    panel.innerHTML = enh.renderSensitivityTable({ xVals, yVals, cells, fmt: (v) => (v == null ? '—' : v.toFixed(1) + '×'), xfmt: (v) => '$' + (v / 1000000).toFixed(1) + 'M', yfmt: (v) => '$' + Math.round(v / 1000000) + 'M', xName: t('view.evebitda.label.ebitda') || 'EBITDA', yName: t('view.evebitda.label.debt') || 'Debt' });
}
