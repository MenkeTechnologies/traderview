...
export async function renderExports(mount, state) {
    ...
    mount.innerHTML = `
        ...
        <p class="muted small">CSV downloads use RFC 4180 quoting. The tax package opens a
        ...
        <div class="chart-panel">
            <h2>CSV downloads</h2>
            <table class="trades">
                <thead><tr><th>Dataset</th><th>Description</th><th>Action</th></tr></thead>
                <tbody>
                    <tr>
                        <td>Executions</td>
                        <td class="small muted">Every raw fill: timestamp, side, qty, price, fee, broker order id.</td>
                        <td><button class="btn" data-action="csv" data-path="/export/executions/${acct.id}.csv" data-name="executions-${acct.id}.csv">Download CSV</button></td>
                    </tr>
                    <tr>
                        <td>Trades</td>
                        <td class="small muted">Aggregated open→close positions with entry/exit avg, P/L, MFE/MAE, risk.</td>
                        <td><button class="btn" data-action="csv" data-path="/export/trades/${acct.id}.csv" data-name="trades-${acct.id}.csv">Download CSV</button></td>
                    </tr>
                </tbody>
            </table>
        </div>
        ...
        <div class="chart-panel">
            ...
            <table class="trades">
                <thead><tr><th>Output</th><th>Description</th><th>Action</th></tr></thead>
                <tbody>
                    <tr>
                        ...
                        <td><button class="btn" data-action="tax-csv" data-which="realized">Download CSV</button></td>
                    </tr>
                    <tr>
                        <td>Open lots (CSV)</td>
                        <td class="small muted">Year-end open inventory: per-lot qty, cost-per-share, basis, holding-period.</td>
                        <td><button class="btn" data-action="tax-csv" data-which="open">Download CSV</button></td>
                    </tr>
                    <tr>
                        ...
                        <td><button class="btn" data-action="tax-pkg">Open report</button></td>
                    </tr>
                </tbody>
            </table>
            <p class="muted small">Tax-package PDF: opens in a new tab, then ⌘P / Ctrl+P → "Save as PDF".</p>
        </div>
    `;
