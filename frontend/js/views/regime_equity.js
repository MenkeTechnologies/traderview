...
export async function renderRegimeEquity(mount, _appState) {
    ...
    mount.innerHTML = `
        <h1 class="view-title">// EQUITY REGIME</h1>

        <div class="chart-panel">
            <h2>Paste equity curve (one value per line, or CSV / whitespace-separated)</h2>
            <textarea id="re-blob" rows="6" placeholder="10000&#10;10100&#10;10250&#10;...">${esc(state.equity.join('\n'))}</textarea>
            <div class="inline-form">
                <label>Trend slope %  <small class="muted">(min |slope/mean_eq| to count as trending; default 0.001 = 0.1%)</small>
                    <input id="re-slope" type="number" step="any" min="0" value="${state.config.trend_slope_pct}"></label>
                <label>Clean rel stdev <small class="muted">(max residual/mean_eq for "clean" trend; default 0.02 = 2%)</small>
                    <input id="re-rsd"   type="number" step="any" min="0" value="${state.config.clean_trend_rel_stdev}"></label>
                <button id="re-run" class="primary" type="button">Analyze</button>
            </div>
            <div class="inline-form">
                <button id="re-demo-trend-up"   class="secondary" type="button">Demo: TRENDING UP</button>
                <button id="re-demo-trend-dn"   class="secondary" type="button">Demo: TRENDING DOWN</button>
                <button id="re-demo-vol-up"     class="secondary" type="button">Demo: VOLATILE UP</button>
                <button id="re-demo-vol-dn"     class="secondary" type="button">Demo: VOLATILE DOWN</button>
                <button id="re-demo-choppy"    class="secondary" type="button">Demo: CHOPPY</button>
                <button id="re-demo-realistic" class="secondary" type="button">Demo: realistic 90-day</button>
            </div>
        </div>
        ...
        <div class="chart-panel">
            <h2>Equity vs fitted trend line</h2>
            <div id="re-chart" style="height:320px"></div>
            <p class="muted">Cyan = your equity. Yellow = OLS regression fit. Spread of equity around the fit ⇒ residual stdev ⇒ trending vs volatile classification.</p>
        </div>
...
function renderSummary(r, pending) {
    ...
    document.getElementById('re-summary').innerHTML = [
        ...
        card('# samples',        String(r.n)),
        card('Slope / period',   fmtUSDSigned(r.slope_per_period, 2),
            r.slope_per_period >= 0 ? 'pos' : 'neg'),
        card('R²',               fmtNum(r.r_squared, 4),
            r.r_squared >= 0.5 ? 'pos' : ''),
        card('Residual stdev',   fmtUSD(r.residual_stdev, 2)),
        card('Mean equity',      fmtUSD(r.mean_equity ?? local.mean_equity, 2)),
        card('Total Δ',          fmtUSDSigned(state.equity[state.equity.length - 1] - state.equity[0], 2),
            state.equity[state.equity.length - 1] - state.equity[0] >= 0 ? 'pos' : 'neg'),
        card('Rel slope',        fmtPct((r.mean_equity ?? local.mean_equity) > 0
