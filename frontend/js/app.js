...
// Side-effect import: self-arms a MutationObserver that enhances every
// <table> in the DOM with click-to-sort + drag-to-resize columns.
import './table_enhance.js';
...
import { renderLeaseRenewal } from './views/lease_renewal.js';
...
import { renderPurchaseOrder } from './views/purchase_order.js';
import { renderSublease } from './views/sublease_agreement.js';
...
import { renderLlcOperatingAgreement } from './views/llc_operating_agreement.js';
...
import { renderLandContract } from './views/land_contract.js';
import { renderLeaseAssignment } from './views/lease_assignment.js';
...
import { renderSafe } from './views/safe_agreement.js';
...
import { renderFinalPaycheck } from './views/final_paycheck.js';
...
import { renderYearsToFi } from './views/years_to_fi.js';
...
import { renderTraditionalIraDeduction } from './views/traditional_ira_deduction.js';
...
import { renderSaasQuickRatio } from './views/saas_quick_ratio.js';
...
import { renderBondPricing } from './views/bond_pricing.js';
import { renderCashOutRefinance } from './views/cash_out_refinance.js';
...
export async function dispatch() {
    ...
    try {
        switch (view) {
            ...
            case 'lease-renewal': await renderLeaseRenewal(mount, state); break;
            ...
            case 'estimate': await renderEstimate(mount, state); break;
            case 'purchase-order': await renderPurchaseOrder(mount, state); break;
            case 'sublease': await renderSublease(mount, state); break;
            ...
            case 'llc-operating-agreement': await renderLlcOperatingAgreement(mount, state); break;
            ...
            case 'land-contract': await renderLandContract(mount, state); break;
            case 'lease-assignment': await renderLeaseAssignment(mount, state); break;
            ...
            case 'safe': await renderSafe(mount, state); break;
            ...
            case 'final-paycheck': await renderFinalPaycheck(mount, state); break;
            ...
            case 'years-to-fi': await renderYearsToFi(mount, state); break;
            ...
            case 'traditional-ira-deduction': await renderTraditionalIraDeduction(mount, state); break;
            ...
            case 'saas-quick-ratio': await renderSaasQuickRatio(mount, state); break;
            ...
            case 'bond-pricing': await renderBondPricing(mount, state); break;
            case 'cash-out-refinance': await renderCashOutRefinance(mount, state); break;
...
export const viewRenderers = {
    ...
    // Generated from the go() switch so every TILE that has a registered
    // route also mounts cleanly when added to a custom dashboard. Manually
    // editing entries here is fine — the autogen will not clobber existing
    // keys, only fill gaps. Sym/rest params get '' default (dashboard tile
    // has no URL context). See scripts comment block for the regen path.
    '529-roth': (m, s) => renderRollover529Roth(m, s),
    'able-account': (m, s) => renderAbleAccount(m, s),
    'accountable-plan': (m, s) => renderAccountablePlan(m, s),
    'accounts': (m, s) => renderAccounts(m, s, (() => {})),
    'accounts-overview': (m, s) => renderAccountsOverview(m, s),
    'after-hours': (m, s) => renderAfterHours(m, s),
    'ai': (m, s) => renderAiSettings(m, s),
    'alerts': (m, s) => renderAlerts(m, s),
    'algo': (m, s) => renderAlgo(m, s),
    'american-option': (m, s) => renderAmericanOption(m, s),
    'amt-calc': (m, s) => renderAmtCalc(m, s),
    'augusta-rule': (m, s) => renderAugustaRule(m, s),
    'backdoor-roth': (m, s) => renderBackdoorRoth(m, s),
    'backtest': (m, s) => renderBacktest(m, s),
    'backtest-presets': (m, s) => renderBacktestPresets(m, s, ''),
    'biz-categorizer': (m, s) => renderBizCategorizer(m, s),
    'boards': (m, s) => renderBoards(m, s, ''),
    'bocpd': (m, s) => renderBocpd(m, s),
    'bond-yield-curve': (m, s) => renderBondYieldCurve(m, s),
    'breadth': (m, s) => renderBreadth(m, s),
    'breadth-divergence': (m, s) => renderBreadthDivergence(m, s),
    'broker-compare': (m) => renderBrokerCompare(m),
    'brokers': (m) => renderBrokersManage(m),
    'budget': (m, s) => renderBudget(m, s),
    'business-compare': (m) => renderBusinessCompare(m),
    'businesses': (m) => renderBusinessesManage(m),
    'calendar': (m, s) => renderCalendar(m, s),
    'catalyst-correlations': (m, s) => renderCatalystCorrelations(m, s),
    'catalysts': (m, s) => renderCatalysts(m, s),
    'categorize': (m, s) => renderCategorize(m, s),
    'charitable-planner': (m, s) => renderCharitablePlanner(m, s),
    'charts': (m, s) => renderCharts(m, s, ''),
    'clean-energy-25d': (m, s) => renderCleanEnergy25D(m, s),
    'compare': (m, s) => renderCompare(m, s),
    'confluence': (m, s) => renderConfluence(m, s),
    'confluence-autotrade': (m, s) => renderConfluenceAutotrade(m, s),
    'congressional-trading': (m, s) => renderCongressionalTrading(m, s, ''),
    'conservation-easement': (m, s) => renderConservationEasement(m, s),
    'correlation': (m, s) => renderCorrMatrix(m, s),
    'cost-seg': (m, s) => renderCostSeg(m, s),
    'cov-denoiser': (m, s) => renderCovDenoiser(m, s),
    'crat': (m, s) => renderCrat(m, s),
    ...
    'daf': (m, s) => renderDaf(m, s),
    'darkpool': (m, s) => renderDarkpool(m, s, ''),
    'dashboard': (m, s) => renderDashboard(m, s),
    'dashboards': (m, s) => renderDashboards(m, s),
    'dca-simulator': (m, s) => renderDcaSimulator(m, s),
    'dcfsa': (m, s) => renderDcfsa(m, s),
    'defined-benefit': (m, s) => renderDefinedBenefit(m, s),
    'depreciation': (m, s) => renderDepreciation(m, s),
    'developer': (m, s) => renderDeveloper(m, s),
    'disabled-access': (m, s) => renderDisabledAccess(m, s),
    'discipline': (m, s) => renderDiscipline(m, s),
    'disclosures': (m, s) => renderDisclosures(m, s),
    'dividend-aristocrats': (m, s) => renderDividendAristocrats(m, s),
    'dividend-calendar': (m, s) => renderDividendCalendar(m, s),
    'dividend-capture': (m, s) => renderDividendCapture(m, s),
    'dividend-tracker': (m, s) => renderDividendTracker(m, s),
    'drawdown-cutoff': (m, s) => renderDrawdownCutoff(m, s),
    'dtw': (m, s) => renderDtw(m, s),
    'earnings-cal': (m, s) => renderEarningsCal(m, s),
    'earnings-call-live': (m, s) => renderEarningsCallLive(m, s),
    'earnings-iv': (m, s) => renderEarningsIv(m, s, ''),
    'earnings-quality': (m, s) => renderEarningsQuality(m, s, ''),
    'earnings-revisions': (m, s) => renderEarningsRevisions(m, s),
    'economic-calendar': (m, s) => renderEconomicCalendar(m, s),
    'economy': (m, s) => renderEconomy(m, s),
    'education-credits': (m, s) => renderEducationCredits(m, s),
    'esg': (m, s) => renderEsg(m, s, ''),
    'espp-calc': (m, s) => renderEsppCalc(m, s),
    'estimates-dashboard': (m, s) => renderEstimatesDashboard(m, s, ''),
    'etf-profile': (m, s) => renderEtfProfile(m, s, ''),
    'ev-credit': (m, s) => renderEvCredit(m, s),
    'execution-scheduler': (m, s) => renderExecutionScheduler(m, s),
    'expense-calendar': (m) => renderExpenseCalendar(m),
    'expense-dashboard': (m) => renderExpenseDashboard(m),
    'exports': (m, s) => renderExports(m, s),
    'fbar-8938': (m, s) => renderFbar8938(m, s),
    'fda-calendar': (m, s) => renderFdaCalendar(m, s),
    'fear-greed': (m, s) => renderFearGreed(m, s),
    'file-taxes': (m, s) => renderTaxWizard(m, s),
    'filings-browser': (m, s) => renderFilingsBrowser(m, s, ''),
    'fill-quality': (m, s) => renderFillQuality(m, s),
    'film-181': (m, s) => renderFilm181(m, s),
    'finnhub-aggregate': (m, s) => renderFinnhubAggregate(m, s, ''),
    'finnhub-pattern': (m, s) => renderFinnhubPattern(m, s, ''),
    'finnhub-search': (m, s) => renderFinnhubSearch(m, s),
    'finnhub-sr': (m, s) => renderFinnhubSr(m, s, ''),
    'forecast': (m, s) => renderEquityForecast(m, s),
    'foreign-tax-credit': (m, s) => renderForeignTaxCredit(m, s),
    'forex-988': (m, s) => renderForex988(m, s),
    'forex-rates': (m, s) => renderForexRates(m, s),
    'forward-vol': (m, s) => renderForwardVolCurve(m, s),
    'fx-option': (m, s) => renderFxOption(m, s),
    'gamma-squeeze': (m, s) => renderGammaSqueeze(m, s),
    'gift-tax': (m, s) => renderGiftTax(m, s),
    'goals': (m, s) => renderGoals(m, s),
    'grat': (m, s) => renderGrat(m, s),
    'halts': (m, s) => renderHalts(m, s),
    'heatmap': (m, s) => renderHeatmap(m, s),
    'historic-rehab': (m, s) => renderHistoricRehab(m, s),
    'historical-market-cap': (m, s) => renderHistoricalMarketCap(m, s, ''),
    'home-office': (m, s) => renderHomeOffice(m, s),
    'hotkeys': (m, s) => renderHotkeys(m, s),
    'hsa-max': (m, s) => renderHsaMax(m, s),
    'htb-ranker': (m, s) => renderHtbRanker(m, s),
    'hurst': (m, s) => renderHurst(m, s),
    'ilit': (m, s) => renderIlit(m, s),
    'import': (m, s) => renderImportView(m, s),
    'income-1099': (m, s) => renderIncome1099(m, s),
    'index-constituents': (m, s) => renderIndexConstituents(m, s),
    'inherited-ira-rmd': (m, s) => renderInheritedIraRmd(m, s),
    'insider-clusters': (m, s) => renderInsiderClusters(m, s),
    'insider-finnhub': (m, s) => renderInsiderTransactionsFinnhub(m, s, ''),
    'insider-sentiment': (m, s) => renderInsiderSentiment(m, s, ''),
    'insider-stream': (m, s) => renderInsiderStream(m, s),
    'installment-sale': (m, s) => renderInstallmentSale(m, s),
    'institutional-13f': (m, s) => renderInstitutional13F(m, s),
    'ipo-calendar': (m, s) => renderIpoCalendar(m, s),
    'ipo-lockups': (m, s) => renderIpoLockups(m, s),
    'iso-exercise': (m, s) => renderIsoExercise(m, s),
    'iv-solver': (m, s) => renderIvSolver(m, s),
    'iv-term': (m, s) => renderIvTerm(m, s),
    'journal': (m, s) => renderJournalView(m, s, ''),
    'kalman-beta': (m, s) => renderKalmanBeta(m, s),
    'kiddie-tax': (m, s) => renderKiddieTax(m, s),
    'lihtc': (m, s) => renderLihtc(m, s),
    'live': (m, s) => renderLivePositions(m, s),
    'live-scanner': (m, s) => renderLiveScanner(m, s),
    'lobbying': (m, s) => renderLobbying(m, s, ''),
    'log-viewer': (m) => renderLogViewer(m),
    'magic-formula': (m, s) => renderMagicFormula(m, s),
    'market-gamma': (m, s) => renderMarketGammaRegime(m, s),
    'market-status': (m, s) => renderMarketStatus(m, s),
    'meal-deduction': (m, s) => renderMealDeduction(m, s),
    'mega-backdoor-roth': (m, s) => renderMegaBackdoorRoth(m, s),
    'mentorship': (m, s) => renderMentorship(m, s),
    'microprice': (m, s) => renderMicroprice(m, s),
    'mileage-log': (m, s) => renderMileageLog(m, s),
    'mlp-k1': (m, s) => renderMlpK1(m, s),
    'monte-carlo': (m, s) => renderMonteCarlo(m, s),
    'mood': (m, s) => renderMoodAnalytics(m, s),
    'mtm-election': (m, s) => renderMtmElection(m, s),
    'multi-broker': (m, s) => renderMultiBroker(m, s),
    'multichart': (m, s) => renderMultichart(m, s, ''),
    'mutual-fund': (m, s) => renderMutualFund(m, s, ''),
    'new-trade': (m, s) => renderNewTrade(m, s),
    'news': (m, s) => renderNews(m, s),
    'news-sentiment': (m, s) => renderNewsSentiment(m, s, ''),
    'nol-tracker': (m, s) => renderNolTracker(m, s),
    'note-templates': (m) => renderNoteTemplates(m),
    'nso-exercise': (m, s) => renderNsoExercise(m, s),
    'nua-strategy': (m, s) => renderNuaStrategy(m, s),
    'optimal-f': (m, s) => renderOptimalF(m, s),
    'option-payoff': (m, s) => renderOptionPayoff(m, s),
    'options': (m, s) => renderOptions(m, s, ''),
    'pair-trade-calc': (m, s) => renderPairTrade(m, s),
    'pairs': (m, s) => renderPairs(m, s),
    'pairs-coint': (m, s) => renderPairsCoint(m, s),
    'paper': (m, s) => renderPaper(m, s),
    'paper-rebalance': (m, s) => renderPaperRebalance(m, s),
    'paper-tax-loss-harvest': (m, s) => renderPaperTaxLossHarvest(m, s),
    'partial-disposition': (m, s) => renderPartialDisposition(m, s),
    'passive-loss': (m, s) => renderPassiveLoss(m, s),
    'pattern-discovery': (m, s) => renderPatternDiscovery(m, s),
    'pead': (m, s) => renderPead(m, s),
    'plans': (m, s) => renderPlans(m, s),
    'portfolio-allocator': (m, s) => renderPortfolioAllocator(m, s),
    'portfolio-exposure': (m, s) => renderPortfolioExposure(m, s),
    'premarket': (m, s) => renderPremarket(m, s),
    'price-target': (m, s) => renderPriceTarget(m, s, ''),
    'purchases': (m, s) => renderPurchases(m, s),
    'qbi-199a': (m, s) => renderQbi199A(m, s),
    'qcd-tracker': (m, s) => renderQcdTracker(m, s),
    'qoz-tracker': (m, s) => renderQozTracker(m, s),
    'qsbs-1202': (m, s) => renderQsbs1202(m, s),
    'quarterly-tax': (m, s) => renderQuarterlyTax(m, s),
    'r-dist': (m, s) => renderRDist(m, s),
    'rd-credit': (m, s) => renderRdCredit(m, s),
    'rebalance': (m, s) => renderRebalance(m, s),
    'receipts': (m, s) => renderReceipts(m, s),
    'regime-detector': (m, s) => renderRegimeDetector(m, s),
    'replay': (m, s) => renderReplay(m, s, ''),
    'reports': (m, s) => renderReports(m, s, '' || 'overview'),
    'research': (m, s) => renderResearch(m, s, ''),
    'golden-stars': (m, s) => renderGoldenStars(m, s),
    ...
    'revenue-breakdown': (m, s) => renderRevenueBreakdown(m, s, ''),
    'reviews': (m, s) => renderTradeReviews(m, s),
    'risk': (m, s) => renderRisk(m, s),
    'risk-gate': (m, s) => renderRiskGate(m, s),
    'roth-ladder': (m, s) => renderRothLadder(m, s),
    'rr-butterfly': (m, s) => renderRrButterfly(m, s),
    'rsu-vest-tracker': (m, s) => renderRsuVestTracker(m, s),
    'rvol-accel': (m, s) => renderRvolAccel(m, s),
    'savers-credit': (m, s) => renderSaversCredit(m, s),
    'scanner-backtest': (m, s) => renderScannerBacktest(m, s),
    'scanners': (m, s) => renderScanners(m, s),
    'scorp-calc': (m, s) => renderScorpCalc(m, s),
    'screener': (m, s) => renderScreener(m, s),
    'se-health-deduction': (m, s) => renderSeHealthDeduction(m, s),
    'search': (m, s) => renderSearch(m, s),
    'sec-1256': (m, s) => renderSec1256(m, s),
    'section-1014': (m, s) => renderSection1014(m, s),
    'section-1015': (m, s) => renderSection1015(m, s),
    'section-102': (m, s) => renderSection102(m, s),
    'section-1031': (m, s) => renderSection1031(m, s),
    'section-1033': (m, s) => renderSection1033(m, s),
    'section-1035': (m, s) => renderSection1035(m, s),
    'section-1041': (m, s) => renderSection1041(m, s),
    'section-1042': (m, s) => renderSection1042(m, s),
    'section-105': (m, s) => renderSection105(m, s),
    'section-1058': (m, s) => renderSection1058(m, s),
    'section-1059': (m, s) => renderSection1059(m, s),
    'section-106': (m, s) => renderSection106(m, s),
    'section-1092': (m, s) => renderSection1092(m, s),
    'section-119': (m, s) => renderSection119(m, s),
    'section-1202': (m, s) => renderSection1202(m, s),
    'section-121': (m, s) => renderSection121(m, s),
    'section-1212': (m, s) => renderSection1212(m, s),
    'section-1231': (m, s) => renderSection1231(m, s),
    'section-1233': (m, s) => renderSection1233(m, s),
    'section-1234': (m, s) => renderSection1234(m, s),
    'section-1239': (m, s) => renderSection1239(m, s),
    'section-1244': (m, s) => renderSection1244(m, s),
    'section-1245': (m, s) => renderSection1245(m, s),
    'section-1245-1250': (m, s) => renderSection12451250(m, s),
    'section-1248': (m, s) => renderSection1248(m, s),
    'section-125': (m, s) => renderSection125(m, s),
    'section-1250': (m, s) => renderSection1250(m, s),
    'section-1259': (m, s) => renderSection1259(m, s),
    'section-127': (m, s) => renderSection127(m, s),
    'section-1273': (m, s) => renderSection1273(m, s),
    'section-1276': (m, s) => renderSection1276(m, s),
    'section-129': (m, s) => renderSection129(m, s),
    'section-1291': (m, s) => renderSection1291(m, s),
    'section-1295': (m, s) => renderSection1295(m, s),
    'section-1296': (m, s) => renderSection1296(m, s),
    'section-1296-pfic': (m, s) => renderSection1296Pfic(m, s),
    'section-1297': (m, s) => renderSection1297(m, s),
    'section-1298': (m, s) => renderSection1298(m, s),
    'section-132': (m, s) => renderSection132(m, s),
    'section-134': (m, s) => renderSection134(m, s),
    'section-1361': (m, s) => renderSection1361(m, s),
    'section-1362': (m, s) => renderSection1362(m, s),
    'section-1366': (m, s) => renderSection1366(m, s),
    'section-1368': (m, s) => renderSection1368(m, s),
    'section-1374': (m, s) => renderSection1374(m, s),
    'section-1377': (m, s) => renderSection1377(m, s),
    'section-1400z': (m, s) => renderSection1400z(m, s),
    'section-1402': (m, s) => renderSection1402(m, s),
    'section-1411': (m, s) => renderSection1411(m, s),
    'section-1445': (m, s) => renderSection1445(m, s),
    'section-1446f': (m, s) => renderSection1446F(m, s),
    'section-152': (m, s) => renderSection152(m, s),
    'section-162a1': (m, s) => renderSection162a1(m, s),
    'section-162c': (m, s) => renderSection162C(m, s),
    'section-162f': (m, s) => renderSection162F(m, s),
    'section-162l': (m, s) => renderSection162l(m, s),
    'section-162m': (m, s) => renderSection162m(m, s),
    'section-163j': (m, s) => renderSection163j(m, s),
    'section-164': (m, s) => renderSection164(m, s),
    'section-165': (m, s) => renderSection165(m, s),
    'section-165c3': (m, s) => renderSection165c3(m, s),
    'section-165d': (m, s) => renderSection165D(m, s),
    'section-165g': (m, s) => renderSection165g(m, s),
    'section-168': (m, s) => renderSection168(m, s),
    'section-168g': (m, s) => renderSection168g(m, s),
    'section-168k': (m, s) => renderSection168k(m, s),
    'section-170': (m, s) => renderSection170(m, s),
    'section-172': (m, s) => renderSection172(m, s),
    'section-174': (m, s) => renderSection174(m, s),
    'section-179': (m, s) => renderSection179(m, s),
    'section-179d': (m, s) => renderSection179d(m, s),
    'section-195': (m, s) => renderSection195(m, s),
    'section-197': (m, s) => renderSection197(m, s),
    'section-199a': (m, s) => renderSection199A(m, s),
    'section-2010c': (m, s) => renderSection2010c(m, s),
    'section-2032': (m, s) => renderSection2032(m, s),
    'section-2055': (m, s) => renderSection2055(m, s),
    'section-2056': (m, s) => renderSection2056(m, s),
    'section-21-cdcc': (m, s) => renderSection21Cdcc(m, s),
    'section-213': (m, s) => renderSection213(m, s),
    'section-219': (m, s) => renderSection219(m, s),
    'section-221': (m, s) => renderSection221(m, s),
    'section-23': (m, s) => renderSection23(m, s),
    'section-24': (m, s) => renderSection24(m, s),
    'section-24-ctc': (m, s) => renderSection24Ctc(m, s),
    'section-245a': (m, s) => renderSection245A(m, s),
    'section-248': (m, s) => renderSection248(m, s),
    'section-250': (m, s) => renderSection250(m, s),
    'section-2503': (m, s) => renderSection2503(m, s),
    'section-2518': (m, s) => renderSection2518(m, s),
    'section-25a': (m, s) => renderSection25a(m, s),
    'section-25c': (m, s) => renderSection25c(m, s),
    'section-25d': (m, s) => renderSection25D(m, s),
    'section-25e': (m, s) => renderSection25E(m, s),
    'section-263-tpr': (m, s) => renderSection263Tpr(m, s),
    'section-263a': (m, s) => renderSection263a(m, s),
    'section-263c': (m, s) => renderSection263C(m, s),
    'section-269': (m, s) => renderSection269(m, s),
    'section-269a': (m, s) => renderSection269A(m, s),
    'section-274': (m, s) => renderSection274(m, s),
    'section-280c': (m, s) => renderSection280C(m, s),
    'section-280e': (m, s) => renderSection280e(m, s),
    'section-280f': (m, s) => renderSection280f(m, s),
    'section-280g': (m, s) => renderSection280g(m, s),
    'section-302': (m, s) => renderSection302(m, s),
    'section-303': (m, s) => renderSection303(m, s),
    'section-304': (m, s) => renderSection304(m, s),
    'section-305': (m, s) => renderSection305(m, s),
    'section-30c': (m, s) => renderSection30C(m, s),
    'section-30d': (m, s) => renderSection30D(m, s),
    'section-311': (m, s) => renderSection311(m, s),
    'section-318': (m, s) => renderSection318(m, s),
    'section-32': (m, s) => renderSection32(m, s),
    'section-32-eic': (m, s) => renderSection32Eic(m, s),
    'section-332': (m, s) => renderSection332(m, s),
    'section-336': (m, s) => renderSection336(m, s),
    'section-338': (m, s) => renderSection338(m, s),
    'section-351': (m, s) => renderSection351(m, s),
    'section-351-721': (m, s) => renderSection351721(m, s),
    'section-355': (m, s) => renderSection355(m, s),
    'section-357': (m, s) => renderSection357(m, s),
    'section-362': (m, s) => renderSection362(m, s),
    'section-367a': (m, s) => renderSection367A(m, s),
    'section-367d': (m, s) => renderSection367D(m, s),
    'section-368': (m, s) => renderSection368(m, s),
    'section-36b': (m, s) => renderSection36b(m, s),
    'section-38': (m, s) => renderSection38(m, s),
    'section-382': (m, s) => renderSection382(m, s),
    'section-401a9': (m, s) => renderSection401a9(m, s),
    'section-401k-hardship': (m, s) => renderSection401kHardship(m, s),
    'section-408a': (m, s) => renderSection408A(m, s),
    'section-408d3': (m, s) => renderSection408d3(m, s),
    'section-409a': (m, s) => renderSection409A(m, s),
    'section-41': (m, s) => renderSection41(m, s),
    'section-412': (m, s) => renderSection412(m, s),
    'section-414': (m, s) => renderSection414(m, s),
    'section-414v': (m, s) => renderSection414v(m, s),
    'section-415': (m, s) => renderSection415(m, s),
    'section-416': (m, s) => renderSection416(m, s),
    'section-42': (m, s) => renderSection42(m, s),
    'section-421': (m, s) => renderSection421(m, s),
    'section-444': (m, s) => renderSection444(m, s),
    'section-446': (m, s) => renderSection446(m, s),
    'section-4501': (m, s) => renderSection4501(m, s),
    'section-451': (m, s) => renderSection451(m, s),
    'section-457': (m, s) => renderSection457(m, s),
    'section-45l': (m, s) => renderSection45l(m, s),
    'section-45q': (m, s) => renderSection45Q(m, s),
    'section-45v': (m, s) => renderSection45V(m, s),
    'section-45w': (m, s) => renderSection45W(m, s),
    'section-45x': (m, s) => renderSection45X(m, s),
    'section-460': (m, s) => renderSection460(m, s),
    'section-461': (m, s) => renderSection461(m, s),
    'section-461l': (m, s) => renderSection461L(m, s),
    'section-467': (m, s) => renderSection467(m, s),
    'section-469': (m, s) => renderSection469(m, s),
    'section-47': (m, s) => renderSection47(m, s),
    'section-471': (m, s) => renderSection471(m, s),
    'section-472': (m, s) => renderSection472(m, s),
    'section-475': (m, s) => renderSection475(m, s),
    'section-475f': (m, s) => renderSection475f(m, s),
    'section-48': (m, s) => renderSection48(m, s),
    'section-481': (m, s) => renderSection481(m, s),
    'section-481a': (m, s) => renderSection481a(m, s),
    'section-482': (m, s) => renderSection482(m, s),
    'section-483': (m, s) => renderSection483(m, s),
    'section-48c': (m, s) => renderSection48C(m, s),
    'section-4940': (m, s) => renderSection4940(m, s),
    'section-4941': (m, s) => renderSection4941(m, s),
    'section-4942': (m, s) => renderSection4942(m, s),
    'section-4943': (m, s) => renderSection4943(m, s),
    'section-4944': (m, s) => renderSection4944(m, s),
    'section-4945': (m, s) => renderSection4945(m, s),
    'section-4958': (m, s) => renderSection4958(m, s),
    'section-4960': (m, s) => renderSection4960(m, s),
    'section-4972': (m, s) => renderSection4972(m, s),
    'section-4973': (m, s) => renderSection4973(m, s),
    'section-4974': (m, s) => renderSection4974(m, s),
    'section-4975': (m, s) => renderSection4975(m, s),
    'section-4980d': (m, s) => renderSection4980d(m, s),
    'section-4980h': (m, s) => renderSection4980h(m, s),
    'section-51': (m, s) => renderSection51(m, s),
    'section-511': (m, s) => renderSection511(m, s),
    'section-529': (m, s) => renderSection529(m, s),
    'section-530': (m, s) => renderSection530(m, s),
    'section-59a': (m, s) => renderSection59A(m, s),
    'section-6011': (m, s) => renderSection6011(m, s),
    'section-6015': (m, s) => renderSection6015(m, s),
    'section-6033': (m, s) => renderSection6033(m, s),
    'section-6038': (m, s) => renderSection6038(m, s),
    'section-6038a': (m, s) => renderSection6038a(m, s),
    'section-6038b': (m, s) => renderSection6038b(m, s),
    'section-6038d': (m, s) => renderSection6038D(m, s),
    'section-6039': (m, s) => renderSection6039(m, s),
    'section-6041': (m, s) => renderSection6041(m, s),
    'section-6045': (m, s) => renderSection6045(m, s),
    'section-6045a': (m, s) => renderSection6045A(m, s),
    'section-6045b': (m, s) => renderSection6045b(m, s),
    'section-6048': (m, s) => renderSection6048(m, s),
    'section-6049': (m, s) => renderSection6049(m, s),
    'section-6050w': (m, s) => renderSection6050W(m, s),
    'section-6051': (m, s) => renderSection6051(m, s),
    'section-6072': (m, s) => renderSection6072(m, s),
    'section-6111': (m, s) => renderSection6111(m, s),
    'section-6112': (m, s) => renderSection6112(m, s),
    'section-6159': (m, s) => renderSection6159(m, s),
    'section-6166': (m, s) => renderSection6166(m, s),
    'section-6213': (m, s) => renderSection6213(m, s),
    'section-6221': (m, s) => renderSection6221(m, s),
    'section-6321': (m, s) => renderSection6321(m, s),
    'section-6325': (m, s) => renderSection6325(m, s),
    'section-6330': (m, s) => renderSection6330(m, s),
    'section-6331': (m, s) => renderSection6331(m, s),
    'section-6404': (m, s) => renderSection6404(m, s),
    'section-6502': (m, s) => renderSection6502(m, s),
    'section-6601': (m, s) => renderSection6601(m, s),
    'section-6651': (m, s) => renderSection6651(m, s),
    'section-6654': (m, s) => renderSection6654(m, s),
    'section-6655': (m, s) => renderSection6655(m, s),
    'section-6662': (m, s) => renderSection6662(m, s),
    'section-6663': (m, s) => renderSection6663(m, s),
    'section-6664': (m, s) => renderSection6664(m, s),
    'section-6672': (m, s) => renderSection6672(m, s),
    'section-6694': (m, s) => renderSection6694(m, s),
    'section-6695': (m, s) => renderSection6695(m, s),
    'section-67': (m, s) => renderSection67(m, s),
    'section-6700': (m, s) => renderSection6700(m, s),
    'section-6707a': (m, s) => renderSection6707a(m, s),
    'section-6724': (m, s) => renderSection6724(m, s),
    'section-691': (m, s) => renderSection691(m, s),
    'section-707': (m, s) => renderSection707(m, s),
    'section-71-alimony': (m, s) => renderSection71Alimony(m, s),
    'section-7122': (m, s) => renderSection7122(m, s),
    'section-72p': (m, s) => renderSection72p(m, s),
    'section-72t': (m, s) => renderSection72t(m, s),
    'section-731': (m, s) => renderSection731(m, s),
    'section-7345': (m, s) => renderSection7345(m, s),
    'section-736': (m, s) => renderSection736(m, s),
    'section-743': (m, s) => renderSection743(m, s),
    'section-7430': (m, s) => renderSection7430(m, s),
    'section-7491': (m, s) => renderSection7491(m, s),
    'section-7508a': (m, s) => renderSection7508a(m, s),
    'section-752': (m, s) => renderSection752(m, s),
    'section-754': (m, s) => renderSection754(m, s),
    'section-7701b': (m, s) => renderSection7701B(m, s),
    'section-7701o': (m, s) => renderSection7701o(m, s),
    'section-7702': (m, s) => renderSection7702(m, s),
    'section-7702a': (m, s) => renderSection7702a(m, s),
    'section-7811': (m, s) => renderSection7811(m, s),
    'section-7872': (m, s) => renderSection7872(m, s),
    'section-7874': (m, s) => renderSection7874(m, s),
    'section-79': (m, s) => renderSection79(m, s),
    'section-83': (m, s) => renderSection83(m, s),
    'section-86': (m, s) => renderSection86(m, s),
    'section-871': (m, s) => renderSection871(m, s),
    'section-871a': (m, s) => renderSection871A(m, s),
    'section-871m': (m, s) => renderSection871m(m, s),
    'section-877a': (m, s) => renderSection877A(m, s),
    'section-882': (m, s) => renderSection882(m, s),
    'section-884': (m, s) => renderSection884(m, s),
    'section-894': (m, s) => renderSection894(m, s),
    'section-897': (m, s) => renderSection897(m, s),
    'section-901': (m, s) => renderSection901(m, s),
    'section-901j': (m, s) => renderSection901J(m, s),
    'section-904': (m, s) => renderSection904(m, s),
    'section-911': (m, s) => renderSection911(m, s),
    'section-951': (m, s) => renderSection951(m, s),
    'section-951a': (m, s) => renderSection951A(m, s),
    'section-956': (m, s) => renderSection956(m, s),
    'section-962': (m, s) => renderSection962(m, s),
    'section-988': (m, s) => renderSection988(m, s),
    'section-989': (m, s) => renderSection989(m, s),
    'sector-heatmap': (m, s) => renderSectorHeatmap(m, s),
    'sector-rotation': (m, s) => renderSectorRotation(m, s),
    'sector-rotation-strategy': (m, s) => renderSectorRotationStrategy(m, s),
    'sector-timing': (m, s) => renderSectorTiming(m, s),
    'sectors': (m, s) => renderSectors(m, s),
    'sentiment': (m, s) => renderSentiment(m, s, ''),
    'sentiment-velocity': (m, s) => renderSentimentVelocity(m, s),
    'sep-ira': (m, s) => renderSepIra(m, s),
    'series-smoother': (m, s) => renderSeriesSmoother(m, s),
    'settings': (m, s) => renderSettings(m, s),
    'shares': (m, s) => renderShares(m, s),
    'short-interest': (m, s) => renderShortInterest(m, s, ''),
    'signal-decomposition': (m, s) => renderSignalDecomposition(m, s),
    'simple-ira': (m, s) => renderSimpleIra(m, s),
    'sizing': (m, s) => renderPositionSize(m, s),
    'slat': (m, s) => renderSlat(m, s),
    'solo-401k': (m, s) => renderSolo401k(m, s),
    'sp500-predict': (m, s) => renderSp500Predict(m, s),
    'splits-history': (m, s) => renderSplitsHistory(m, s, ''),
    'squeeze-scanner': (m, s) => renderSqueezeScanner(m, s),
    'state-tax': (m, s) => renderStateTax(m, s),
    'str-loophole': (m, s) => renderStrLoophole(m, s),
    'strategy-alerts': (m, s) => renderStrategyAlerts(m, s),
    'subscriptions': (m, s) => renderSubscriptions(m, s),
    'supply-chain': (m, s) => renderSupplyChain(m, s, ''),
    'symbol-changes': (m, s) => renderSymbolChanges(m, s),
    'tags': (m, s) => renderTags(m, s),
    'tape': (m, s) => renderTape(m, s),
    'tape-replay': (m, s) => renderTapeReplay(m, s, ''),
    'tax-lots': (m, s) => renderTaxLots(m, s),
    'tax-workshop': (m, s) => renderTaxWorkshop(m, s),
    'toast-history': (m) => renderToastHistory(m),
    'top-news': (m, s) => renderTopNews(m, s),
    'top-signals': (m, s) => renderTopSignals(m, s),
    'trade-compare': (m, s) => renderTradeCompare(m, s),
    'trades': (m, s) => renderTradesView(m, s),
    'travel-per-diem': (m, s) => renderTravelPerDiem(m, s),
    'tts-qualification': (m, s) => renderTtsQualification(m, s),
    'tts-scorer': (m, s) => renderTtsScorer(m, s),
    'tutorial': (m, s) => renderTutorial(m, s),
    'unusual-options': (m, s) => renderUnusualOptions(m, s, ''),
    'uoa-stream': (m, s) => renderUoaStream(m, s),
    'uspto-patents': (m, s) => renderUsptoPatents(m, s, ''),
    'var-calculator': (m, s) => renderVarCalculator(m, s),
    'vasicek': (m, s) => renderVasicek(m, s),
    'vol': (m, s) => renderVol(m, s),
    'vol-smile': (m, s) => renderVolSmile(m, s),
    'vol-surface': (m, s) => renderVolSurface(m, s),
    'vrp': (m, s) => renderVrp(m, s),
    'walk-forward': (m, s) => renderWalkForward(m, s),
    'wash-sale-tracker': (m, s) => renderWashSaleTracker(m, s),
    'watchlists': (m, s) => renderWatchlists(m, s),
    ...
    'lease-renewal': (m, s) => renderLeaseRenewal(m, s),
    ...
    'estimate': (m, s) => renderEstimate(m, s),
    'purchase-order': (m, s) => renderPurchaseOrder(m, s),
    'sublease': (m, s) => renderSublease(m, s),
    ...
    'llc-operating-agreement': (m, s) => renderLlcOperatingAgreement(m, s),
    ...
    'land-contract': (m, s) => renderLandContract(m, s),
    'lease-assignment': (m, s) => renderLeaseAssignment(m, s),
    ...
    'final-paycheck': (m, s) => renderFinalPaycheck(m, s),
    ...
    'years-to-fi': (m, s) => renderYearsToFi(m, s),
    ...
    'traditional-ira-deduction': (m, s) => renderTraditionalIraDeduction(m, s),
    ...
    'saas-quick-ratio': (m, s) => renderSaasQuickRatio(m, s),
    ...
    'bond-pricing': (m, s) => renderBondPricing(m, s),
    'cash-out-refinance': (m, s) => renderCashOutRefinance(m, s),
    ...
    'preferred-stock': (m, s) => renderPreferredStock(m, s),
